...
query.weights[0] = ecc_weight(sx.faces(S, 0))
query.weights[1] = ecc_weight(sx.faces(S, 1))
query.weights[2] = ecc_weight(sx.faces(S, 2))

from primate.functional import numrank
query.p_solver = lambda L: numrank(L, atol=2.0, gap = "simple", confidence=0.80, verbose=True)
query.q_solver = lambda L: numrank(L, atol=2.0, gap = "simple", confidence=0.80, verbose=True)

query.p_solver = lambda L: hutchpp(L, fun="numrank", atol=0.50, verbose=True, b=64, maxiter=250, quad="fttr")

hutchpp(A, fun="smoothstep", atol=0.50, verbose=True, nb=64, maxiter=250, quad="fttr", a=1e-12, b=1e-8)
hutchpp(A, fun="numrank", atol=0.50, verbose=True, nb=128, maxiter=250, quad="fttr", threshold=1e-6)
numrank(A)

from scipy.sparse import save_npz
save_npz("challenge_1328_rank871.npz", A)

# 1.48, 1.68, 1.70, 1.88
query(i=1.48, j=1.68, terms=True)
A = query.operator(term=2, i=1.48, j=1.68, deflate=True)
# 824, 295, 871, 346

from primate.trace import hutchpp, xtrace
from primate.operator import matrix_function
M = matrix_function(A, fun="numrank", threshold=1e-2)
hutchpp(M, atol=0.50, verbose=True, maxiter=2000, quad="fttr")

## TODO: make a custom callback that, instead of checking that number of points equals something, 
## we early-stop the convergence once we know the estimate is *not* equal something  
xtrace(M, verbose=True)


# hutchpp(M, atol=0.50, verbose=True, b = 64, maxiter=200, quad="fttr")
# hutchpp(M, atol=0.50, verbose=True, b = 64, maxiter=50, quad="fttr", deg = 2, orth=0)
