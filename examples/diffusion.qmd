...
NV = 100
X_data = mpeg7(simplify=NV+1, which=['turtle', 'bird', 'hammer'], shape_nums = np.arange(8))
X, y = mpeg7(simplify=NV+1, which=['turtle', 'bird', 'hammer'], shape_nums = np.arange(8), return_X_y = True)
S = cycle_graph(NV)
```
...
p = column(
  row([patch_plot(x.reshape(NV, 2), "red") for x in X[y == 0]]),
  row([patch_plot(x.reshape(NV, 2), "blue") for x in X[y == 1]]), 
  row([patch_plot(x.reshape(NV, 2), "green") for x in X[y == 2]])
)
show(p)
```
...
## Effective seems like a good heuristic
HK.timepoints = geomspace(*HK.time_bounds("informative", interval=[0.0,1.00]), p=32, reverse=False)
hks = HK.signature(HK.timepoints)
# hks, _ = HK.sihks(HK.timepoints)
...
def figure_scatter(X, scatter_kwargs = {}, **kwargs):
  p = figure(**kwargs)
...
for i in range(6):
  ...
  sigs_norm = np.diag(np.reciprocal(sigs.sum(axis=1))) @ sigs
  sigs_norm_shift = np.array([signal_dist(s1, s2) for s1,s2 in combinations(sigs_norm, 2)])
  ...
  p = figure_scatter(emb, scatter_kwargs=dict(color=["red"]*7 + ["blue"]*7 + ["green"]*7), width=180, height=180)
  p.toolbar_location = None
  ...
  for j in np.unique(y):
    ...
    scatters_figs = [figure_scatter(x.reshape(NV, 2), scatter_kwargs = dict(color=bin_color(sig, "turbo")), width=60, height=60) for x, sig in shape_sig_it]
...
# sigs = np.array([hk.signature()[:,0] for hk in HKS])
# sigs_norm = np.diag(np.reciprocal(sigs.sum(axis=1))) @ sigs
# sigs_norm_shift = np.array([signal_dist(s1, s2) for s1,s2 in combinations(sigs_norm, 2)])

# show(figure_dist(sigs_norm_shift), palette = "turbo")
# show(figure_dist(pdist(sigs_norm)), palette = "turbo")
```
