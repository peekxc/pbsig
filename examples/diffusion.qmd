...
def figure_heat(S: ComplexLike, X: ArrayLike, vertex_weights: Sequence, **kwargs):
  p = figure_complex(S, X, **kwargs)
  p.scatter(*X.T, color=bin_color(vertex_weights, "inferno"), size=10.5)
  return p

## Show heat diffusion from a single vertex 
ind = np.argmin(X @ np.array([0,1]))
HK = HeatKernel(S).param_laplacian("mesh", X, normed=False).fit()
HK.timepoints = (6, "informative")
figs = [figure_heat(S, X, heat, width=200, height=200) for heat in HK.diffuse(subset=ind)]
show(row(*figs))
```
...
NV = 100
data_params = dict(simplify = NV+1, which=['turtle', 'bird', 'hammer'], shape_nums = np.arange(8))
...
p = column(
  ...
  row([patch(x.reshape(NV, 2), "blue") for x in X[y == 1]]), 
  row([patch(x.reshape(NV, 2), "green") for x in X[y == 2]])
)
show(p)
```
...
X_pos = X_data[('turtle',2)]
S = cycle_graph(X_pos.shape[0])
triangles = list(cycle_window(range(X_pos.shape[0]), offset=4, w=5))
S.update(triangles)
```
...
from pbsig.linalg import * 
HK = HeatKernel(S).param_laplacian(approx="mesh", X=X_pos, normed=False).fit()
...
## Fit the heat kernel 
HKS = [HK.clone().param_laplacian("mesh", x.reshape(NV, 2), normed=False).fit() for x in X]
...
for i in range(6):
  ...
  sigs_norm = np.reciprocal(sigs.sum(axis=1))[:,np.newaxis] * sigs
  sigs_norm_shift = pdist(sigs_norm, metric=signal_dist)
  # sigs_subset = np.array([hk.signature(subset=landmarks(X_data[k], 30)[0])[:,i] for k, hk in zip(shape_keys, HKS)])
  emb = cmds(squareform(sigs_norm_shift**2))

  ## MDS plot
  p = figure_scatter(emb, color=["red"]*7 + ["blue"]*7 + ["green"]*7, width=180, height=180)
...
To do this for multiple values of `t`, one can perform the same steps with a slight augmentation: in step 1, rather than minimizing cross correlation, 
minimize multiple cross correlation simulataneously across mutliple signals. This is like simultanous signal phase alignment. Then, in step 2, use 
...
## Need a randomized transformer 
## TODO: How do I generically store the heat kernel object? What is its contract?
## a) 
...
class RandomizedVectorizer(_VectorizerMixin, BaseEstimator):
  def __init__(self, random_state = None, dimension: int, **kwargs):
  ...
  @staticmethod
  def sample(size: Union[int, tuple]) -> np.ndarray:
  ...
  def fit(self, X: ArrayLike = None, y: Optional[np.ndarray] = None):
...
class HypothesisSpace(BaseEstimator):
  """A _hypothesis space_ is a space of estimators/classifiers that depend on some random state.
  

  """
  def __init__(estimator: BaseEstimator, hspace: Callable, random_state = None):
    self.estimator = estimator ## deterministic estimator 
    self.hspace = hspace       ## hypothesis space -- a transformer with support for a random state
    self.random_state = random_state
  ...
  def fit(X: Sized, y: np.ndarray, sample_weight = None):
    ...
    self.estimator.fit(self.hspace(rng))
