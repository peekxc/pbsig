...
def figure_heat(S: ComplexLike, X: ArrayLike, vertex_weights: Sequence, lb: float = None, ub: float = None, **kwargs):
  p = figure_complex(S, X, **kwargs)
  v_color = bin_color(vertex_weights, color_pal="inferno", lb=lb, ub=ub) if lb is not None else bin_color(vertex_weights, color_pal="inferno")
...
figs = [figure_heat(S, X, heat, lb = 0.0, ub = max(heat), width=200, height=200) for heat in heat_diffusion] # geometrically accurate
# figs = [figure_heat(S, X, heat, width=200, height=200) for heat in heat_diffusion] # pretty, nice geometrically accurate
show(row(*figs))
```
...
```{python}
#| label: TO REPLACE: adds triangles along a sliding window such that there is non-trivial curvature 
...
for each timepoint/signature, as although they are all 1-dimensional as the metric varies per pair. 
...
```{python}
#| label: Minimum HKS distances for varying t 
...
for i in range(6):
  ...
  ## MDS plot
  p = figure_scatter(emb, color=["red"]*7 + ["green"]*7 + ["blue"]*7, title="MDS emb of min HKS dist", width=180, height=180)
  ...
  ## Distance plot 
  figs.append(column(figure_dist(sigs_norm_shift, title="min HKS dist", width=180, height=180)))
...
Next, we generate an "average" heat kernel signature for each class of shapes at some fixed time `t > 0`. To do this, we phase-align the HKS of each signature at time `t` to fixed but arbitrary reference signature by minimizing the cross-correlation. This gives a canonical correspondence 
...
To do this for multiple values of `t`, one can perform the same steps with a slight augmentation: in step 1, rather than minimizing cross correlation, minimize multiple cross correlation simulataneously across multiple signals. This is like simultanous signal phase alignment. Then, in step 2, use generalized procrustes analysis...
...
norm_dist = partial(signal_dist, normalize="sum") # improves scores dramatically, but O(n^2)
...
profiler.add_function(HK_classifier.fit)
profiler.add_function(ab.fit)
profiler.add_function(HK_classifier.predict_proba)
...
normed = False
timepoints = geomspace(1e-2, 1024, 30)
HK = HeatKernel(approx="unweighted")
# HK1 = HK.fit(S=S1, normed=normed, shift_invert=False)
# HK2 = HK.fit(S=S2, normed=normed, shift_invert=False)
# HK3 = HK.fit(S=S3, normed=normed, shift_invert=False)
# HK4 = HK.fit(S=S4, normed=normed, shift_invert=False)

## The trace is still discriminating when normalized by |V|, but not by too much (about 10% off)
p = figure(width=250, height=150, x_axis_type="log")
p.line(timepoints, HK.fit(S=S1, normed=normed, shift_invert=False).trace(timepoints)/4, color="red")
p.line(timepoints, HK.fit(S=S2, normed=normed, shift_invert=False).trace(timepoints)/5, color="green")
p.line(timepoints, HK.fit(S=S3, normed=normed, shift_invert=False).trace(timepoints)/15, color="blue")
p.line(timepoints, HK.fit(S=S4, normed=normed, shift_invert=False).trace(timepoints)/15, color="cyan")
show(p)

## Up to a constant shift, this looks very similar to the paper
p = figure(width=250, height=150, x_axis_type="log")
p.line(timepoints, HK.fit(S=S1, normed=normed, shift_invert=False).trace(timepoints), color="red")
p.line(timepoints, HK.fit(S=S2, normed=normed, shift_invert=False).trace(timepoints), color="green")
p.line(timepoints, HK.fit(S=S3, normed=normed, shift_invert=False).trace(timepoints), color="blue")
p.line(timepoints, HK.fit(S=S4, normed=normed, shift_invert=False).trace(timepoints), color="cyan")
show(p)
