...
ew, ev = eigsh(L, k=L.shape[1]-1, which="LM", sigma=1e-6)
heat_gen = diffuse_heat(ev, ew, logspaced_timepoints(5, lb=0.2, ub=50.0), subset=ind)
...
X, y = mpeg7(simplify=200, which=['turtle', 'bird'], return_X_y = True)
...
HK.time_bounds("absolute")
HK.time_bounds("laplacian")
HK.time_bounds("informative")
HK.time_bounds("heuristic")
HK.timepoints = geomspace(*HK.time_bounds("informative", interval=[0.99,1.0]), p=32, reverse=False)
...
```{python}
# emb = cmds(squareform(pdist(si)**2))
p = figure(width=200, height=200)
p.scatter(*X_pos.T, color=bin_color(np.linalg.norm(si - si[ind], axis=1), "turbo"))
show(p)
