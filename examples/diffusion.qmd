...
L_sim = up_laplacian(S, weight=gauss_similarity(S, X_pos), normed=True)
# logspaced_timepoints(30, lb=1e-10, ub=max(eigsh(L_sim)[0]))
...
hks = heat_kernel_signature(L_sim, timepoints=np.array([0.000001]))
p = figure_complex(S, X_pos, height=250, width=250)
p.scatter(*X_pos.T, color=bin_color(hks[:,0].flatten(), "turbo"), size=10.5)
show(p)
