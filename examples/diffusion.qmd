...
X_data = mpeg7(simplify=101, which=['turtle', 'bird'])
X, y = mpeg7(simplify=101, which=['turtle', 'bird'], return_X_y = True)
...
shape_keys = [('turtle', i) for i in range(1,6)] + [('bird', i) for i in range(1,6)]
HK = HeatKernel(S)
...
for i in range(6):
  ...
  emb = cmds(squareform(pdist(sigs)**2))
  p = figure_scatter(emb, scatter_kwargs=dict(color=["red"]*5 + ["blue"]*5), width=125, height=125)
  p.toolbar_location = None
  p.xaxis.visible = False
  p.yaxis.visible = False
  figs.append(p)
  for k, sig in zip(shape_keys, sigs):
    p = figure_scatter(X_data[k], scatter_kwargs = dict(color=bin_color(sig, "turbo")), width=125, height=125)
    p.toolbar_location = None
    figs.append(p)
