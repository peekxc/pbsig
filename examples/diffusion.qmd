...
def figure_heat(S: ComplexLike, X: ArrayLike, vertex_weights: Sequence, lb, ub, **kwargs):
  ...
  # v_color = pass_kwargs(bin_color, **(dict(x=vertex_weights, color_pal="inferno") | kwargs))
...
HK = HeatKernel(S, timepoints=6, approx="mesh", bounds="informative").fit(X)
heat_diffusion = list(HK.diffuse(subset=ind))
# figure_heat(S, X, heat_diffusion[0], lb = 0.0, ub = max(heat_diffusion[0]), width=200, height=200)
