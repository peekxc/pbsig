...
A = diags(vertex_masses(S, X_pos)) # mass matrix
...
L_sim = up_laplacian(S, weight=gauss_similarity(S, X_pos), p=1, normed=False)

max_ew = eigsh(L_sim, M=A, k=1, which='LM')[0][0]
min_ew = eigsh(L_sim, M=A, k=2, sigma=1e-8, which='LM')[0][1]
timepoints = logspaced_timepoints(20, lb=min_ew, ub=max_ew, method="local")

l_mi, l_ma = eigsh(L_sim, k=4, which="BE", tol=1e-10)[0][np.array([1,3],dtype=int)] 

l_min, l_max = eigsh(L_sim, M=A, k=4, which="BE", tol=1e-10)[0][np.array([1,3],dtype=int)] # checks out
np.max(eigsh(LM, M=MM, k=10)[0])
np.max(eigsh(LM, k=10)[0])*np.max(A.diagonal())
...
from pbsig.linalg import heat_kernel_signature, logspaced_timepoints
# timepoints = logspaced_timepoints(64, ub=0.5)
...
p = figure_complex(S, X_pos, height=250, width=250)
p.scatter(*X_pos.T, color=bin_color(np.linalg.norm(hks - hks[ind], axis=1), "turbo"), size=10.5)
show(p)
