...
LG = up_laplacian(SimplicialComplex(edges | compl)).todense()

from pbsig.linalg import * 
assert is_symmetric(LM) and is_symmetric(LC)
L1 = -np.sort(-np.linalg.eigvalsh(LM))
L2 = -np.sort(-np.linalg.eigvalsh(LC))
L3 = -np.sort(-np.linalg.eigvalsh(LG))
D3 = np.ravel(LG.diagonal())
A3 = np.sort(np.linalg.eigvalsh(-(LG - diags(D3))))
np.sort(n - L1)
np.sort(L2)
np.sort(L3)
...
Test homotopy between scalar functions equipped to laplacian to see how nuclear norms of various subgraphs behave 
```{python}
from pbsig.utility import cycle_window
from itertools import combinations
n = 5
...
SV = []
ew0 = np.random.uniform(size=comb(n,2), low=0, high=5)
for i in range(20):
  ew1 = np.random.uniform(size=comb(n,2), low=0, high=5)
  for alpha in np.linspace(0, 1, 100):
    ew = alpha*ew1 + (1-alpha)*ew0
    sf = lambda s: ew[rank_lex(s, n=n)] if len(s) == 2 else 1
    LM = up_laplacian(SimplicialComplex(edges), weight=lambda s: sf(s)).todense()
    LC = up_laplacian(SimplicialComplex(compl), weight=lambda s: sf(s)).todense()
    LG = up_laplacian(SimplicialComplex(edges | compl), weight=lambda s: sf(s)).todense()
    sm = np.sum(np.sqrt(np.maximum(0.0, np.linalg.eigvalsh(LM))))
    sc = np.sum(np.sqrt(np.maximum(0.0, np.linalg.eigvalsh(LC))))
    sg = np.sum(np.sqrt(np.maximum(0.0, np.linalg.eigvalsh(LG))))
    SV.append((sm,sc,sg))
  ew0 = ew1.copy()

ew,ev = np.linalg.eigh(LG)
AG = LG - diags(np.ravel(LG.diagonal()))
(AG @ ev[:,2])/ev[:,2]
...
p = figure()
p.line(I, SV[:,0], color="orange")
p.line(I, SV[:,1], color="red")
p.line(I, SV[:,2], color="blue")
p.line(I, SV[:,2]-SV[:,0], color="green")
p.line(I, 0.05*(SV[:,2]*SV[:,0]), color="black")
show(p) 
# blue should always lie above orange and red; 
# In theory, green and red should have similar shapes + similar minima/maxima 
# Moreover, we hope minimizing red is like maximizing orange, possibly up to normalization
