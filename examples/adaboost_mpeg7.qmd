...
class AdaBoostClassifier:
  ...
  def fit(self, X: ArrayLike, y: ArrayLike, n_estimators: int = 100):
    ...
    # wt * np.exp(at * np.where(yt != y, 1, 0)) # (1)
    # wt * np.exp(at * np.where(yt == y, -1, 1)) # (2)
    # np.where(yt != y, wt * np.sqrt((1.0-et)/et), wt * np.sqrt(et/(1.0-et))) # (3)
    # (2) matches (3)
    # 0.5 * np.log((1+(1-2*et))/(1 - (1-2*et)))         ## alternative but equivalent coefficient calculation
  ...
  def predict(self, X: ArrayLike) -> ArrayLike:
    return self.classes_[np.where(A.decision_function(X_train) < 0, 0, 1)]
