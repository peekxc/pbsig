...
class AdaBoostClassifier:
  ...
  def fit(self, X: ArrayLike, y: ArrayLike, n_estimators: int = 100):
    ...
    for t in range(n_estimators):
      ...
      at = 0.5 * np.log((1.0-et)/et)              ## voting coefficient update  
      # wt = wt * np.exp(at * np.where(yt != y, 1, 0))
...
A_skl = AdaBoostClassifier(ShellsClassifier(bins=10), n_estimators=30, algorithm="SAMME", random_state=0)
A_skl.fit(X_train, y)
# np.sum(A_skl.predict(X_train) == y)/len(y)

list(A_skl.staged_score(X_train, y))
