...
\begin{equation}\label{eq:alt_sum}
	\partial_p(\sigma) = \partial_p([v_0, v_1, \dots, v_p]) := \sum_{i=0}^p (-1)^i [v_0, \dots, v_{i-1}, v_{i+1}, \dots v_p]
\end{equation}
...
and let $\Delta_{+}^m = \{ (i,j) : 0 \leq i \leq j \leq m \}$ denote the set of possible filtration index pairs. 
