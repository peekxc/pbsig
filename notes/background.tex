...
%\end{equation}
where $\pi$ is a permutation on $[\,p+1\,]$ and $\lvert \pi \rvert$ the number of its inversions. 
...
We will at times need to generalize the notation given thus far to the \emph{parameterized} setting. Toward this end, for some $\mathcal{A} \subset \mathbb{R}^d$, define a $\mathcal{A}$-parameterized as the pair $(K, f_\alpha)$ where $K$ is a simplicial complex and $f : K \times \mathcal{A} \to \mathbb{R}$ an $\mathcal{A}$-parameterized filter function satisfying:
$$	 
...
%\end{enumerate}
We will often refer to $\alpha$ as the \emph{time} parameter. Examples of parameterized filtrations include:
\begin{enumerate}
	...
	\item (Dynamic Rips) When $f = \mathrm{diam}$ is the diameter function over subsets of a metric space $(X,d_X)$ and $\mathcal{A} = \mathbb{R}$, the pair $(K, f_\alpha)$ recovers a natural filtered-equivalent of the dynamic metric space studied in~\cite{kim2021spatiotemporal}  
	\item ($\mathbb{R}^d$-indexed Multi-filtrations) By filtering $K$ along several functions spanning $\mathbb{R}^d$, we recover... 
\end{enumerate} 
