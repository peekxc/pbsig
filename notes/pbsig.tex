...
%~\ref{eq:measure}
The primary advantage the rank-based expressions from~\eqref{eq:betti_four}-\eqref{eq:mu_four} have compared to classical methods in~\cite{edelsbrunner2022computational, zomorodian2004computing} is that their computations are performed directly on \emph{unfactored} boundary (sub)matrices. Restricting the field of choice to $\mathbb{F} = \mathbb{R}$, a natural question arises: computationally, how do expressions~\eqref{eq:betti_four} and~\eqref{eq:mu_four} relate to their measure-theoretic counter-parts~\eqref{eq:pbn_cont} and~\eqref{eq:measure}?
