...
Persistence has proven to be the de facto connection between homology theory and the application frontier: whether for shape recognition, computer vision, metric learning, dimensionality reduction, or time series analysis, researchers have found ways of exploiting the rich information contained in persistence diagrams.
...
These vectorizations have proven useful for learning applications due to their stability and configurability~\cite{}, however, they are limited practically as diagrams are prerequisite to their computation.
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), the boundary matrix $\partial(K_\bullet)$ must maintain a simplexwise filtration order to preserve the inclusion structure of $(K_\bullet, f_\alpha)$.
In contrast, recall that the rank function is permutation invariant for any $A \in \mathbb{R}^{n \times n}$: 
$$ \mathrm{rank}(A) = \mathrm{rank}(P^T A  P) $$
...
This suggests the expressions from~\eqref{eq:betti_four} and~\eqref{eq:mu_four} need not maintain this order---as long as each constitutive term has the same non-zero pattern as its filtration-ordered counterpart, their ranks will be identical.

To more formally convey the computational utility of permutation invariance in parameterized settings, consider the following \emph{parameterized} boundary operator $\partial_p$ definition:
...
\end{equation}
Note in~\eqref{eq:rank_equiv_param} we write $\partial_p^\ast(K_\bullet)$ and $\partial_p^\ast(K)$ to make the distinction that the boundary operator of the former is explicitly filtered in the traditional simplexwise manner, whereas the latter is ordered arbitrarily via the linear extension $\preceq^\ast$ from definition~\ref{def:time_boundary_matrix}. 
...
\\
\noindent
\textbf{Regularization:} Another common approach used in sparse inverse problems to relax the rank function is to impose a regularization scheme. 
...
This fact can be attributed to the spectral theorem~\cite{bhatia2013matrix}. A similar proof can show this using the inertia theorem. %Section I.2 of stewart 
} that $\sigma_i(X) > 0$ whenever $\mathrm{rank}(X) \leq i$ to derive a spectral relaxation of the rank function. In this section, we exploit another well known identity of the rank function: 
...
The \emph{graph Laplacian} $L$ given in terms of adjacency, incidence, and element-wise interpretations, is defined as: 
\begin{equation}
...
%	\simeq H() 
\end{equation}
All three operators $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are symmetric, non-negative, and compact.
...
\end{equation}
Horak and Jost developed a study of the spectra of combinatorial Laplacians that unifies many of the Laplace operators on simplicial complexes~\cite{}. 
...
Many researchers studying higher order combinatorial Laplacians have found applications to areas such as graph optimization~\cite{}, network circuit theory~\cite{}, and graph sparsification. 

Various difficulties arise when these Laplacian operators are equipped with weight functions. 
...
By choosing the weights, one effectively determines the corresponding inner product, which in-turn determines the spectral range of the corresponding operator. Computationally, the weights of the simplices $\sigma \in K$ manifest in the corresponding Laplace operators via positive diagonal matrices $W_\ast = \mathrm{diag}(\{w(\sigma_i) \}_{i=1}^n)$, 
...
\end{equation}
Since the right hand side of~\eqref{eq:l_up} is of the form $W^{-1} P W$ where $P$ is symmetric positive semi-definite and $W$ is a positive diagonal matrix, though the weighted $L_p^{\text{up}}$ may not be a symmetric operator, it is matrix-similar to one. 
...
This is markedly improved in the case of the graph Laplacian derived from a graph $G = (V, E)$: the $x \mapsto \partial_p \partial_p^T x$ operation can be carried out in a time linear in $\lvert E \rvert$ by summing across edges---any storage representation of $G$ can be used, and $L$ need not be explicitly constructed.
...
\end{flalign}
Observe that, if $G = (V, E)$ has $m$ edges and $n$ vertices taking labels in the set $[n]$, the matrix-vector product in~\eqref{eq:lap_quad_from} can be evaluated in $O(m)$ time and $O(n)$ storage via two $O(m)$ edge traversals: one to accumulate the vertex degrees and one to subtract components $x_j$ from incident edges at $i$. By storing the precomputed vertex degrees, this computation reduces to one $O(n)$ vector multiplication and a single $O(m)$ edge pass. Unfortunately, it is not immediately clear how to generalize~\eqref{eq:lap_quad_from}  to higher dimensions $p > 0$, as the orientations of $[\sigma] \in K^{p+1}$ change the sign of the off-diagonal entries in $L_p^\ast$. 
...
Noting the observations in section~\ref{sec:laplacian_theory}, we exclusively consider the symmetric version of $\hat{L}_p^{\mathrm{up}}$, as the down-Laplacian $\hat{L}_p^{\mathrm{dn}}$ can be handled in essentially the same way. 
Extending~\eqref{eq:lap_quad_from} to higher dimensions requires a generalization of the path-connected relation $i \sim j$. Let $\mathrm{co}(\tau) = \{ \, \sigma \in K^{p+1} \mid \tau \subset \sigma \, \}$ denote the set of proper cofaces of $\tau \in K^p$, or \emph{cofacets}, and let 
$$\mathrm{deg}_f(\tau) = \sum_{\sigma \in \mathrm{co}(\tau)} f(\sigma) $$
denote the (weighted) \emph{degree} of $\tau$. Setting $f(\sigma) = 1$ for all $\sigma \in K$ recovers the integral notion of degree representing the number of cofacets a given simplex has. 
...
\end{equation}
Observe~\eqref{eq:l_up_matvec} can be evaluated now via a very similar two-pass algorithm as described for the graph Laplacian if the simplices of $K^{p+1}$ can be enumerated quickly both the sign term $s_{\tau, \tau'}$ and the indexing function $h$ can be evaluated efficiently. 
...
\begin{proposition}
	...
	\end{equation}
	in $O(m(p+1))$ time and $O(m)$ storage, where $n = \lvert K^p \rvert$ and $m = \lvert K^{p+1} \rvert$. 
