...
\end{equation}
where $X_\alpha$ is an $\alpha$-parameterized input data set, $\epsilon$ a relaxation parameter, and $\mathcal{R}$ a \emph{sieve} which sifts through $\mathcal{A}$, summarizing the topological and geometric behavior $X_\alpha$ exhibited with respect to all $\alpha \in \mathcal{A}$. 
...
Informally, $\mathcal{L}$ encodes specific geometric and topological information about $(K, f_\alpha)$, and $\Phi$ determines how this information is summarized in the mapping. 
...
This line of thought suggests other algebraic properties of the rank function---such as invariance under permutations and invariance under adjoint multiplication---may yield even simpler rank-based expressions even further.
...
\end{equation}
The fact that the Betti numbers of $K$ may be recovered via the nullity of $\Delta_p(K)$ has been studied previously, see e.g. Proposition 2.2 of~\cite{}. 
In fact, one is not strictly limited to using $\Delta_p$ for homological computations; as pointed out by~\cite{}, the spectra of $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are highly related:
...
where $A \doteq B$ and $A \stackrel{\cdot}{\cup} B$ denotes equivalence and union, respectively, between \emph{non-zero} elements of the multisets $A, B$.
Moreover, all three operators $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are symmetric, positive semidefinite, and compact.
 Thus, for the purpose of estimating $\beta_p$, it suffices to consider only one family of operators.
...
Here, a \emph{weight function} is a non-negative real-valued function defined over the faces of $K$:
$$ w: K \to \mathbb{R}_+ $$
...
%In fact, 
The set of weight functions and the choice of scalar product on $C^p(K, \mathbb{R})$ wherein elementary cochains are orthogonal are in one-to-one correspondence~\cite{} (see Appendix~\ref{sec:inner_products} for details). 
Given a weight function $w$, this correspondence permits us to write explicit matrix representatives of $\Delta_p$: 
...
\end{equation} 
where $W_p = \mathrm{diag}(\{ \, w(\sigma_i) \, \}_{i=1}^n)$ represents a non-negative diagonal weight matrices weighted by $w$ restricted to simplices $\sigma \in K^p$ and $W^+$ denotes the pseudoinverse. 
Note that Laplacian operators are uniquely determined by the choice of weight function---for example,~\eqref{eq:comb_lap} is determined by the constant map $w(\sigma) = 1$.  
...
\end{equation}
Since ~\eqref{eq:l_up} is of the form $W^{-1} P W$ where $P \in S_n^+$ and $W$ is a positive diagonal matrix, this rectifies the symmetry problem.
Towards bounding the spectra of $L_p^{\text{up}}$, Horek and Jost~\cite{} propose \emph{normalizing} $\Delta_p$ by augmenting the weight function: 
\begin{equation}
	w(\tau) = \sum\limits_{\tau \in \partial(\sigma) }w(\sigma) \quad \forall \; \tau \in K^{p}, \, \sigma \in K^{p+1}
\end{equation}
Performing this substitution for the $p$-simplices via $W_p \mapsto \mathcal{D}_p$ in~\eqref{eq:l_up} yields the operator:
...
\end{align*}
where $\mathcal{D}_p$ is referred to as the \emph{diagonal degree matrix} and $\mathcal{A}_p^{\text{up}}(w)$ is a weighted adjacency matrix, and $I_n$ is the identity matrix with $I(\tau) = \mathrm{sign}(w(\tau))$ (see Section~\ref{sec:comb_lap} for more details). The primary benefit of this normalization is that it guarantees $\Lambda( \mathcal{L}_p^{\text{up}}) \subseteq [0,p+2]$ for any choice of weight function, from which one may obtains several useful implications, such as tight bounds on the spectral norm~\cite{}. 
The same results holds for up-, down-, and combinatorial Laplacians.
Moreover, as we will show in a subsequent section, one obtains maybe stability properties with degree-normalization not shared otherwise. For these reasons, we exclusively consider the normalized combinatorial Laplacian operator in this effort.  
\begin{remark}
	Compared to~\eqref{eq:l_up}, is it worth remarking that one important quality lost is that $\mathcal{L}_p^{\text{up}}$ may no longer be diagonally dominant (though it is PSD). 
\end{remark}
...
where $Q \in \mathbb{R}^{n \times n}$ is non-singular and $A, \tilde{A} \in \mathbb{R}^{n \times n}$ are the original and perturbed matrices, respectively. If the perturbation is small, the intuition is that $Q^T Q$ will be close to identity. This is captured by an extension of Ostrowski's Theorem, given by Li~\cite{}. 
 $$ \frac{\lvert \tilde{\lambda}_j - \lambda_j \rvert}{\lvert \lambda_j\rvert } \leq \lVert I - Q^T Q\rVert_2 \quad \Leftrightarrow \quad\tilde{\lambda_j} = \lambda_j(1 + \delta_j), \quad \lvert \delta_j \rvert \leq \lVert I - Q^T Q\rVert_2 $$
 Thus, if 
