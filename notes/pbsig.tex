...
Persistence-related pipelines typically follow a well-established pattern: given input data set $X$, construct a filtered simplicial complex $(K, f)$ from $X$ such that useful topological or geometric information may be profitably extracted from its \emph{persistence diagram}~\cite{} or "barcodes."
%$$ \mathrm{dgm}(K, f) = \{ (i,j) \in \mathbb{R}^2 : i < j \}$$ 
Historically, persistence was thought of as a shape descriptor~\cite{} due to its effectiveness in tackling the \emph{homology inference problem}: given a finite data $X$ sampled from a topological space $\mathcal{X}$, can one infer the homology of $\mathcal{X}$ from $X$ with high confidence?
...
The cornerstone of persistence is that the stability of topological features with respect to $f$ is demarcated by a pairing of simplices defined using homomorphisms between homology groups. 
That this pairing exists, is induced by simplicial inclusion maps, and is unique is a surprising but essential quality of persistence.
Indeed, the stability of persistence alone creates a bridge to application frontier: domains which operate on noisy measurement data like shape recognition, computer vision, and time series data have all found ways of exploiting the multi-scale information provided by persistence. 
...
In this effort, we propose a spectral relaxation of the rank invariant that is amenable to parameterized settings, such as the transform setting above. 
Although much persistence-related research has concentrated on exploiting properties of the diagram itself~\cite{}---notable exceptions include~\cite{cerri2013betti, chen2011output})---we contend  there are several useful properties of the rank invariant that justifies its use and study in continuously parameterized settings, like persistence diagrams. 
Moreover, though diagrams completely characterize their rank functions, there are certain computational advantages to working with the rank invariant directly: whereas the only competitive way of computing the latter is through the reduction algorithm~\cite{edelsbrunner2022computational}, the former is computable solely from rank computations~\cite{zomorodian2004computing} and thus can be approximated spectrally. 
...
\begin{enumerate}
	...
	This choice is typically application- and domain-dependent, requiring a priori knowledge. Randomly sampling may be sufficient in featurization or exploratory settings when $\mathcal{R}$ is unknown.
...
\end{tcolorbox}
Exemplary applications include filtration optimization, incorporating topological priors into loss functions, and....
 \\
 \\
