...
\end{equation}
Unlike the spectral decomposition $A = U \Lambda U^T$, which identifies $A$ with its spectrum $\Lambda(A)$ up to a change of basis $A \mapsto M^{-1} A M$, there is no canonical $T_j$ due to the arbitrary choice of $v$. 
...
Indeed, the three-term recurrence from~\eqref{eq:three_term_rec} implies the Lanczos iteration of $A$ may be carried out with just three $O(n)$-sized vectors. Moreover, if $A \in \mathbb{R}^{n \times n}$ is a symmetric rank-$r$ matrix with an average of $\nu$ nonzeros per row, then approximately $(2\nu + 8)n$ flops are needed for a single Lanczos step, implying a $O(n\nu r)$ time complexity for a single iteration~\cite{golub2013matrix}. Once $T_j$ has been obtained for sufficient $j > 0$, the corresponding spectrum $\Lambda(T_j) = \{\, \theta_i \,\}_{i=1}^j$ may be obtained in $O(n \log n)$ time~\cite{}. 

The sparsity pattern $\partial_p$ inherits from the simplicial structure of $K$ confers a few computational advantages compared to the traditional linear operators. In particular, the operator $\partial_p$ need not be represented explicitly in memory. To see this, observe each column of $\partial_p$ corresponds to a $p$-chain of the form~\eqref{eq:matrix_pchain}, which is a constant up to difference in sign. Thus, evaluating $y = \partial_p^T v$ reduces to computing the locations $(k_1, k_2, \dots, k_{p+1})$:
\begin{equation}
