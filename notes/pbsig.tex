...
Notice this lies in stark contrast to e.g. the vineyards algorithm~\cite{cohen2006vines}---which requires a $\approx O(m^2)$ maintenance procedure\footnote{The bound $O(m^2)$ assumes the homotopy changes each filtration value monotonically throughout the homotopy. Otherwise the number of order changes is clearly unbounded.} to simulate persistence across a homotopy, the canonical continuous parameterized setting. PBNs need no such procedure even in parameterized settings. 
...
Due to its connection as the discrete analogue of the Laplace differential operator, the graph Laplacian and its spectrum have been studied in many areas of mathematical research as they often have particular geometric interpretations in physical chemistry, and financial mathematics~\cite{}. Since the investigation of the question ``can you hear the shape of a drum?'' by~\cite{} investigates, the question of what geometric properties of a manifold are determined by its Laplace operator has been of pertinent interest. 

Given a graph $G = (V, E)$ with edge set $E \subseteq V \times V$, let $A \in \mathbb{R}^{n \times n}$ denote its corresponding \emph{adjacency matrix} with non-zero entries $A[i,j] = 1$ for every $(i, j) \in E$ (0 otherwise). The \emph{graph Laplacian} $L$ is defined as: 
\begin{equation}
	L = D - A = \partial_1 \partial_1^T
\end{equation} 
where $D$ is a diagonal matrix whose entries $d_{ii} = \{ \, \mathrm{deg}(v_i) \, \}$ are the degrees $\mathrm{deg}(v_i) = \sum_{j \neq i} A[i,j]$ of the vertices $v \in V$. 
The graph Laplacian and its many variants~\cite{} are well studied. Structurally, they admit a number are attractive features: $L$ is symmetric and thus its eigenvalues are real-valued; $L$ is positive semi-definite ($L \succeq 0$) and thus satisfies $x^T L x \geq 0$ for all $x \neq 0$; $\mathrm{rank}(L)$ is given by $n - \mathrm{cc}(G)$, where $\mathrm{cc}(G)$ is the number of connected components of $G$ (and thus, $0 \in \Lambda(L)$ with multiplicity $\omega(G)$). For a list of additional properties, see~\cite{}. 
In particular, the quadratic form that $L$ admits has the particular graph interpretation:
\begin{equation}
	 x^T L x = \sum\limits_{i \sim j} (x_i - x_j)^2, \quad x \in \mathbb{R}^n
\end{equation}
where we use the notation $i \sim j$ to indicate vertices $i,j \in V$ are path-connected in $G$. The graph Laplacian is generally known to capture the connectivity structure of $G$. To make this more concrete, we adapt an example from~\cite{newman2001laplacian}. Suppose the vertices of $G$ are ordered and labeled from $1$ to $n$ arbitrarily such that we may define column vector $x = (\, x_i\, )$ whose components $x_i = 1$ indicate $i \in X$ and $x_i = 0$ otherwise. Consider any subset $X \subseteq V$, and let $X' = V \setminus X$ denote the complement set and $c_v(X) = \{ (v,w) \in E \mid v \in X \text{ and } w \in V \setminus X \}$ denote the \emph{cutset} of $G$ restricted to $v$, i.e. the set of edges having as one endpoint $v \in X$ and another endpoint outside of $X$. 
Given $y = L x$, we have: 
\begin{align*}
	y_i > 0 &\Longleftrightarrow i \in X \text{ and } \lvert c_i(X) \rvert = y_i  \\
	y_i < 0 &\Longleftrightarrow i \in X' \text{ and } \lvert c_i(X') \rvert = \lvert y_i \rvert \\
	y_i = 0 &\Longleftrightarrow i \in X \cup X' \text{ and } c_i(X) = \emptyset
\end{align*}
In other words, $L$ captures exactly how $X$ is connected to the rest of $G$. Notice that if $X  = V$, then $y = 0$ and thus $0$ must be an eigenvalue of $L$. This example can be generalized to powers of $L$, wherein just as entries in $A^k$ model paths, entries in $L^k$ are seen to model boundaries, see~\cite{newman2001laplacian}.
...
To understand the connection to the Laplacian, we focus 
Another property of the rank function that we may exploit is -invariance: $rank(A) = \mathrm{rank}(A A^T) = \mathrm{rank}(A^T A)$.
...
\end{equation}
where $K_j(A, v) = [ v \mid Av \mid A^2 v \mid \dots \mid A^{j-1}]$ are the corresponding Krylov matrices. To generate an orthonormal basis for $\mathcal{K}_j(A, v)$, the Lanczos method proceeds by constructing a reduced QR factorizations of $K_j(A,v) = Q_j R_j$, for all $j = 1, 2, \dots, n$.
Due orthogonality of $Q_j$, when $A$ is symmetric and real, we have $q_k^T A q_l = q_l^T A^T q_k = 0$ for $k > l + 1$, implying the corresponding projections $T_j = Q_j^T A Q_j$ have a tridiagonal structure:
\begin{equation}
	T_j = \begin{bmatrix} 
	\alpha_1 & \beta_1 & & & \\
...
Instead, the Lanczos method exploits a particular connection between the iterates $K_j(A,v)$ (for any $j < n$) and the full tridiagonalization of $A$: if $Q^T A Q = T$ is tridiagonal and $Q= [\, q_1 \mid q_2 \mid \dots \mid q_n \,]$ is an $n \times n$ orthogonal matrix $Q Q^T = I_n$, then:
\begin{equation}
...
More explicitly, the Implicit Q Theorem~\cite{golub2013matrix} asserts that if an upper Hessenburg matrix $T \in \mathbb{R}^{n \times n}$ has only positive elements on its first subdiagonal and there exists an orthogonal matrix $Q$ such that $Q^T A Q = T$, then $Q$ and $T$ are \emph{uniquely} determined by $(A, q_1)$. 
Thus, given an initial pair $(A, q_1)$, we may restrict and project $A$ to its $j$-th Krylov subspace $T_j$ via: 
...
\end{equation}
where $\alpha_j = q_j^T A q_j$, $\beta_j = \lVert r_j \rVert_2$, $r_j = (A - \alpha_j I)q_j - \beta_{j\text{-}1} q_j$, and $q_{j+1} = r_j / \beta_j$. Observe that if $q_{j-1}, \beta_j$ and $q_j$ are known, then $\alpha_j$, $\beta_{j+1}$ and $q_{j+1}$ are completely determined and~\eqref{eq:three_term_rec} may be used to obtain $T_j$---this sequential process is called the \emph{Lanczos iteration}. Note that if we encounter $\beta_j = 0$ for some $j < n$ , then $\mathrm{range}(Q_j) = \mathcal{K}_j(A, q_1)$ is $A$-invariant subspace, the iteration stops, and we have $j = \mathrm{rank}(A)$ and $\Lambda(T_j) = \Lambda(A)$. % The spectrum is indeed identical when j = rank(A), see e.g. An Explicit Formula for Lanczos Polynomials
...
For any symmetric $A \in \mathbb{R}^{n \times n}$ rank-$r$ matrix with an average of $\nu$ nonzeros per row, approximately $(2\nu + 8)n$ flops are needed for a single Lanczos step, implying a $O(n\nu r)$ time complexity for a single iteration~\cite{golub2013matrix}. In general, the reduction is tridiagonal form is the more difficult part of the the two-phase computation: once $T_j$ has been obtained for sufficient $j > 0$, the corresponding spectrum $\Lambda(T_j)$ may be obtained in $O(n \log n)$ time~\cite{gu1995divide}. %\{\, \theta_i \,\}_{i=1}^j
...
\end{equation}
In particular, the operator $\partial_p$ need not be represented explicitly in memory.
...
The essential result from~\cite{} is that the eigenvalues $\Lambda(T_k) = \{\theta_1, \theta_2, \dots, \theta_k \}$ are `optimal' in the sense that $T_k = B$ is the matrix that minimizes  $\lVert A Q_k - Qk B \rVert_2$ over the space of all $k \times k$ matrices. Thus, if $\mathrm{rank}(A) = r$, the eigenvalues of $T_k$ for some $k < r$ tend to converge quickly to the largest eigenvalues of $A$. 
