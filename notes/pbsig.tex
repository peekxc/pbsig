...
Like the rank invariant, the relaxation satisfies inclusion-exclusion, is derived from simplicial boundary operators, and may be evaluated iteratively to recover persistence information. 
...
Fundamental to the family we propose is their characterization as norms of spectral L{\"o}wner operators.
...
Persistent homology~\cite{} is the most widely deployed tool in data analysis and learning applications within the topological data analysis (TDA) community. 
Though not all identical, persistence-related pipeline now follow a well-established pattern: given input data set $X$, construct a filtered simplicial complex $(K, f)$ on $X$ such that useful topological or geometric information may be profitably extracted from $f$. 
Historically, persistence came about due to its use as a \emph{shape descriptor} and is often used due to its ability to tackle the homology inference problem: given a finite data $X$ sampled from a topological space $\mathcal{X}$, can one infer the homology of $\mathcal{X}$ from $X$ with high confidence?
...
Moreover, the stability of persistence in the Hausdorff noise model allows stable homological features to be distinguished from topological noise, which is summarized visually via a \emph{persistence diagram}.
Although the theory of persistence is interesting mathematical subject in its own right, the stability of persistence really bridges the homology usefulness to the frontier of application: domains which operate on noisy measurement data like shape recognition, computer vision, and time series data have all found ways of adapting persistence's ability to describe shape to their own problems. 

Although homology is topological, the geometric information contained in persistence diagrams extracted from metric spaces likely a large part of the success of persistence in these setting. 
This can be seen by the local attempts that use localized information about $X$ to characterize things.
%In fact, persistence contains geomteric information too... distributed persistence, curvature sets. 
Geometric information can supplement or emphasize the topological information captured by persistence. 
...
This is very much so an exciting and active area of research, more recent work has extended this, yielding a foundation for shape analysis on shape space. 
 ...
 Missing from the above work is the are two important directions: how do you configure such transforms to retain the important topological/geometric information and discard irrelevant information, and (2) how we efficiently compute them? 
 In the former case, though we know the number of directions is bounded, but the bound is simply too high to be of any practical use.  
 While there are efficient algorithms for the ECC and persistence in static settings, the state of the art in parameterized settings is non-trivial.
  
 

Like persistence diagrams, the stability of the rank invariant justifies its use and study in continuously parameterized settings. Despite this, much of persistence-related research has concentrated on exploiting properties of the diagram itself~\cite{} rather than PBNs or multiplicities (notable exceptions include~\cite{cerri2013betti, chen2011output}). 
...
\noindent 
\textbf{Contributions: }
In this effort, we introduce an iterative $\epsilon$-approximation method for computing the \emph{the persistent rank invariants}---$\mu_p^{\ast}$ and $\beta_p^\ast$---in \emph{essentially} $O(m)$ memory and $O(mn)$ time, where $m, n$ are the number of $p+1, p$ simplices in the complex, respectively. 
...
In deriving the approximation, we obtain families of continuous rank invariants which are shown to satisfy a variety of attractive properties including as smoothness, perturbation stability, and differentiability on the positive semi-definite cone. 
Moreover, unlike existing dynamic persistence algorithms, our approach is quite simple, requiring no complicated data structures or maintenance procedures to implement. 
%require explicitly filtered complexes $K$ to reside in memory .
Indeed, much like other large sparse iterative methods known to numerical linear algebra, the spectral relaxation we propose is \emph{matrix-free}---as the boundary matrix $\partial$ is not modified during computation, it need not necessarily be stored in memory at all.
...
Interestingly, our results also imply the existence of an efficient output-sensitive algorithm for computing persistence diagrams (via~\cite{chen2011output}) that requires as its only input an implementation of a matrix-vector product $x \mapsto \partial x$, which consider to be of independent interest. 

\subsection*{The Relaxation}
So as to not leave the reader in suspense, we outline the proposed relaxation informally. The rest of the paper is discusses the theoretical and practical implications of these choices. At a high level, the relaxation we propose can be thought of as constructing a mapping: 
$$ (X, \mathcal{R}, \epsilon) \mapsto \mathbb{R}^{\lvert \mathcal{A} \rvert}$$
where $X$ is the input a data set, $\epsilon \in \mathbb{R}_+$ a relaxation parameter, and $\mathcal{R}$ a \emph{sieve} which sifts through a set $\mathcal{A}$, summarizing the topological and geometric behavior observed by $(K, f_\alpha)$ for all $\alpha \in \mathcal{A}$ in the process.  The steps to produce this featurization are as follows:
\begin{enumerate}
	...
	Exemplary choices of $f_\alpha$ include filtrations geometrically realized from methods that themselves are have parameters (``hyper-parameters''), such as density-filtered filtrations~\cite{} or Rips filtrations built over dynamic metric spaces~\cite{}.  
	\item Choose a \emph{sieve pattern} $\mathcal{R} \subset \Delta_+$ that is decomposable to a disjoint set of rectangles:
	$$ \mathcal{R} = R_1 \cup R_2 \cup \dots \cup R_m = \{(i,j) \in \bar{\mathbb{R}}^2  \mid i < j \}$$ 
	This choice is typically application- and domain-dependent, requiring a priori knowledge. Alternatively, randomly sampling may be sufficient in the featurization or exploratory settings when $\mathcal{R}$ is unknown.
	
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	\item Choose a homology dimension $p\geq 0$ and relaxation parameter $\epsilon > 0$ representing how closely the relaxation should model the quantity: 
	...
	\end{equation}
	Larger choices of $\epsilon$ yield smoother relaxations that depend more on the geometry of $K$, whereas smaller values of $\epsilon$ more accurately approximate the topology of $f_\alpha$. 
	\item Associate with each unique \emph{corner point} $(i,j) \in \partial(\mathcal{R})$ an $\alpha$-parameterized combinatorial Laplacian operator $\mathcal{L}_{ij}(\alpha) = \mathcal{L}_{ij}(K, f_\alpha)$:
	$$ \mathcal{L}_{ij}(K, f_\alpha): \mathbb{R}^n \to \mathbb{R}^n, \quad  \mathcal{L}_{ij}(\alpha) \in S_m^+ \; \forall \, \alpha \in \mathcal{A} $$
%	$$ L_{ij}^{\mathrm{up}}(K, f_\alpha) = \mathcal{D}_{i}^{1/2}( \alpha) \circ \partial \circ W_{j}( \alpha ) \circ \partial^T \circ \mathcal{D}_{i}^{1/2}( \alpha) $$
	Like step (2), the choice of Laplacian operator depends on the application---we discuss options in section~\eqref{sec:comb_lap}.
	\item For each corner point $(i,j) \in \mathcal{R}$, restrict and project $\mathcal{L}$ onto the Krylov subspace: 
	$$
	T = \mathrm{proj}_{\mathcal{K}} \restr{\mathcal{L}}{\mathcal{K}}, \quad \quad \mathcal{K}_n(\mathcal{L}, v) \triangleq \mathrm{span}\{ v, \mathcal{L}v, \mathcal{L}^2 v, \dots, \mathcal{L}^{n-1}v \}
	$$
	for some initially chosen vector $v \in \mathrm{span}(\textbf{1})^\perp$. As we will show in section~\ref{sec:spectral_relax}, the eigenvalues of $T$ form the basis of the $\epsilon$-approximation of~\eqref{eq:mu_alpha}. 
...
\end{enumerate}
The remaining steps of the relaxation depend on the application in mind. The applications we consider in this effort reduce effectively to one of the two generic steps: 
\begin{itemize}
	\item Compute an $\epsilon$-approximation of $\mu_p(\mathcal{R}_\alpha)$ over some family $\mathcal{A}$ (in subcubic time per $\alpha$)
	\item Compute a gradient direction $\nabla f_\alpha (\mathcal{R}) \in \mathbb{R}^{\lvert \mathcal{A}\rvert}$ with which to optimize $\mu_p(\mathcal{R}_\alpha)$ over $\mathcal{A}$
\end{itemize}
The duality between diagrams and rank functions suggests any application which exploits vectorized persistence information may benefit from our relaxation.
Such applications include, but are not limited to: characterizing swarm and flocking behavior with Betti curves~\cite{}, topology-guided image denoising~\cite{}, detecting bifurcations in dynamical systems~\cite{}, generating pseudo-metrics in shape space for metric learning~\cite{},  ...

Moreover, the fact that our relaxation is differentiable enables applications which seek to learn persistence information, such as filtration optimization, topological loss, .  
...
In the following section, we illuminate some of the related works that motivated the theory on which our relaxation is based on.  

 
\subsection*{Motivation: Duality between ranks and diagrams}\label{sec:duality_pbn_dgm}
...
Section~\ref{sec:background_notation} introduces the notation and background theory on which the rest of the paper depends, including an illuminating derivation of a relatively little-known expression of the persistent rank invariant $\beta_p(K)$ in~\ref{sec:duality_pbn_dgm} which largely enables much of the technical work presented here.
Section~\ref{sec:sec:methodology} contains the main results: a family of spectral relaxations of the rank invariants, their properties and interpretations, and their connections to other areas of mathematics. 
Section~\ref{sec:computation} is dedicated solely to the details of computing these families, including computational optimizations that highlight the contrasts between our proposed relaxation and the persistence computation.
Section demonstrates some of the prototypical use cases one may exploti 
...
For any filtration $K_\bullet$ of size $N = \lvert K \rvert$, let $R = \partial V$ denote the decomposition of the filtered boundary matrix $\partial \in \mathbb{F}^{N \times N}$ given in equation~\eqref{eq:boundary_matrix}. Then, for any pair $(i,j)$ satisfying $1 \leq i < j \leq N$, we have:
...
%~\ref{eq:measure}
Collecting these results, we summarize the duality between diagrams and their ranks with the following proposition.
...
%In theory, one need not represent these operators with matrices explicitly, a trick 
%Moreover, the rank function over 
...
It is more common in practice to define persistence of a persistent-pair $(\sigma_i, \sigma_j) \in \mathrm{dgm}(K_\bullet)$ via $f(\sigma_j) - f(\tau_i)$, rather than as $j - i$ when the filter function $f$ is derived from geometric settings.
...
Since the rank invariant obeys inclusion-exclusion~\cite{chazal2016structure} with real-valued persistence modules, as discussed in section~\ref{sec:duality_pbn_dgm}, we use the notation $(\,i, j\,)$ to denote pairs $(\,i, j\,) \in \Delta_{+}$ from the upper-half plane $\Delta_{+} = \{ \, (x,y) \in \mathbb{R}^2 : y > x \, \} $ for the remainder of the paper.
...
The lesser known rank invariant expressions from~\eqref{eq:betti_four} and~\eqref{eq:mu_four} can profitably benefit from the many of invariances of the rank function itself. One such property is permutation invariance: given any $A \in \mathbb{R}^{n \times n}$, it is easy to see that $\mathrm{rank}(A) = \mathrm{rank}(P^T A P)$ for any permutation matrix $P$.  
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), the filtration boundary matrix $\partial(K_\bullet)$ must maintain a simplexwise total order to preserve the filtered poset structure of $K_\bullet$.
However, the permutation invariance of the rank function suggests $\partial$ need not maintain this order---as long as each constitutive term in e.g.~\eqref{eq:mu_four} has the same non-zero pattern as its filtration-ordered counterpart, their ranks will be identical.

To convey the computational utility permutation invariance enables in parameterized settings, consider the following reformulation of the boundary operator $\partial_p$. 
\begin{definition}[Parameterized $\partial_p$]\label{def:time_boundary_matrix}
Let $(K, f_\alpha)$ denote a simplicial complex of size $\lvert K \rvert = N$ filtered by a parameterized filter $f_\alpha : K \times \mathcal{A} \to \mathbb{R}$. Fix an arbitrary linear extension $(K, \preceq^\ast)$ of the face poset. Define the $\mathcal{A}$-\emph{parameterized} \emph{boundary matrix} $\partial_p(\alpha)$ as the $N \times N$ matrix ordered by $\preceq^\ast$ for all $\alpha \in \mathcal{A}$ whose entries $(k,l)$ satisfy:
...
\end{definition}
\noindent
Observe that (1) the non-zero entries of the boundary operator from definition~\ref{eq:param_boundary_matrix} vary continuously in $f_\alpha$ and (2) $\partial_p(\alpha)$ can be decoupled into a product of diagonal matrices $D_\ast(f_\alpha)$ via: 
	\begin{equation}\label{eq:decouple}
		\partial_p(\alpha) = D_p(f_\alpha) \circ \partial_p \circ D_{p+1}(f_\alpha) 
	\end{equation}
	where the entries of $D_p(f_\alpha)$ depend on $f_\alpha(\tau)$ for $\tau \in K^p$.
We refer to the fixed inner matrix $\partial_p \in \{-1, 0, 1\}^{n \times m}$ as the \emph{sign pattern matrix}.
Now, observe that exploiting permutation invariance and the decoupling from~\ref{eq:decouple} we have:
...
$$
Note in~\eqref{eq:rank_equiv_param} we write $K_\bullet$ and $K$ to make the distinction that the boundary operator of the former is explicitly filtered in the traditional simplexwise manner, whereas the latter is ordered arbitrarily. 
Since these operators have the same rank, we may replace the constitutive boundary terms in~\eqref{eq:mu_four} and~\eqref{eq:betti_four} appropriately, thus obtaining expressions for the rank invariants using parameterized boundary operators of the form given in definition~\ref{def:time_boundary_matrix}.
...
\begin{proposition}\label{prop:mu_betti_1}
	Let $\delta > 0$ denote the number from~\eqref{eq:pbn_cont} satisfying $i + \delta < j - \delta$. Given $(K, f_\alpha)$ and any rectangle $R = [i,j] \times [k,l] \subset \Delta_{+}$ satisfying $i < j \leq k < l$, the $\mathcal{A}$-parameterized invariants $\beta_p^{i,j} : \mathcal{A} \times K \to \mathbb{N}$ and $\mu_p^{R} : \mathcal{A} \times K \to \mathbb{N}$ functions given by: 
...
\subsection{Spectral rank relaxation} 
...
Like betti numbers, the unstable nature of the rank function is due to its inherently combinatorial properties. In particular, the discontinuity in~\eqref{eq:rank_def} manifests due to the one-sided sign function. 
...
We follow the framework presented by Bi et al.~\cite{bi2013approximation} for constructing a smooth rank function approximations via relaxations of the $\mathrm{sgn}_+$ function. 
Building off the seminal work of Chen \& Mangasarian~\cite{mangasarian1994class}, 
their approach is to approximate the $\mathrm{sgn}_+$ function in~\eqref{eq:rank_def} by integrating smoothed variations $\hat{\delta}$ of the Dirac delta function $\delta$:  
...
Many of the properties of~\eqref{eq:phi} extend naturally to the rank function when the sign function is substituted appropriately in~\eqref{eq:rank_def}.
In particular, if $X \in \mathbb{R}^{n \times m}$ admits a singular value decomposition $X = U\Sigma V^T$ and is paired with a scalar-valued $\phi$ continuously differentiable at every $\sigma \in \Sigma$, then for any $\epsilon > 0$ the corresponding \emph{Löwner operator} $\Phi_\epsilon$ given by: 
...
Before summarizing the additional properties this operator has and its relationship with the rank function, we give a definition:
\begin{definition}[Spectral Rank Approximation]
	Given $X \in \mathbb{R}^{n \times m}$ with singular value decomposition $X = U \Sigma V^T$ where $\Sigma = \mathrm{diag}(\sigma_1, \sigma_2, \dots, \sigma_n)$ and a fixed $\epsilon > 0$, any choice of $\phi: \mathbb{R}_+ \times \mathbb{R}_{++}$ satisfying~\eqref{eq:phi} defines a \emph{spectral rank approximation} $\lVert \Phi_\epsilon(X) \rVert_\ast$ of $X$ via: 
...
\\
\\
\noindent To adapt these relaxations to the counting functions defined in equations~\eqref{eq:betti_mult} and~\eqref{eq:mu_four}, we need to modify the expression of the boundary chains~\eqref{eq:alt_sum} to vary continuously in $\mathcal{A}$. 
Fortunately, this requires a simple augmentation to the step function from the one in definition~\ref{def:time_boundary_matrix}. In particular, we swap out the step functions $S : \mathbb{R} \to \{ 0, 1\}$ from~\eqref{eq:param_boundary_matrix} with \emph{smoothstep} functions $\mathcal{S}: \mathbb{R} \to [0, 1]$. 
\begin{figure}
\centering
%	\includegraphics[width=0.45\textwidth]{mult_both}
...
\end{figure}
These are clamped ``S-curve'' functions which interpolate the discontinuous step portion of $S$ along a fixed interval $(a,a+\omega)$, which are given by:  
...
\end{equation}
where $\Phi_\epsilon^\alpha(\partial) = (\Phi_\epsilon \circ \partial)(\alpha)$ for any $\alpha \in \mathcal{A}$, $\hat{I}_{p}^i = \mathrm{diag}(\{ \mathcal{S}_i^\omega(\tau_1), \mathcal{S}_i^\omega(\tau_2), \dots, \mathcal{S}_i^\omega(\tau_n) \})$, and $\hat{\partial}_p$ is $\mathcal{A}$-parameterized $p$-th boundary matrix from definition~\ref{def:time_boundary_matrix} with step functions $S_\ast$ replaced by the smoothstep $\mathcal{S}_\ast^\omega$ from~\eqref{eq:smoothstep}.
\end{definition}
...
In what follows, we investigate the smoothness of $f_\alpha$ and towards stabilizing the constitutive terms in $\hat{\mu}_p^\ast$ and $\hat{\beta}_p^{\ast}$. 
...
\noindent
\textbf{Regularization:} Another approach to stabilizing the rank function often employed in the context of large-scale sparse inverse problems is to regularize the solution. 
...
\end{equation}
To illustrate the relevance of TR with our proposed approximation, consider the parameterization\footnote{Similar regularization interpretations may be derived for other choices of $\nu(\epsilon)$ and $p(x)$.} of $\phi$ obtained by setting $\nu(\epsilon) = \sqrt{\epsilon}$ and $p(x) = 2x (x^2 + 1)^{-2}$. By~\eqref{eq:phi}, one obtains:
\begin{equation}
...
\end{equation}
The connection between the TR right hand side of~\eqref{eq:tikhonov_1} is now clear: obtaining $ \lVert \Phi_\epsilon(L) \rVert_\ast$ is equivalent to finding the eigenvalues of solving $n$ TR least-squares problems of the form~\eqref{eq:tikhonov} where $X$ is substituted with the appropriate boundary/coboundary operator $\partial_p$/$\partial_p^T$ and $b$ is substituted with an $\alpha$-parameterized $p$-(co)chain.
...
On the positive side, expressions like~\eqref{eq:betti_four} and~\eqref{eq:mu_four} may benefit from the identities $\mathrm{rank}(A) = \mathrm{rank}(A A^T) = \mathrm{rank}(A^T A)$ and the fact that the square roots of the non-zero eigenvalues of either $AA^T$ or $A^T A$ yield the singular values of $A$. 
Indeed, observe expressing $\mu_0$  and $\beta_0$ solely in terms of boundary operators allows use to substitute the constitutive terms of with $L = \partial_1 \partial_1^T$, the well studied \emph{graph Laplacian}. 
More generally, we may consider the study of spectra of \emph{combinatorial Laplacians}---generalizations of the graph Laplacian---as the study of singular values of boundary operators. Since these are central quantities studied thus far, we elaborate on this connection in what follows.  
%Towards revealing  
...
\end{equation}
Notice that the right hand side of\eqref{eq:l_up} is of the form $W^{-1} P W$ where $P$ is symmetric positive semi-definite and $W$ is a positive diagonal matrix. Thus, $L_p^{\text{up}}$ is matrix-similar to $W_p^{-1/2}  \partial_{p+1} W_{p+1} \partial_{p+1}^T W_p^{-1/2}$, which is symmetric and positive semi-definite. 
...
For now we summarize this fact with a proposition. 
\begin{proposition}[\cite{}]
	Let $f : (\mathbb{R}^n, H_n) \to (\mathbb{R}^m, H_m)$ be a linear map between inner product matrices $H_n \in \mathbb{R}^{n \times n}$ and $H_m \in \mathbb{R}^{m \times m}$, and let $F \in \mathbb{R}^{m \times n}$ denote the matrix representation of $f$. Then, for any $x \in \mathbb{R}^n$ and $y \in \mathbb{R}^m$, we have the following equivalence of inner products: 
	$$ \langle f x, y \rangle_{\mathbb{R}^m}  = \langle x, f^\ast y \rangle_{\mathbb{R}^n} = x^T F^T H_m y = x^T H_n F^\ast y $$
	where $F^\ast = H_n^+ F^T H_m$ is a representative of the adjoint $f^\ast : (\mathbb{R}^m, H_m) \to (\mathbb{R}^n, H_n)$ of $f$. 
\end{proposition}
...
Each eigen-pair $(\lambda, v)$ satisfies $A v = \lambda v$, and when $A$ is symmetric, every $\lambda$ is real-valued and every pair of eigenvectors $v, v' \in \mathbb{R}^n$ with non-identical eigenvalues $\lambda \neq \lambda'$ is orthogonal.  
% https://netlib.org/lanczos/vol2/Chp_2_RealSymmetric.pdf
Thus, we may reveal the spectrum $\Lambda(A)$ of $A$---the lengths of the aforementioned ellipsoid---via orthogonal diagonalization:
...
Moreover, the three-term recurrence from~\eqref{eq:three_term_rec} implies the Lanczos iteration requires just three $O(n)$-sized vectors and a few $O(n)$ vector operations. We summarize these benefits with a proposition. 
...
Several decades of research have been dedicated to developing orthogonality-enforcement schemes that retain the simplicity of the Lanczos iteration without increasing either the time or space complexities by non-trivial factors. 
As these extensions are complex, multifaceted, and beyond the scope of the present work, we defer their discussion to the appendix~\ref{subsec:rounding} and refer the curious reader to~\cite{golub2013matrix} and references therein for an overview. 
...
As proposition~\ref{lemma:exact_arith_lanczos} shows, the efficiency of the Lanczos method depends crucially on the existence of the three-term recurrence and a fast matrix-vector product. 
The former arises in the decomposition of symmetric matrices while the latter depends on the structure of the $x \mapsto A x$ operation. 
In general, a symmetric rank-$r$ matrix $A \in \mathbb{R}^{n \times n}$ with an average of $\nu$ nonzeros per row incurs approximately $(2\nu + 8)n$ floating point operations (FLOPS) per Lanczos step, implying a $O(n\nu r)$ time complexity for a single iteration~\cite{golub2013matrix}. 
This is markedly improved in the case of the graph Laplacian derived from a graph $G = (V, E)$: the $x \mapsto \partial_p \partial_p^T x$ operation can be carried out in a time linear in $\lvert E \rvert$ by summing across edges---no sparse matrix need be explicitly constructed.
 In contrast, it is not clear one may readily generalize this process to the combinatorial Laplacian for more general $p > 1$ as the form of the $\mathtt{matvec}$ depends on the specific Laplace operator one considers.
Towards accelerating the computation, we examine these nuanced differences more in detail.
...
To begin understanding the computational nuances associated with simplicial Laplacian operators, first consider the graph Laplacian $L = D - A$ derived from an undirected simple graph $G = (V, E)$. As in~\cite{chung1997spectral}, the linear and quadratic forms of $L$ may be succinctly expressed using the path-connected relation $i \sim j$, shown below:
...
\end{flalign}
Observe that, if $G = (V, E)$ has $m$ edges and $n$ vertices taking labels in the set $[n]$, the matrix-vector product in~\eqref{eq:lap_quad_from} can be evaluated in $O(m)$ time and $O(n)$ storage via two $O(m)$ edge traversals: one to accumulate the degree of each vertex, and one to subtract off components $x_j$ from incident edges at $i$. By precomputing the degree of each vertex and storing an auxiliary $O(n)$ amount of memory, this computation can be reduced to a one $O(n)$ vector multiplication and a single $O(m)$ pass through the edges, which is often beneficial for repeated computation. 
Note that the exact form of equation~\eqref{eq:lap_quad_from} does not immediately generalize to combinatorial Laplacians for $p > 0$, as the orientation of higher dimensional simplices changes the sign of the off-diagonal entries in $L_p^\ast$ and it is not immediately clear how to generalize the notion of path-connected to higher dimensions. 
...
Noting the observations in section~\ref{sec:laplacian_theory}, we exclusively consider the symmetric version of $\hat{L}_p^{\mathrm{up}}$. The down-Laplacian $\hat{L}_p^{\mathrm{dn}}$ can be handled in essentially the same way. 
Towards extending~\eqref{eq:lap_quad_from} to higher dimensions, we need to generalize the path-connected relation $i \sim j$. Let $\mathrm{co}(\tau) = \{ \, \sigma \in K^{p+1} \mid \tau \subset \sigma \, \}$ denote the set of proper cofaces of $\tau \in K^p$, or \emph{cofacets}, and let 
$$\mathrm{deg}_f(\tau) = \sum_{\sigma \in \mathrm{co}(\tau)} f(\sigma) $$
denote the (weighted) \emph{degree} of $\tau$. Observe setting $f(\sigma) = 1$ for all $\sigma \in K$ recovers the integral notion of degree as representing the number of cofacets a given simplex has, which in the graphical setting recovers the usual notion. 
Now, observe that since $K$ is a simplicial complex, if $\mathrm{co}(\tau) \cap \mathrm{co}(\tau') \neq \emptyset$ then the faces $\tau, \tau'$ not only share a common cofacet $\sigma$, but this cofacet $\{\sigma\} = \mathrm{co}(\tau) \cap \mathrm{co}(\tau')$ is \emph{unique}~\cite{goldberg2002combinatorial}. 
Thus, given two distinct $p$-simplices $\tau \neq \tau'$ in $K^p$ we may use the notation $\tau \overset{\sigma}{\sim} \tau'$ to denote this cofacet relation and now the operator from~\eqref{eq:l_up} can now be written element-wise as: 
...
\begin{align}\label{eq:up_laplace_theory}
	 \hat{L}_p^{\text{up}}(\tau, \tau')= \begin{cases}
		 \mathrm{deg}_f(\tau) \cdot \mathrm{deg}_f^{+}(\tau) & \text{ if } \tau = \tau' \\ 
%		\mathrm{deg}(\tau_i) & \text{ if } i = j \\ 
		s_{\tau, \tau'} \cdot  \mathrm{deg}_f(\tau)^{+/2} \cdot f(\sigma) \cdot \mathrm{deg}_f(\tau')^{+/2} & \text{ if } \tau \overset{\sigma}{\sim} \tau' \\
		0 & \text{ otherwise} 
	\end{cases}
\end{align}
...
\end{equation}
Observe~\eqref{eq:l_up_matvec} can be evaluated now in the same $O(m(p+1))$ time and $O(n)$ storage, where $m = \lvert K^{p+1} \rvert$ and $n = \lvert K^p \rvert$ via the two-pass algorithm described for the graph Laplacian if both the sign term $s_{\tau, \tau'}$ and the indexing function $h$ can be evaluated in $O(1)$ time. 
...
In general, the signs of the coefficients $\mathrm{sgn}([\tau], \partial[\sigma])$ and $\mathrm{sgn}([\tau'], \partial[\sigma])$ depends on the position of $\tau, \tau'$ as summands in $\partial[\sigma]$ via~\eqref{eq:alt_sum}, which itself depends on the orientation of $[\sigma]$ defined by~\eqref{eq:oriented_simplex}. Thus, naive evaluation of these sign terms takes $O(k)$ time to determine for a given $\tau \in \partial[\sigma]$ with $\mathrm{dim}(\sigma) = k$. 
...
Thus, if we fixed the orientation of the simplices of $K^p$, the sign pattern $s_{\tau, \tau'}$ for every $\tau \overset{\sigma}{\sim} \tau'$ can be precomputed and stored ahead of time, reducing the evaluation $s_{\tau, \tau'}$ in $O(1)$ time and $O(n)$ storage. 
Alternatively, if the labels of the $p+1$ simplices $\sigma K^{p+1}$ are consistently enumerated in a fixed order---for example according to the poset order on $K$ inherited from the total order on $V$---then we can remove the storage requirement entirely and simply alternate the sign during the computation as appropriate.

A subtle but important aspect of algorithmically evaluating~\eqref{eq:l_up_matvec} is the choice of indexing function $h: K^p \to [n]$. This map is necessary to deduce the contributions of the components $x_\ast$ during the operation. While one may use any standard associative array to generate this map, typical implementations must rely on collision-resolution schemes such as open addressing or chaining to obtain a constant expected lookup time.
%Preliminary benchmarks suggest 
Taking advantage again of the fact that our simplicial complex $K$ is fixed, we note that one can build a hash function $h$ with deterministic $O(1)$ access by building a \emph{minimal perfect hash function} (PMHF) on the simplices $\tau \in K^p$.
It is known how to build PMHF's over fixed input sets of size $n$ in $O(n)$ time and $O(n \log m)$ bits~\cite{}. Note that this process happens only once for a fixed simplicial complex $K$: once $h$ has been built, it is fixed for every $\mathtt{matvec}$ operation. 
...
Since a $p+1$-simplex has $p$ proper faces in its boundary, and we need to iterate through the $p+1$ simplices $\sigma \in K^{p+1}$ just once in the case of the $\mathrm{up}$-Laplacian, we may combine the above observations into a proposition. 
\begin{proposition}
	For any $p \geq 0$ and simplicial pair $(K, f)$, there exists a two-phase algorithm for computing the product: 
	\begin{equation}
		L_p^{\mathrm{up}} x = (W_p^{+/2} \circ \partial_{p+1} \circ W_{p+1} \circ \partial_{p+1}^T \circ W_p^{+/2})x
	\end{equation}
...
\subsection{Matrix-free persistence computation}
% The first and most general application of the work presented here is the matrix-free computation of persistence invariants---Betti numbers, multiplicities, and diagrams. 
...
\textbf{Storage requirements}: On the left side of figure~\ref{fig:watts_strogatz} next to the examle network model, we record the ratio of $\mathtt{matvec}$ operations (relative to $n$) needed to compute $p\%$ of the spectrum as a function of the maximum number basis vectors kept in-memory for reorthogonalization purposes. 
...
\Ensure Weight functions $w_{p+1}: K^{p+1} \to \mathbb{R}_{+}$ and  $w_{p}^l, w_{p}^l: K^p \to \mathbb{R}_{+}$
...
%    $\mathrm{sgn}(\partial[\sigma])$
    ...
    \EndFor 
\EndFor
\Return $y$ \Comment $(y = \langle \, L_p^{\mathrm{up}}, x \, \rangle)$
\EndFunction
\end{algorithmic}
\end{algorithm}
