...
Moreover, we find the combinatorial Laplacians operators these relaxations derive from encode rich geometric information in their non-harmonic spectra, providing several avenues for geometric data analysis. 
...
\begin{equation}\label{eq:relaxation_mapping}
	(X_\alpha, \mathcal{R}, \epsilon, \lambda) \mapsto \mathbb{R}^{O(\lvert \mathcal{R} \rvert)}
\end{equation}
where $X_\alpha$ is an $\mathcal{A}$-parameterized input data set, $\mathcal{R} \subset \Delta_+$ a \emph{sieve} over the upper half-plane $\Delta_+$, and $(\epsilon, \lambda) \in \mathbb{R}_+^2$ are approximation/smoothness parameters, respectively.
%The intuition is that that are many situations where one has a data set 
The intuition is that $\mathcal{R}$ is used to extract, filter, and summarize the topological and geometric behavior exhibited by $X_\alpha$ for all $\alpha \in \mathcal{A}$, thereby \emph{sifting} the space $\mathcal{A} \times \Delta_+$. 
...
\begin{enumerate}
	...
	\end{equation}
	Exemplary choices of $f_\alpha$ include filtrations geometrically realized from methods that themselves are have parameters, such as density filtrations~\cite{} or time-varying filtrations over dynamic metric spaces~\cite{}. 
...
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	\item Fix a homology dimension $p\geq 0$ and parameters $(\epsilon, \lambda) \in \mathbb{R}_+^2$ representing how \emph{closely} and \emph{smoothly} the relaxation should model the quantity: 
	...
	\end{equation}
	We will show in section~\ref{sec:spri_properties}, letting both $\lambda \to 0$ and and $\epsilon \to 0$ yields the multiplicity function $\mu_p$ exactly.
...
For a variety of reasons, the classical least-squares problem objective to the solving the [possibly ill-posed] linear system $A x = b$ is often substituted with an explicit \emph{Tikhonov regularization} (TR) for some $\epsilon > 0$:
 \begin{equation}\label{eq:tikhonov}
	x_\epsilon^\ast = \argmin\limits_{x \in \mathbb{R}^n} \lVert Ax - b\rVert^2 + \epsilon \lVert x \rVert^2
\end{equation}
When $\epsilon = 0$, one recovers the $\ell_2$ minimization, whereas when $\epsilon > 0$, solutions $x^\ast$ with small norm are favored. 
