...
\end{equation}
A notable feature of our approach is that it not only avoids explicitly constructing diagrams, but it in fact avoids omits the reduction algorithm from~\cite{edelsbrunner2022computational} entirely.  
...
In fact, the proposed relaxation is also \emph{matrix-free}, requiring only as much memory as is needed to enumerate simplices in the underlying complex $K$. Moreover, the spectral quality of the and is particularly efficient when executed on parameterized families of inputs. 
\\
\\
\noindent 
...
Though this observation is typically viewed as a minor fact, it implications are quite general, as recently noted by~\cite{bauer2022keeping}:
...
%	\mathrm{rank}(\, \partial_p^{i,j}(K_\bullet) \, ) = 
	\mathrm{rank}(\, \partial_p^{i,j}(\alpha) \, ) \triangleq \mathrm{rank}\!\left( \, D_p(\bar{S}_i \circ f_\alpha) \circ \partial_p(K) \circ D_{p+1}(S_j \circ f_\alpha) \, \right)
\end{equation}
...
\end{equation}
In~\eqref{eq:rank_equiv_param}, we write $\partial_p(K)$ (as opposed to $\partial_p(K_\bullet)$) to emphasize $\partial_p$ is ordered arbitrarily according to the ordering $(K, \preceq^\ast)$. Note that, in contrast, the boundary terms in proposition~\ref{prop:mu_reduction} are explicitly filtered in the traditional simplexwise manner by $f_\ast$. 
As a result, expressions of persistent rank invariants obtained by replacing the constitutive terms in~\eqref{eq:betti_four} and~\eqref{eq:mu_four} using their parameterized versions from definition~\ref{def:time_boundary_matrix} are permutation invariant. 
...
These functions interpolate the discontinuous step portion of $S$ along a fixed interval $(a,a+\omega)$ (see Figure~\ref{fig:smoothstep}). 
%\begin{equation}
...
\end{definition}
\noindent Since the sum of two Lipshitz functions $g$ and $h$ is also Lipshitz, it is easy to verify both $\hat{\mu}_p^R$ and $\hat{\beta}_p^{i,j}$ are Lipshitz continuous functions in $\alpha$ whenever the filter function $f$ is Lipshitz by combining the smoothness of $\mathcal{S}_\ast^\omega$ with the global Lipshitz continuity of~\eqref{def:cont_rank}. 
