...
expression of the persistent Betti number, beginning with its definition. 
...
%\end{equation}
The structure theorem from~\cite{zomorodian2004computing} shows that 1-parameter persistence modules can be decomposed in an \emph{essentially unique} way into indecomposables. One consequence of this phenomenon is the Pairing Uniqueness Lemma~\cite{edelsbrunner2000topological}, which asserts that if $R = \partial V$ decomposes the boundary matrix $\partial$ to a \emph{reduced} matrix $R$, then:  
\begin{equation}\label{eq:uniq_pivot}
r_R(i,j) \neq 0 \Leftrightarrow R[i,j] \neq 0
\end{equation}
In other words, due to equation~\eqref{eq:uniq_pivot}, the existence of any non-zero entry in $R$ may be inferred from the ranks of its submatrices.   
This is important, as although $R$ is not unique, its lowest non-zero ``pivot'' entries are---and these entries define the persistence diagram. 
We record a fact that exploits this idea and, as we will show, has non-trivial computational implications.
...
An explicit proof of this can be found in~\cite{dey2022computational}, though it was noted in passing by Edelsbrunner~\cite{edelsbrunner2000topological}. It essentially follows by combining the Pairing Uniqueness Lemma with the fact that the reduced matrix $R$ is obtained from $\partial$ via left-to-right column operations, which preserve the ranks of every such submatrix.
...
The first proposition we prove is that Lemma~\ref{lemma:rank} may be used to write the persistent Betti number in a simpler way as a sum of rank functions. 
...
\end{proposition}
\noindent A detailed proof of Proposition~\ref{prop:rank_reduction} is given in the appendix. It turns out Lemma~\ref{lemma:rank} can also be further generalized to arbitrary rectangles $R = [i,j] \times [k,l]$ in $\Delta_+^N$, see the fourth picture in Figure~\ref{fig:mult}.
Geometrically, these rank computations manifest as $\mu$\emph{-queries}: box-parameterized multiplicity queries which count the number of persistence pairs that intersect a fixed ``box'' placed in the index persistence plane~\cite{chen2011output}. 
As a result, the $p$-th multiplicity function can also be defined in a rank-based formulation akin to~\eqref{eq:betti_four}, which we record here as well. 
...
\noindent In conclusion, compared to other classical methods of obtaining $\beta_p^{\ast}(K_\bullet)$ and $\mu_p^\ast(K_\bullet)$, such as those in~\cite{edelsbrunner2022computational, zomorodian2004computing}, the primary advantage the rank-based expressions from~\eqref{eq:betti_four}-\eqref{eq:mu_four} have is that their computations are performed directly on \emph{unfactored} boundary matrices. 
Note the only distinction between the interpretation of~\eqref{eq:mu_four} and that of~\eqref{eq:measure} is that the former is indexed on the index-persistence plane $\Delta_+^N$ whereas the latter is indexed over $\Delta_+$. 
If we work only with persistence modules with real-valued coefficients, not only do we inherit the measure-theoretic perspective from~\eqref{eq:measure}, but we may also replace the non-zero entries of $\partial_p$ with real-valued coefficients. 
...
\end{definition}
\noindent 
 We record a few basic properties of $\hat{\partial}_p^{i,j}$. To simplify the notation, we write $A^{x} = A^{\ast,x}$ to denote the submatrix including all rows of $A$ and all columns of $A$ up to $x$, and let $q = p + 1$. 
...
In particular, if $X \in \mathbb{R}^{n \times m}$ admits a singular value decomposition $X = U\Sigma V^T$ and is paired with a scalar-valued $\phi$ continuously differentiable at every $\sigma \in \Sigma$, then for any $\epsilon > 0$ the corresponding \emph{Löwner} operator $\Phi_\epsilon : \mathbb{R}^{n \times m} \to \mathbb{R}^{n \times m}$: 
...
In fact, if $\phi$ is twice-differentiable at each $\sigma_i(X)$ for all $i = 1, \dots, n$, then operator~\eqref{eq:lowner} is also twice continuously differentiable at $X$~\cite{bi2013approximation}. 
...
\begin{proposition}[Bi et al.~\cite{bi2013approximation}] The operator $\Phi_\epsilon: \mathbb{R}^{n \times m} \to \mathbb{R}^{n \times m}$ defined by~\eqref{eq:lowner} satisfies: 
	\begin{enumerate}
		...
		\item The function $\lVert \Phi_\epsilon(X) \rVert_\ast$ is globally Lipshitz continuous and semismooth\footnote{The notion of semismoothness here refers to the existence certain directional derivatives in the limit as $\epsilon \to 0^+$, see~\cite{} for more details.} on $\mathbb{R}^{n \times m}$.
...
Let $K$ denote a fixed simplicial complex and $f: K \times \mathcal{H} \to \mathbb{R}$ a parameterized family of functions which vary continuously in $\mathcal{H}$. For some fixed $R = [i,j] \times [k,l] \subset \Delta_+$ and some choice of parameters $(\epsilon, p, \omega)$ satisfying $\epsilon > 0$, $p \geq 0$, and $\omega > 0$, define the $\mathcal{H}$-parameterized \emph{smooth multiplicity function} as:
...
\end{equation}
where $\Phi_\epsilon^h(\partial) = (\Phi_\epsilon \circ \partial)(h)$ for any $h \in \mathcal{H}$, and $\hat{\partial}_p$ is $\mathcal{H}$-parameterized $p$-th boundary matrix from definition~\ref{def:time_boundary_matrix} with step functions $S_\ast$ replaced by the smoothstep $\mathcal{S}_\ast^\omega$ from~\eqref{eq:smoothstep}.
\end{definition}
...
\subsection{Laplacian connections}\label{sec:laplacian_theory}
...
\end{equation}
Horak and Jost given a unified perspective of the spectra of combinatorial Laplacians derived from simplicial complexes~\cite{}, and how they are related. 
Namely, one can verify that a consequence of~\eqref{eq:laplace_hom} is that the non-zero parts of the spectra satisfy:
\begin{equation}
	\Lambda(\Delta_p(K)) = \Lambda(L_p^{\text{up}}(K)) \cup \Lambda(L_p^{\text{dn}}(K))
\end{equation}
Moreover, since $\Lambda(AB) = \Lambda(BA)$ for operators $A,B$ on suitably chosen Hilbert spaces, the non-zero eigensets of $\Lambda(L_p^{\text{up}}(K))$ and $\Lambda(L_{p+1}^{\text{dn}}(K))$ are identical---thus, it suffices to consider only one of them. 
Many subsequent papers have been published studying the properties of higher order combinatorial Laplacians have subsequently found applications to graph optimization problems, network circuit theory, and sparsification problems; see~\cite{} and references therein for an overview. 
...
\end{equation} 
Note that none of the constitutive terms are symmetric matrices, in general. 
...
\end{align*}
where $A^{+/2} = (A^{1/2})^+$. Thus, in this way we think of our definition of a parameterized boundary matrix as corresponding to a choice of a \emph{non-negative scalar-product} on the vector space $C^p(K, \mathbb{R})$, which induces a (degenerate) inner product $\langle \cdot, \cdot \rangle : C^p(K, \mathbb{R}) \times C^p(K, \mathbb{R}) \to \mathbb{R}$ on $C^p(K, \mathbb{R})$. Equivalently, the choice of non-negative weight function induces an inner product on the quotient space $W = C^p(K, \mathbb{R}) / \{ b : \lvert b \rvert = 0 \}$.
...
\end{equation}
has its spectrum bounded in the interval $[0, p+2]$, where $\tilde{D}_p^i = \mathrm{diag}(\{ \mathrm{deg}_w(\sigma_1), \mathrm{deg}_w(\sigma_2), \dots, \mathrm{deg}_w(\sigma_n) \})$ corresponds to the weighted degree matrix. We defer discussion of the exact forms of these matrices, including the computational details the matrix-vector product, to section~\eqref{sec:comb_lap}. For now we conclude this section with a proposition. 
...
As~\eqref{eq:rank_def} is defined completely in terms of the singular values of $X$, which as we've mentioned are given by the square roots of eigenvalues of the corresponding Laplace operator, we focus on iterative methods specialized for symmetric positive semi-definite matrices arising as Laplacians. 
...
\subsubsection{The efficiency of the Laplacian \texttt{matvec}}\label{sec:comb_lap}
...
The former only arises in the decomposition of symmetric matrices and the latter depends on the structure of the $x \mapsto A x$ operation. 
Though the Laplacian operator we consider here are all symmetric---enabling the use of the three-term recurrence---and $x \mapsto Lx = (\partial_1 \partial_1^T)x$ can be carried out in linear time in the number of edges of the underlying graph, it is not immediately clear whether $x \mapsto \partial_p \partial_p^T x$ can be carried out in a time linear in the number of $p+1$ simplices for more general $p > 1$, or whether the computation is efficient in practice. Moreover, as mentioned in~\ref{}, the exact form of the matrix-vector product depends on the type of Laplacian operator. 
...
% Flesh out combinatorial number system
...
In general, the spectra of a given Laplacian operator encodes certain aspects of the underlying spaces combinatorial structure. 
For example, the Laplacians of a simplicial complex encode its basic topology via its homology groups, which is characterized by the nullspace of the corresponding operator.
...
In contrast, these operators differ in the nonzero part of the spectrum, which when equipped with a scalar-product encode specific geometric features in addition to topological properties. For example, weighting edges of the graph Laplacian originally studied by Kirchoff typically has natural interpretations as electrical \emph{conductance}/\emph{resistance} that aris electrical circuit networks, due to Ohms law and conservation of flow. 
By normalizing 
