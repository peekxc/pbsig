...
% Change index set
\textbf{Remark:} All of the notation given thus far have used integer indices $(i,j) \in \Delta_+^N$ to describe persistent quantities over a filtration $(K_\bullet, f)$ of size $\lvert K \rvert = N$. Equivalently, we have thus far implicitly assumed the range of the filter function $f : K \to I$ to be the typical index set $I = [N]$.
...
\end{equation}
where $\widetilde{W}_p^x$ is defined synonymous with $\tilde{g}_h(\sigma) = (\tilde{S}_x \circ f_h)(\sigma)$. Thus, definition~\ref{def:time_boundary_matrix} corresponds simply a row-and-column weighted version of the \emph{fixed} $p$-th boundary matrix $\partial_p$.
\\
\\
\noindent 
...
\begin{proposition}
	Let $\delta > 0$ denote the number from~\eqref{eq:pbn_cont} satisfying $i + \delta < j - \delta$. 
	For any pair $i,j \in \Delta_+$ and any rectangle $R = [i,j] \times [k,l] \subset \Delta_{+}$ satisfying $i < j \leq k < l$, the $\mathcal{H}$-parameterized PBN $\beta_p^{i,j} : \mathcal{H} \times \mathcal{P}(V) \to \mathbb{N}$ and multiplicity $\mu_p^{R} : \mathcal{H} \times \mathcal{P}(V) \to \mathbb{N}$ functions given by: 
	...
	$\mathrm{card} \left( \, \restr{\mathrm{dgm}_p(M)}{R} \, \right)$ and 
