...
\end{equation}
where $X_\alpha$ is an $\mathcal{A}$-parameterized input data set, $\epsilon$ a relaxation parameter, and $\mathcal{R} \subset \Delta_+$ a \emph{sieve} over the upper half-plane $\Delta_+$ which sifts through $\mathcal{A} \times \Delta_+$, extracting and summarizing the topological and geometric behavior $X_\alpha$ exhibits. 
...
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	...
	\begin{equation}\label{eq:mu_alpha}
		\mu_p(\mathcal{R} \times \mathcal{A}) \stackrel{\epsilon}{\approx} \{ \, \mathrm{card}(\mathcal{R} \, \cap \, \mathrm{dgm}(K, f_\alpha)) \mid \alpha \in \mathcal{A} \, \}
	\end{equation}

	\item Choose a Laplacian operator $\mathcal{L}$ to associate to $\mathcal{R}$: 
...
The choice of $\mathcal{L}$ encodes specific geometric and topological information about $(K, f_\alpha)$.%and $\Phi$ determines how this information is incorporated into the mapping. 
	...
	$$
	The eigenvalues of $T = \mathrm{proj}_{\mathcal{K}} \restr{\mathcal{L}}{\mathcal{K}}$ form the basis of the $\epsilon$-approximation\footnote{Depending on the amount of memory available,  step (5) may be accelerated along similar $\alpha \in \mathcal{A}$ by storing the largest $k \geq 0$ eigenvectors of $T$ at each corner point $(i,j) \in \partial(\mathcal{R})$ (see section~\ref{sec:lanczos_it} for details).} of~\eqref{eq:mu_alpha} (see section~\ref{sec:spectral_relax}).
\end{enumerate}
\end{boxedenumerate}
...
\subsection{Properties \& Interpretations}
