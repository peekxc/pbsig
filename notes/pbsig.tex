...
In particular, writing down the quadratic form of $L_p^{\text{up}}$ requires an explicit expression for the sign of each oriented $[\tau] \in K^{p}$ in the boundary of each of its proper 1-d cofaces $[\sigma] \in K^{p+1}$. 
To make this clear, we require more notation. 

Given a simplicial complex $K$, we denote with $\tau \overset{\sigma}{\sim} \tau'$ the relation where $\tau \neq \tau'$ are two distinct $p$-simplices in $K$ that share a proper coface $\sigma \in K^{p+1}$.
...
Notice that signs of the coefficients of $(\tau, \tau')$ in $\partial[\sigma]$ does not depend on the orientation of $\sigma$, only on the orientations $[\tau]$ and $[\tau']$. 
...
\begin{align}
	L_p^{\text{up}}[i,j] = \begin{cases}
		...
		(-1)^{s_{ij}} & \text{ if } \tau_i \overset{\sigma}{\sim} \tau_j \\
		0 & \text{ otherwise} 
	\end{cases}
\end{align}
