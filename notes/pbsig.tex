...
Moreover, since we assume the poset order on $K$ is induced from the vertex order, the sign pattern each $[\sigma]$ takes induced by $(V, \preceq)$ is constant, and since a $p$-simplex has $p+1$ proper faces in its boundary we deduce that if $n = \lvert K^p \rvert$ and $m = \lvert K^{p+1}\rvert$ then we may compute the matrix-vector product $x \mapsto L_p^{\text{up}} \, x$ in $O(m(p+1))$ time and $O(n)$ memory.  
From a practical perspective, $p \geq 0$ is generally a very small constant---typically no more than $2$---thus we conclude that we can evaluate $x \mapsto \partial_{p+1} \partial_{p+1}^T x$ is $\approx O(m)$ time, essentially linear in the number of $p+1$ simplices. 
