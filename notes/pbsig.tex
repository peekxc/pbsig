...
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	...
	\item Construct a combinatorial Laplacian operator $L_p(K)$ for the homology dimension $p \geq 0$ of interest, The choice of Laplacian operator depends on the application; we will in many cases generically use:
	$$ L_p^{\mathrm{up}}(K, f_\alpha) = W_p \partial_{p+1} W_{p+1} \partial_{p+1}^T W_p $$ 
	whose matrix vector product $x \mapsto L_p(K) x$ can be efficiently computed. The implementation of this operator computationally need not involve constructing the Laplacian $L_p$ itself nor does it necessarily require $K$ is available explicitly in memory. 
