...
\subsection*{Rank invariant relaxation}
...
To more formally convey the utility permutation invariance yields in parameterized settings, consider the following definition of a \emph{parameterized} boundary operator $\partial_p$:
...
%	\simeq H() 
\end{equation}
Like $L$, all three operators $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are symmetric, positive semidefinite, and compact. When $p = 0$, we recover $\Delta_0(K) = \partial_1 \partial_1^T = L$ since $L_0^{\text{dn}} = 0$.
...
We first recall the characteristics of the graph Laplacian $L = D - A$ derived from an undirected simple graph $G = (V, E)$. As in~\cite{chung1997spectral}, the linear and quadratic forms of $L$ may be succinctly expressed using the path-connected relation $i \sim j$, shown below:
