...
We now record a fact that exploits this idea and, as we will show, has non-trivial computational implications.
...
A little known proposition that follows from this, which has also been pointed out in~\cite{dey2022computational}, is that Lemma~\ref{lemma:rank} may be used to write the persistent Betti number in a simpler way as a sum of rank functions. 
...
\end{proposition}
\noindent We give a detailed proof of Proposition~\ref{prop:rank_reduction} in the appendix. It turns out Lemma~\ref{lemma:rank} can also be further generalized to arbitrary rectangles $R = [i,j] \times [k,l]$ in $\Delta_+^N$, see the fourth picture in Figure~\ref{fig:mult}.
Geometrically, these rank computations manifest as $\mu$\emph{-queries}: box-parameterized multiplicity queries which count the number of persistence pairs that intersect a fixed ``box'' placed in the index persistence plane. 
As a result, the $p$-th multiplicity function can also be defined in a rank-based formulation akin to~\eqref{eq:betti_four}, which was proven independently by Chen \& Kerber~\cite{chen2011output}. 
