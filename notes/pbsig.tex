...
\noindent 
\textbf{Contributions: }
In this effort, we introduce an iterative method capable of approximating both topological invariants $\mu_p^{\ast}$ and $\beta_p^\ast$---\emph{the counting invariants}---in essentially $O(m)$ memory and $O(mn)$ time, where $m, n$ are the number of $p+1, p$ simplices in the complex, respectively. 
The approximation obeys certain stability properties which allow it to be particularly efficient when [re]computation is required on slightly perturbed inputs, which we call the \emph{parameterized setting}. 
The accuracy parameter $\epsilon > 0$ on which the approximation depends recovers both invariants exactly when $\epsilon$ is made small enough.
In deriving the approximation, we naturally obtain continuous variations of both counting invariants which are both globally Lipshitz in the space of real-valued matrices and differentiable on the positive semi-definite cone, which we believe to be of independent interest. 
Moreover, the approximation does not modify $\partial$ (as in~\cite{edelsbrunner2022computational}) and is permutation invariant is a certain sense, forgoing the need to complicated $O(m^2)$ filtration-maintenance procedures (as in~\cite{cohen2006vines}). Indeed, the algorithm need not store $\partial$ in memory, nor does it require the underlying complex $K$ to be explicitly filtered.
...
In this section we discuss the computational details involved in evaluating the rank function on the boundary matrices whose columns take the form~\eqref{eq:alt_sum}. We start general by working with arbitrary matrices. Given a matrix $X \in \mathbb{R}^{n \times m}$ satisfying $n < m$ and its singular value decomposition (SVD) $X = U \Sigma V^T$, define the \emph{rank} of $X$ as the composition:
...
%This is called a projection because each $q_{j+1} \perp \{q_1, q_2, \dots, q_j \}$, and~\eqref{eq:krylov_proj} can be thought of as successive orthogonalization, 
 ...
 In fact, the three-term recurrence from~\eqref{eq:three_term_rec} implies the Lanczos iteration may be carried out with just three $O(n)$-sized vectors and $O(1)$ additional storage. We summarize the main take-a-ways of the Lanczos method we've discussed thus far with a Lemma. 
...
\end{lemma}
\noindent As in~\cite{parlett1994we}, the assumption of exact arithmetic simplifies both the presentation of the theory and the corresponding complexity statements. In practice, this is unrealistic, and we must use finite-precision arithmetic. Extending the Lanczos method to retain similar complexity guarantees using finite-precision arithmetic is a non-trivial and multifaceted topic; each such extension of the iteration essentially to control for loss of orthogonality between the Lanczos vectors or improve convergence rate (or both), which typically involves increasing either the time or storage complexities by non-trivial factors. We defer its discussion to section~\ref{subsec:rounding}. 
Independent of orthogonality-enforcement scheme of choice, the assumption of exact arithmetic gives us a grounded expectation of what is possible to achieve with any such scheme. 
\begin{corollary}\label{cor:finite_arith_lanczos}
	Given the same inputs as Lemma~\ref{lemma:exact_arith_lanczos}, any implementation that computes $\Lambda(A) = \{ \lambda_1, \lambda_2, \dots, \lambda_r \}$ using the Lanczos iteration in finite-precision arithmetic requires $\Omega(\mathcal{M}(n) \cdot r)$ time and $\Omega(n)$ storage complexity. 
\end{corollary}
\noindent
...
In principle, the three-term recurrence that drives the Lanczos method only be exploited with symmetric matrices, and as previously mentioned, the Lanczos iteration is only efficient if one as has access to a fast matrix-vector product. Though the boundary matrix $\partial$ is not symmetric, it is well known that the non-zero part of the spectrum of $\partial \partial^T$ contains the same information as the singular values of $\partial$. By our definition of rank in~\eqref{eq:rank_def}, this implies $\mathrm{rank}(\partial) = \mathrm{rank}(\partial \partial^T) = \mathrm{rank}(\partial^T \partial)$ and thus the Lanczos method may be readily applied to $\partial \partial^T$ or $\partial^T \partial$. % up to a change in the multiplicity of the zero 
%since the latter are by definition the nonnegative square roots of former and thus . 
Moreover, since the matrix $\partial_1 \partial_1^T$ is the well studied \emph{graph Laplacian}, we can consider the study of spectra of combinatorial Laplacians---generalizations of the graph Laplacian for $p > 0$---as the study of singular values of boundary operators. In what follows, we examine the form of $\partial_p \partial_p^T$ for more general $p > 1$ towards revealing exploitable structure in accelerating the rank computation.
...
The $p$-th \emph{combinatorial Laplacian} $\Delta_p$ is given by: 
\begin{equation}
	\Delta_p(K) = 
	\underbrace{\partial_{p\+1} \circ \partial_{p\+1}^T}_{L_p^{\text{up}}} + \underbrace{\partial_{p}^T  \circ  \partial_{p}}_{L_p^{\text{dn}}} 
%	\simeq H() 
\end{equation}
When $p = 0$, we have $\Delta_0(K) = \partial_1 \partial_1^T = L$ is the \emph{graph Laplacian} $L = D - A$ of a graph $G = (V, E)$, where $A \in \mathbb{R}^{n \times n}$ denote the graphs adjacency matrix $D$ is the diagonal degree matrix $D = \mathrm{diag}(\{ \, \mathrm{deg}(v_i) \, \}$. 
The graph Laplacian is a heavily structured matrix known to capture the connectivity structure of $G$. In particular, $L$ is symmetric, positive semi-definite, and it's linear and quadratic forms have particular graph-theoretic interpretations:
...
Given a simplicial complex $K$, we denote with $\mathrm{co}(\tau) = \{ \, \sigma \in K^{p+1} \mid \tau \subset \sigma \, \}$ the set of proper cofaces of $\tau \in K^p$, or \emph{cofacets}.
We would like to generalize the path-connected notation in~\eqref{eq:lap_quad_from} to the setting where $p \geq 1$. 
Since $K$ is a simplicial complex, note that if $\mathrm{co}(\tau) \cap \mathrm{co}(\tau') \neq \emptyset$, then the $\tau, \tau'$ not only share a cofacet $\sigma$, but this cofacet is \emph{unique}~\cite{}. Thus, given two distinct $p$-simplices $\tau \neq \tau'$ in $K^p$, we use the notation $\tau \overset{\sigma}{\sim} \tau'$ to denote the cofacet $\{\sigma\} = \mathrm{co}(\tau) \cap \mathrm{co}(\tau')$, and we use $\mathrm{d_f}(\tau) = \sum_{\sigma \in \mathrm{co}(\tau)} f(\sigma)$.
...
\begin{align}
	(W \partial_p W^2 \partial_p^T W)[i,j] = L_p^{\text{up}}[i,j] = \begin{cases}
		f(\tau_i)^2 \cdot d_f(\sigma)^2 & \text{ if } i = j \\ 
%		\mathrm{deg}(\tau_i) & \text{ if } i = j \\ 
		(-1)^{s_{ij}} f(\tau_j)f(\tau_j)f(\sigma)^2 & \text{ if } \tau_i \overset{\sigma}{\sim} \tau_j \\
		0 & \text{ otherwise} 
	\end{cases}
\end{align}
where $s_{ij} = \mathrm{sgn}([\tau_i], \partial[\sigma]) \cdot \mathrm{sgn}([\tau_j], \partial[\sigma])$.
The sign of the coefficients in $[\tau]$ and $[\tau']$ depends on the position as summands in the boundary of $\sigma$. 
As one can see from~\eqref{eq:oriented_simplex}, there are effectively only two classes of orientations, and thus this task is equivalent to having an explicit expression for the position of $[\tau]$ as a summand in~\eqref{eq:alt_sum}, modulo 2.
...
Fortunately, this requires a simple augmentation to the step function from~\eqref{def:time_boundary_matrix2}. In particular, we swap out the step functions $S : \mathbb{R} \to \{ 0, 1\}$ in ~\eqref{eq:param_boundary_matrix} with continuous \emph{smoothstep} functions $\mathcal{S}: \mathbb{R} \to [0, 1]$. 
\begin{figure}
\centering
%	\includegraphics[width=0.45\textwidth]{mult_both}
