...
\subsection{Complexity of Persistence} TODO 


\section*{An quadratic-time rank computation}
In this section we discuss the computational details involved in evaluating the rank function on the boundary matrices whose columns take the form~\eqref{eq:alt_sum}. For any $X \in \mathbb{R}^{n \times m}$ with $n < m$ that admits a singular value decomposition (SVD) $X = U \Sigma V^T$, define the \emph{rank} of $X$ as composition of the one-side sign function and the singular value function:
...
In particular, we focus on the Lanczos iteration, and related adaptations to positive semi-definite, diagonally dominant (DD) and strictly diagonally dominant (SDD) cases, and the connections the boundary operators have with graph Laplacians. We also show how these methods made be adapted to efficiently compute~\eqref{eq:pbn_parameterized} and~\eqref{eq:mu_parameterized}.
...
The latter may be performed in $O(n \log n)$ time~\cite{gu1995divide}, whereas the former is effectively bounded below by $\Omega(n^3)$ for dense matrices using non-Strassen-like computations, and thus it is the reduction to tridiagonal form that dominates the computation. 
Lanczos~\cite{} proposed the so-called \emph{method of minimized iterations}, now known as the \emph{Lanczos method}, as an attractive alternative for obtaining a tridiagonal form of sparse, symmetric $A$.
...
The spectral theorem implies that $A$ is orthogonally diagonalizable, since $A$ is symmetric, and thus we may obtain $\Lambda(A)$ by generating an orthonormal basis for $\mathcal{K}_n(A, v)$. 
...
\end{equation}
is the QR factorization of $K_n(A, q_1)$, where $e_1$ is the first column of $I_n$. Thus, tridiagonalizing $A$ with respect to a unit-norm $q_1$ completely characterizes $Q$. 
...
% The Lanczos iteration's widespread adoption is in part due to its computational simplicity at reaching tridiagonal form: each step consists of a single matrix-vector product and a few vector operations. .  
 The Lanczos iteration and its many variants are part of a family of so-called ``matrix free'' methods---from the interfacing side, obtaining an eigen-decomposition of a symmetric real matrix $A$ requires only a $v \mapsto Av$ operator. In particular, since $A$ is not modified at all during the computation, one need necessarily involve explicitly storing $A$ in memory. Moreover, the three-term recurrence from~\eqref{eq:three_term_rec} implies the Lanczos iteration may be carried out with just three $O(n)$-sized vectors.
Indeed, if $A$ is sparse and/or has special structure, the iteration may be carried in far fewer than $\approx n^3$ operations and with far less than the $\approx n^2$ memory---the typical time and storage complexities exhibited by direct methods. 
...
However, the sparsity pattern $\partial_p$ inherits from the simplicial structure of $K$ confers a few computational advantages compared to more general linear operators. 
In what follows, we take advantage of the the simplicial structure of $K$ and the fact that $\mathrm{rank}(A) = \mathrm{rank}(A A^T) = \mathrm{rank}(A^T A)$ to both elucidate the structured form of the matrix vector operator and highlight connections to other areas of applied mathematics.
For simplicity, we focus on the case where $p = 0$, and defer higher dimensions for later. 
...
\begin{equation}\label{eq:lap_hadamard}
	...
	\end{cases}
\end{equation}
In all cases, if $L = \partial_1 \partial_1^T$ where $\partial_1 \in \mathbb{R}^{n \times m}$ and $n < m$, then one may observe that the corresponding matrix-vector product for equations~\eqref{eq:lap_edge}-\eqref{eq:lap_hadamard} can all be evaluated in $O(m)$ time. We end this section with a proposition, which essentially follows by combing the above observations with Proposition~\ref{prop:exact_arith_lanczos}.
\begin{proposition}\label{prop:exact_arith_matvec}
...
\subsection{Convergence Rate \& Rounding \& SDD} TODO
%\< TODO \> 
...
\centering
%	\includegraphics[width=0.45\textwidth]{mult_both}
	\includegraphics[width=0.45\textwidth]{smoothstep}
	\caption{The step and smoothstep functions, respectively. Note that }
	\label{fig:mult}
\end{figure}
...
On one hand, equation~\eqref{eq:pbn_stability} shows that PBN functions \emph{are} stable with respect to the extended matching distance: if the two filter functions are $\epsilon$-close in the sense that $\lVert f - f' \lVert_\infty \leq \epsilon$, then the PBN functions have distance $d(\beta_f, \beta_{f'}) \leq \epsilon$. 
On the other hand, computationally, the extending matching distance $d(\beta_f, \beta_{f'})$ requires finding the optimal matching $\phi$ between the points $p \in \Lambda_+$ with non-zero multiplicity, which effectively requires the persistence diagrams of $f$ and $f'$ to compute. Since both (a) computing the full persistence diagram and (b) obtaining the optimal matching are both  high complexity algorithms, we would like to avoid them while still retaining some information contained in them, if possible. 
We tackled the form by fixing specific \emph{regions} of $\Lambda_+$ a-priori; we now focus on on the latter.

One disadvantage in working with counting-type functions restricted to portions of real-plane is that they are not smooth. 
Namely, if $(K, f_h)$ is a parameterized filtration where $K$ is a fixed simplicial complex and $f : \mathcal{H} \times K \to \mathbb{R}$, it's possible to construct counter examples where $\mu_p^R(K, f_h) \sim O(\lvert K^{(p)} \rvert)$ and $\mu_p^R(K, f_{h'}) = 0$, for some choice $h, h' \in \mathcal{H}$ with $\lvert h - h' \rvert$ arbitrarily small~\cite{}.
Intuitively, as both of the invariants studied so far are counting type invariants restricted to a portion of $\Lambda_+$, it's always possible to encounter such situations where small changes in the input  affect the corresponding invariant in a non-Lipshitz way.

 
Like the PBN, the discontinuous nature of the rank function is due to its inherently combinatorial properties. In particular, the discontinuity in~\eqref{eq:rank_def} manifests due to the one-sided sign function. Several other authors~\cite{}, have confronted this discontinuity by relaxing $\mathrm{rank}(X)$ with the nuclear norm $\lVert X\rVert_\ast$. This is partially motivated by the fact that, if $x = (\sigma_1, \dots, \sigma_n)$ is a vector containing the singular values of $X$, then: 
\begin{equation}\label{eq:pseudo_norm}
	\mathrm{rank}(X) = \lVert x \rVert_0  \approx \lVert x \rVert_1 = \lVert X \rVert_\ast 
\end{equation}
...
In the spirit of~\cite{}, we consider a family of continuous $\mathrm{sgn}$ approximation functions $\phi: \mathbb{R}_+ \times \mathbb{R}_{++} \to \mathbb{R}_+$, each of which when composed with the singular value function yield a continuous function that approximates the rank function to any pre-prescribed accuracy. Given a positive $\epsilon > 0$, the basic quality we want out of such a relaxation is:
\begin{flalign}\label{eq:lap_quad_from}
	(\, \forall \, x > 0 \,)  & & \quad\quad\quad 
%\begin{equation}
	\lim_{\epsilon \to 0^+} \phi(x, \epsilon) = \mathrm{sgn}_+(x),\quad \quad & \mathrm{rank}(X) \approx \sum\limits_{i=1}^n \phi(\sigma_i(X), \epsilon) & &&
\end{flalign}
%\end{equation}
Following~\cite{}, Bi et al.~\cite{} give a framework for constructing such smoothing functions which is motivated by the fact that $\mathrm{sgn}_+(x)$ may be equivalently expressed as an integral of the Dirac delta function.  
...
\begin{equation}
	\phi(x, \epsilon) := \int\limits_{-\infty}^x \hat{\delta}(z, \epsilon) dz, \quad \forall \, z \geq 0
\end{equation}
where $\hat{\delta}(z, \epsilon) = \nu(\epsilon)^{\text{-}1} p \left ( u \nu(\epsilon)^{\text{-}1} \right )$ for some continuous density function $p: \mathbb{R}_+ \to \mathbb{R}_+$ and some choice of continuous increasing $\nu : \mathbb{R}_+ \to \mathbb{R}_+$ satisfying $\nu(0) = 0$ and $\nu(\epsilon) > 0$. 
These function were shown to satisfy a number of nice properties of these functions; namely, if $p$ is continuous on $\mathbb{R}_+$, then $\phi(\cdot, \epsilon)$ is continuously differentiable in $\mathbb{R}_+$, and if $p$ is bounded above on $\mathbb{R}_+$, then $\phi(\cdot, \epsilon)$ is globally Lipshitz continuous on $\mathbb{R}_+$. 
Moreover, given a real matrix $X \in \mathbb{R}^{n \times m}$ with SVD $X = U \Sigma V^T$, one obtains an continuously differentiable operator $\Phi_\epsilon$ for any fixed $\epsilon > 0$ by replacing $\sigma \mapsto \phi(\sigma, \epsilon)$ and mapping $X$ as follows:
...
\begin{align*}
 \Phi_\epsilon : \quad & \mathbb{R}^{n \times m}  & \to \quad & \mathbb{R}^{n \times m}  \\
						           & X & \mapsto \quad &  U \, \mathrm{diag}( \phi(\sigma_1, \epsilon), \phi(\sigma_2, \epsilon), \dots, \phi(\sigma_n, \epsilon)) \, V^T \numberthis
\end{align*}
%whose nuclear norm is semismooth on $\mathbb{R}^{n \times m}$. 
we summarize the main points: 
\begin{proposition}[Continuous Rank Relaxation~\cite{}]
	\begin{enumerate}
...
To adapt these relaxations to the PBN functions as defined via~\eqref{}, we need to modify the chain entries in~\eqref{} to vary continuously as functions of $\mathcal{H}$. 
The 
Fortunately, our $\mathcal{H}$-parameterized formulation admits a simple augmentation, similar to this change in the sign function. In particular, we simply swap out the step functions $S : \mathbb{R} \to \{ 0, 1\}$ in ~\eqref{eq:param_boundary_matrix} with continuous \emph{smoothstep} functions $\mathcal{S}: \mathbb{R} \to [0, 1]$. 
These are sigmoid-like functions whose ranges are clamped to the unit interval which interpolate between a prescribed lower and upper bounds $(a,b)$ via a Hermite polynomial. : 
\begin{equation}
\mathcal{S}_n^{a,b} (x) = \begin{cases}
	0 & x \leq a \\
	S_n( (b - x)(b - a)^{\text{-}1}) & a < x < b \\
	1 & b \leq x
\end{cases}
\end{equation} 
where $S_n: [0,1] \to [0,1]$ are generic $n$-th order polynomials of the form: 
\begin{equation}
S_n(x)  = a_n x^n + a_{n-1} x^{n-1} + \dots + a_1 x + a_0
\end{equation} 
whose coefficients are fixed such that $S_n(0) = 0$ and $S_n(1) = 1$. These polynomials have found applications in compute graphics and machine learning applications~\cite{}. 
Our motivation to use them here is based on the observations shown in Figure~\ref{}: by substituting $\mathcal{S}_n^{a,b}$ for the step functions in~\eqref{eq:param_boundary_matrix}, we have a version of the parameterized boundary matrix whose entries varies continuously in $\mathcal{H}$. Moreover, by composing with~\eqref{eq:lap_hadamard} and substituting $\mathrm{rank}(\cdot)$ with $\phi(\cdot, \epsilon)$ for some appropriately chosen $\epsilon > 0$, we have $\epsilon$-approximate, continuously-varying approximation $\hat{\mu}_p^R$ of $\mu_p^R$ is given as: 
\begin{equation}
	\mu_P^R = \phi(\hat{L})
\end{equation}
where 
...
Note that when $\nu(\epsilon) = \sqrt[\leftroot{-2}\uproot{2}p]{\epsilon}$ and $p = 2$, Equation~\eqref{} reduces to: 
\begin{equation}
	\phi_\epsilon(X) = \sum\limits_{i = 1}^n \frac{\sigma_i(X)^2}{\sigma_i(X)^2 + \epsilon} = \mathrm{Tr}\left[X (X X^T + \epsilon \, I_n) X^T\right]
\end{equation}
...
On the other hand, for $f$ a real-valued filter function that various smoothly in $\mathcal{H}$, one would expect the \emph{spectra} of the constitutive terms in $\beta_p^\ast$ and $\mu_p^\ast$ to also vary smoothly as functions of $\mathcal{H}$.
