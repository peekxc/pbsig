...
\end{lemma}
\noindent We now seek to specialize Lemma~\ref{lemma:rank} to the setting where only the $p$-th dimensional homology classes are of interest. This turns out to be particularly easy, as one may observe the reduction algorithm only adds $p$-chains to $p$-chains. That is, if a simplex $\sigma_j$ corresponds to column $j$ in $\partial$, the only columns that may ever be added to $j$ correspond to simplices of dimension $p$ as well. Hence, if we set all columns corresponding to simplices of dimension $q \neq p$ to $0$ in the $m \times m$ boundary matrix $\partial$, then $\partial$ represents the $p$-th boundary operator $\partial_p : C_p(K_\bullet) \to C_{p-1}(K_\bullet)$. 
We will use $\partial_p$ and $R_p$ to refer to these modified matrices of $\partial$ and $R$ for the remainder of this section. 
%with the filter function $f : K_\bullet \to [m]$. 
