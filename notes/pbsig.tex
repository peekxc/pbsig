...
\begin{enumerate}
	...
	\item Choose any rectilinear \emph{template} $\mathcal{R} \subset \Delta_+$ on the output space of diagrams $\Delta_+ = \{(i,j) \in \bar{\mathbb{R}}^2  \mid i < j \}$ within which $f_\alpha$ exhibits non-trivial topological behavior.
	 This choice depends on the application and should be chosen either by a practitioner based on domain knowledge. Alternatively, randomly setting $\mathcal{R}$ via a sampling approach will be sufficient if the goal is featurization or one is in the exploratory setting where $\mathcal{R}$ is unknown.
	
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	\item Choose a relaxation parameter $\epsilon > 0$ representing how closely the relaxation should model the quantity: 
	$$ \mathrm{card}(\mathcal{R}_\alpha) \triangleq \mathrm{card}(\mathcal{R} \cap \mathrm{dgm}(K, f_\alpha))$$
	Larger choices $\epsilon$ yield smoother relaxations that depend more on geometric and combinatorial structure of $K$, whereas smaller values of $\epsilon$ more accurately approximate the topology of $f_\alpha$. 
	\item For any fixed homology dimension  of interest $p \geq 0$, construct a combinatorial Laplacian operator $L_{i,j}(K)$ at every ``corner point'' $(i,j) \in \mathcal{R}$.
	$$ L_{i,j}^{\mathrm{up}}(K, f_\alpha) = \mathcal{D}_{i}^{1/2}( \alpha) \circ \partial \circ W_{j}( \alpha ) \circ \partial^T \circ \mathcal{D}_{i}^{1/2}( \alpha) $$
	The particular choice of Laplacian operator depends on the application; for stability reasons discussed in section~\ref{}, we will in this effort consider the symmetric normalized Laplacian.
	\item For each points $(i,j) \in \mathcal{R}$, choose a vector $v \in \textbf{1}^\perp$ and expand the Krylov subspaces of $\mathcal{L} = L_{i,j}$:
	$$\mathcal{K}_r(\mathcal{L}, v) := \mathrm{span}\{ v, \mathcal{L}v, \mathcal{L}^2 v, \dots, \mathcal{L}^{r-1} \}$$
	Until the $\mathcal{K}_r(\mathcal{L}, v)$ $\epsilon$-approximates an invariant subspace of $\mathcal{L}$. If enough memory is available, store...
	\item For each $\alpha$
%	\item All we require is 
\end{enumerate}
...
In fact, if $\phi$ is twice-differentiable at each $\sigma_i(X)$ for all $i = 1, \dots, n$, then~\eqref{eq:lowner} is also twice continuously differentiable~\cite{ding2018spectral} at $X$. 
...
\noindent
\textbf{Regularization:} Another approach to stabilizing the rank function in the context of large-scale sparse inverse problems is to regularize the solution. To be more precise, we require a more specific choice of $\phi$ in~\eqref{eq:phi}. For illustrative purposes\footnote{Similar expressions may be derived for other choices of $\nu(\epsilon)$ and $p(x)$.}, consider setting $\nu(\epsilon) = \sqrt{\epsilon}$ and $p(x) = 2x (x^2 + 1)^{-2}$. By~\eqref{eq:phi}, one obtains:
\begin{equation}
	\phi(x, \epsilon) = \int\limits_{-\infty}^x \hat{\delta}(z, \epsilon) dz = \frac{x^2}{x^2 + \epsilon}
\end{equation}
By substituting $\mathrm{sgn}_+ \mapsto \phi$, composing with the singular value function~\eqref{def:cont_rank}, and applying the cyclic trace property, the corresponding spectral rank approximation reduces to:
\begin{equation}\label{eq:tikhonov_1}
	\lVert \Phi_\epsilon(X) \rVert_\ast = \sum\limits_{i = 1}^n \frac{\sigma_i(X)^2}{\sigma_i(X)^2 + \epsilon} = \mathrm{Tr}\left[(X X^T + \epsilon \, I_n)^{-1} X X^T \right] 
\end{equation}
Observe the expression on the right hand side of~\eqref{eq:tikhonov_1} contains an $\epsilon$-regularized variant of the pseudoinverse used in large-scale sparse inverse problems. 
Indeed, evaluating the trace function above reduces to solving $n$ least-squares problems regularized by the \emph{Tikhonov regularization} (TR). Setting $A = X$, solutions $x_\epsilon^\ast$ to TR problems in standard form satisfy: 
\begin{equation}
	x_\epsilon^\ast = \mathrm{argmin}_{x \in \mathbb{R}^n} \lVert Ax - b\rVert^2 + \epsilon \lVert x \rVert^2
\end{equation}
Thus, one interpretation of the relaxation parameter $\epsilon$ is as a regularized pseudo-inverse of $X$, where the regularization parameter $\epsilon$ controls the strength of the regularization and determines the degree to which the inverse is biased towards smaller singular values.
...
%Indeed, adding larger $\epsilon$ amount of mass to the diagonal is a common approach to improve the conditioning of a matrix, as it not only guarantees a singular matrix full rank, but it also guarantees the smallest eigenvalue of is at least $\epsilon$.
...
Moreover, the continuous differentiability of $\lVert \Phi_\epsilon(\cdot) \rVert_\ast$ only applies to linear operators over the positive semi-definite cone, and the explicit form of the differential $\partial \lVert \Phi_\epsilon(\cdot) \rVert_\ast$ depends on the domain of its corresponding spectral function $\phi(\cdot)$ being bounded to some interval $(a,b)$~\cite{bhatia2013matrix}. 
We address all of these issue in the following section.  
...
Like the identities $\mathrm{rank}(A) = \mathrm{rank}(A A^T) = \mathrm{rank}(A^T A)$, it is well known that the square roots of the non-zero eigenvalues of either $AA^T$ or $A^T A$ yield the singular values of $A$. 
By expressing the rank invariants solely in terms of boundary operators $\partial_p$, we may exploit the fact that $L = \partial_1 \partial_1^T$ is the well studied \emph{graph Laplacian}. 
More generally, we may consider the study of spectra of \emph{combinatorial Laplacians}---generalizations of the graph Laplacian---as the study of singular values of boundary operators, the central quantities of study thus far. 
%Towards revealing  
...
As these extensions are multifaceted, non-trivial, and beyond the scope of the present work, we defer their discussion to the appendix~\ref{subsec:rounding}. The curious reader is referred to~\cite{golub2013matrix} and references therein for an overview. 
