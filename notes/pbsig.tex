...
For a real, square matrix $A$ of order $n$, the quadratic form $x^T A x$ defines a continuous real-valued function of $x \in \mathbb{R}^n$. When $A$ is symmetric positive definite, the implicit equation $x^T A x = 1$ defines an $n$-dimensional ellipsoid $y^T \Lambda y = \lambda_1 y_1^2 + \lambda_2 y_2^2 + \dots + \lambda_n y_n^2 = 1$ whose $n$ principle axes are the eigenvectors $\{v_i\}_{i=1}^n$ which satisfy $A v_i = \lambda_i v_i$ and whose eigenvalues $\Lambda(A) =\{ \lambda_1, \lambda_2, \dots, \lambda_n \}$ are real-valued.  
% https://netlib.org/lanczos/vol2/Chp_2_RealSymmetric.pdf
For any two distinct eigenvalues of $A$, $\lambda$ and $\lambda'$, the corresponding eigenvectors $v$ and $u$ are orthogonal, thus we may obtain $\Lambda(A)$ by constructing an orthonormal eigenvector basis $V$ of $A$, via orthogonal diagonalization:
