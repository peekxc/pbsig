...
Historically, persistence diagrams were first used as shape descriptors due to their effectiveness in tackling the \emph{homology inference problem}~\cite{}: by pairing simplices using homomorphisms between homology groups, topological features are demarcated succinctly with diagrams.
...
Persistence connects homology theory to the application frontier: whether for shape recognition, computer vision, metric learning, dimensionality reduction, or time series analysis, researchers have found ways of exploiting the rich information contained in persistence diagrams.
...
Towards ameliorating many of these issues, practitioners have developed ways of vectorizing diagrams by mapping them to function spaces, such as Hilbert spaces~\cite{}---popular examples include persistence images~\cite{}, persistence landscapes~\cite{}, and template functions~\cite{}. 
These vectorizations have proven useful for learning applications due to their stability and configurability~\cite{}, however, they are limited practically as they require diagrams for their computation.
Along a different line of research addressing the issue of injectivity, Turner et al. propose the \emph{persistent homology transform} (PHT), which is a shape statistic associating an embedded data set $X \subset \mathbb{R}^d$ with a collection of diagrams via the map: 
...
However, this approach exacerbates both computational problems, as extending the defacto standard persistence computation to parameterized settings has proven non-trivial~\cite{} and achieving rotation invariance in~\eqref{eq:pht1} requires many expensive diagram comparisons.  %moves
 
We seek to shift the computational paradigm: rather than constructing diagrams as a first step and vectorizing them as a second step, we propose a means of performing both steps simultaneously. 
...
However, unlike existing approaches, our proposed vectorization involves a spectral characterization that avoids the reduction algorithm from~\cite{edelsbrunner2022computational} entirely.  
%Among the many benefits of our approach is its iterative nature: 
...
So as to not leave the reader in suspense, we first outline the proposed relaxation informally. The rest of the paper discusses the theoretical and practical details. At a high level, the relaxation we propose constructs a vector-valued mapping, or \emph{vectorization}, over a set $\mathcal{A}$: 
$$ (X, \mathcal{R}, \epsilon) \mapsto \mathbb{R}^{\lvert \mathcal{A} \rvert}$$
where $X$ is the input a data set, $\epsilon$ a relaxation parameter, and $\mathcal{R}$ a \emph{sieve} which sifts through $\mathcal{A}$, summarizing the topological and geometric behavior observed by $(K, f_\alpha)$ for all $\alpha \in \mathcal{A}$ in the process.  The steps to produce this vectorization are as follows:
\begin{enumerate}
...
Informally, the spectra of $\mathcal{L}$ encodes the geometry and topology of the intersection pattern of $(K, f_\alpha)$, and $\Phi$ determines which information to prioritize in the vectorization. 
	\item For each corner point $(i,j) \in \partial(\mathcal{R})$, restrict and project $\mathcal{L}$ onto the Krylov subspace: 
	$$
...
\end{enumerate}
The remaining steps of the relaxation depend on the application in mind. The duality between diagrams and rank functions suggests any application exploiting vectorized persistence information may benefit from our relaxation. 
Moreover, the differentiability of our relaxation enables learning applications to optimize persistence information. We capture these two generic application domains as follows: 
...
Exemplary applications of (B) include filtration optimization, incorporating topological priors into loss functions, and....
\\
\\
