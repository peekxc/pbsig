...
Persistence-related pipelines typically follow a well-established pattern: given input data set $X$, construct a filtration $(K, f)$ from $X$ such that useful topological or geometric information may be profitably extracted from its \emph{persistence diagram}~\cite{}.
%$$ \mathrm{dgm}(K, f) = \{ (i,j) \in \mathbb{R}^2 : i < j \}$$ 
Historically, persistence diagrams were first used as shape descriptors due to their effectiveness in tackling the \emph{homology inference problem}~\cite{}, as they  
...
demarcate topological features by pairings of simplices defined using homomorphisms between homology groups. 
...
Indeed, persistence provides the bridge between the theory of homology and the application frontier: domains which operate on noisy measurement data like shape recognition, computer vision, and time series data have all found ways of exploiting the information contained in persistence diagrams.

Though theoretically sound, diagrams suffer from several practical issues: they are sensitive to strong outliers, far from injective, and are difficult both to compute \emph{and} to compare. 
Indeed, it is difficult to do even basic statistical operations under the standard matching metrics, such as averaging~\cite{}. 
Towards ameliorating many of these issues and facilitating the use of persistence in learning applications, many practitioners have developed means of vectorizing diagrams via maps to function spaces, such as Hilbert spaces~\cite{}---popular examples include persistence images~\cite{}, and persistence landscapes~\cite{}, template functions~\cite{}. 
These diagram-vectorizations have proven useful in learning purposes due to their stability and configurability~\cite{}, they are fundamentally limited in that they all require diagrams to compute.
Along a separate thread of research, Turner et al. propose the \emph{persistent homology transform} (PHT) to handle the injectivity issue: rather than compute one diagram, compute many, each diagram representing a different perspective. 
The main result of~\cite{} is that this collection of diagrams is a sufficient to reconstruct the data.
This sufficiency result of the PHT is both an exciting and foundational development, as it sparks an entire inverse theory for persistence and in fact motivates the creation of a metric. 
However, it too relies on the computation of diagrams: in fact, it exacerbates the problem computationally, as computing diagrams in parameterized settings has proven to be non-trivial~\cite{}.  
 
We seek to shift the paradigm: rather than constructing diagrams first and vectorizing second, we perform both steps simultaneously. 
Our strategy is natural in that it extracts persistence information using the duality between diagrams and their rank functions~\cite{}.
However, unlike existing approaches, our proposed vectorization involves a spectral characterization that avoids the reduction algorithm~\cite{edelsbrunner2022computational} entirely, which has proven non-trivial to extend to the parameterized setting~\cite{}. % moves 
We are partially motivated by recent relationship established between the persistent Betti numbers and combinatorial Laplacian operators, and by a technical observation that suggests several computational advantages to working with the rank invariant directly (see section~\ref{sec:betti_derivation} for details). 
...
So as to not leave the reader in suspense, we first outline the proposed relaxation informally. The rest of the paper discusses the theoretical and practical details. At a high level, the relaxation we propose constructs a vector-valued mapping, or \emph{vectorization}, via: 
$$ (X, \mathcal{R}, \epsilon) \mapsto \mathbb{R}^{\lvert \mathcal{A} \rvert}$$
where $X$ is the input a data set, $\epsilon$ a relaxation parameter, and $\mathcal{R}$ a \emph{sieve} which sifts through a set $\mathcal{A}$, summarizing the topological and geometric behavior observed by $(K, f_\alpha)$ for all $\alpha \in \mathcal{A}$ in the process.  The steps to produce this vectorization are as follows:
\begin{enumerate}
	...
	This choice is typically application-dependent as it requires a priori knowledge. In section~\ref{sec:applications} we give evidence random sampling may be sufficient when $\mathcal{R}$ is unknown.
...
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	...
	\item Associate with each unique \emph{corner point} $(i,j) \in \partial(\mathcal{R})$ an $\alpha$-parameterized combinatorial Laplacian operator $\mathcal{L}_{ij}(K, f_\alpha)$:
$$ \mathcal{L}_{ij}: \mathcal{R} \; \to \; C_1(\mathcal{A}, \mathbb{R}_+)  $$
...
\end{enumerate}
The remaining steps of the relaxation depend on the application in mind. Below, we record two generic objectives and give several example applications that fit within the scope of each objective.
 
The duality between diagrams and rank functions suggests any application which exploits vectorized persistence information may benefit from our relaxation. In particular, any application which, either implicitly or explicitly, depends on \emph{multiplicity} information may benefit. 
\begin{tcolorbox}[title=Generic application \#1: Vectorizing persistence information]
	Compute an $\epsilon$-approximation of $\mu_p(\mathcal{R}_\alpha)$ over some family $\mathcal{A}$ (in subcubic time per $\alpha$)
\end{tcolorbox}
\noindent
Such applications include, but are not limited to: characterizing swarm and flocking behavior with Betti curves~\cite{}, topology-guided image denoising~\cite{}, detecting bifurcations in dynamical systems~\cite{}, generating metric invariants for shape classification and metric learning~\cite{},  and so on. 

The fact that our relaxation is differentiable enables applications which seek to perform a learning task with persistence information.
...
\end{tcolorbox}
\noindent Exemplary applications include filtration optimization, incorporating topological priors into loss functions, and....
 \\
 \\
...
\noindent 
\textbf{Contributions:}
Our contributions are summarized as follows. We introduce an iterative $\epsilon$-approximation method for computing the \emph{the persistent rank invariants}---$\mu_p^{\ast}$ and $\beta_p^\ast$---in \emph{essentially} $O(m)$ memory and $O(mn)$ time, where $m, n$ are the number of $p+1, p$ simplices in the complex, respectively. 
...
In contrast, the second term---the persistence part---typically requires finding a basis in intersection of the two subspaces via either column reductions or projection-based techniques. In general,  direct methods that accomplish this require $\Omega(N^3)$ time and $\Omega(N^2)$ memory.
...
Given a fixed $p \geq 0$, a filtration $K_\bullet = \{K_i\}_{i\in [N]}$ of size $N = \lvert K \rvert$, and a $R = [i,j] \times [k,l]$ whose indices $(i,j,k,l)$ satisfy $0 \leq i < j \leq k < l \leq N$, the $p$-th multiplicity $\mu_p^{R}$ of $K_\bullet$ is given by:
	...
	\end{equation}
\end{proposition}
\noindent For more geometric intuition of this proposition, see Figure~\ref{fig:mult}. Computationally, the relevance of Propositions~\ref{prop:rank_reduction} and~\ref{prop:mu_reduction} is they imply the complexity of obtaining either $\beta_p^{i,j}(K_\bullet)$ or $\mu_p^{R}(K_\bullet)$ may be reduced to the complexity of computing the rank of a set of submatrices of $\partial$, a fact that actually motivated the rank-based persistence algorithm from Chen et al~\cite{chen2011output}.
%~\ref{eq:measure}

Compared to the classical reduction methods suggested in~\cite{edelsbrunner2022computational, zomorodian2004computing}, the primary advantage of the rank-based expressions from~\eqref{eq:betti_four}-\eqref{eq:mu_four} is that they only depend on the ranks of \emph{unfactored} boundary (sub)matrices. 
Thus, if we restrict the field $\mathbb{F}$ to $\mathbb{R}$, we may use iterative Krylov and subspace acceleration methods for their computation instead of reduction-like procedures. 
%Moreover, the measure-theoretic counter-parts~\eqref{eq:pbn_cont} and~\eqref{eq:measure} suggests we may re-use  
Moreover, the many known algebraic properties of the rank function, such as permutation invariance, suggest there may be means of simplifying these rank-based expressions even further.
...
Extending from the ideas outlined in the previous section, we now turn towards ways of substituting the discontinuous rank function with a continuous approximation, beginning with a spectral characterization. Given a matrix $X \in \mathbb{R}^{n \times m}$ and its singular value decomposition (SVD) $X = U \Sigma V^T$, define the \emph{rank} of $X$ as the composition:
...
For a real, square matrix $A$ of order $n$, the quadratic form $x^T A x$ defines a continuous real-valued function of $x \in \mathbb{R}^n$. It is well known that when $A$ is symmetric, each eigenvalue $\lambda$ satisfying $A v = \lambda v$ is real-valued and each pair of eigenvectors $v, v' \in \mathbb{R}^n$ satisfying $\lambda \neq \lambda'$ is orthogonal.  
...
Moreover, the three-term recurrence from~\eqref{eq:three_term_rec} implies the Lanczos iteration requires just three $O(n)$-sized vectors and a few $O(n)$ vector operations to execute. We summarize these benefits with a proposition. 
...
denote the (weighted) \emph{degree} of $\tau$. Observe setting $f(\sigma) = 1$ for all $\sigma \in K$ recovers the integral notion of degree representing the number of cofacets a given simplex has. 
Now, since $K$ is a simplicial complex, if $\mathrm{co}(\tau) \cap \mathrm{co}(\tau') \neq \emptyset$ then the faces $\tau, \tau'$ not only share a common cofacet $\sigma$, but this cofacet $\{\sigma\} = \mathrm{co}(\tau) \cap \mathrm{co}(\tau')$ is \emph{unique}~\cite{goldberg2002combinatorial}. 
...
Thus, if we fixed the orientation of the simplices of $K^p$, the sign pattern $s_{\tau, \tau'}$ for every $\tau \overset{\sigma}{\sim} \tau'$ can be precomputed and stored ahead of time, reducing the evaluation $s_{\tau, \tau'}$ in $O(1)$ time and $O(m)$ storage. 
Alternatively, if the labels of the $p+1$ simplices $\sigma \in K^{p+1}$ follow a consistent orientation---for example by inducing the orientation of all $[\sigma]  \in K^{p+1}$ via the total order on $V$---then we can remove the storage requirement entirely and simply follow a fixed sign pattern during the computation. 

A subtle but important aspect of algorithmically evaluating~\eqref{eq:l_up_matvec} is the choice of indexing function $h: K^p \to [n]$. This map is necessary to deduce the contributions of the components $x_\ast$ during the operation. While one may use any standard associative array to generate this map, typical implementations that rely on collision-resolution schemes such as open addressing or chaining only have $O(1)$ lookup time in expectation.
...
It is known how to build PMHF's over fixed input sets of size $n$ in $O(n)$ time and $O(n \log m)$ bits~\cite{}. Note that this process happens only once for a fixed simplicial complex $K$: once $h$ has been constructed, it is fixed for every $\mathtt{matvec}$ operation. 
