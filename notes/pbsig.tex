...
In contrast, the second term---the persistence part---typically requires finding a basis in intersection of the two subspaces via either column reductions or projection-based techniques. In general, all of the algorithms outlined in section~\ref{sec:related_work} that accomplish this do so in $O(N^3)$ time and $O(N^2)$ in memory.
...
This is important, as although $R$ is not unique, its non-zero pivots are---these pivots define the persistence diagram. 
...
Given a fixed $p \geq 0$, a filtration $K_\bullet = \{K_i\}_{i\in [N]}$ of size $N = \lvert K \rvert$, and a $R = [i,j] \times [k,l]$ whose indices $(i,j,k,l)$ satisfy $0 \leq i < j \leq k < l \leq N$, the $p$-th multiplicity $\mu_p^{R}$ of $K_\bullet$ is given by:
	...
	\end{equation}
\end{proposition}
\noindent The computational relevance of Propositions~\ref{prop:rank_reduction} and ~\ref{prop:rank_reduction} is that the complexity of computing either $\beta_p^{i,j}(K_\bullet)$ or $\mu_p^{R}(K_\bullet)$ may be reduced to the complexity of computing the rank of a set of submatrices of $\partial$, which was in fact the core observation used by Chen et al~\cite{chen2011output} in their rank-based persistence algorithm---the first \emph{output sensitive} persistence algorithm.
%~\ref{eq:measure}
...
\noindent In conclusion, compared to other classical methods of obtaining $\beta_p^{\ast}(K_\bullet)$ and $\mu_p^\ast(K_\bullet)$, such as those in~\cite{edelsbrunner2022computational, zomorodian2004computing}, the primary advantage the rank-based expressions from~\eqref{eq:betti_four}-\eqref{eq:mu_four} have is that their computations are performed directly on \emph{unfactored} boundary (sub)matrices. 
...
Moreover, suppose we: 
\begin{enumerate}
	\item Restrict ourselves to $H(K; \mathbb{R})$
	\item Index $K_\bullet$ over the (real) upper-half plane $\Delta_+$
	\item Parameterize $C_p(K_\bullet; \mathbb{R})$ with real-valued entries
\end{enumerate}
A natural question arises from this set of assumptions: what connections can be made between the computational expressions~\eqref{eq:betti_four} and~\eqref{eq:mu_four} and their measure-theoretic counter-parts~\eqref{eq:pbn_cont} and~\eqref{eq:measure}?
