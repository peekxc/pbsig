...
Additionally, let $\partial_p : C_p( K_{\bullet}) \to C_{p-1}(K_{\bullet})$ denote the boundary operator sending $p$-chains to their respective boundaries. 
With a slight abuse of notation, we also use $\partial_p$ to also denote the $p$-th filtration boundary matrix with respect to an ordered basis $(\sigma_i)_{1 \leq i \leq N}$ induced by $K_\bullet$:
\begin{equation}\label{eq:boundary_matrix}
	\partial_p[i,j] = \begin{cases}
		(-1)^{s_{ij}} & \sigma_i \in \partial_p[\sigma_j]  \\
		0 & \text{otherwise}
	\end{cases}
\end{equation}
As the matrix representation of the $p$-th boundary operator $\partial_p : C_p( K_{\bullet}) \to C_p(K_{\bullet})$, note that $\partial_p$ is completely characterized by the oriented $p$ and $p-1$ simplices of $K_\bullet$.  
The $p$-th persistent Betti number $\beta_p^{i,j}$ at index $(i,j) \in \Delta_+^N$, where $\Delta_+^N  := \{ \, (i,j) \in [N] \times [N] : i < j \, \}$, is defined as: 
...
Given a fixed $p \geq 0$, a filtration $K_\bullet$ of size $m = \lvert K_\bullet \rvert$, and any pair $(i,j) \in \Delta_+^N$, the persistent Betti number $\beta_p^{i,j}(K_\bullet)$ at $(i,j)$ is given by:
...
Given a fixed $p \geq 0$, a filtration $K_\bullet$ of size $m = \lvert K \rvert$, and a $R = [i,j] \times [k,l]$ whose indices $(i,j,k,l)$ satisfy $0 \leq i < j \leq k < l \leq m$, the $p$-th multiplicity $\mu_p^{R}$ of $K_\bullet$ is given by:
	...
	\end{equation}
\end{proposition}
\noindent In summary, the complexity of computing either $\beta_p^{i,j}(K_\bullet)$ or $\mu_p^{R}(K_\bullet)$ can be reduced to the complexity of computing the rank of a set of ``lower-left'' submatrices of $\partial_p$. We summarize this fact with two corollaries. Let $\mathrm{R}_p(k)$ denotes the complexity of computing the rank of square $k \times k$ matrix with at most $O((p+1)k)$ non-zero $\mathbb{F}$ entries. Then we have:
\begin{corollary}
	Given a filtration $K_\bullet$ of size $N = \lvert K_\bullet \rvert$ and indices $i,j \in \Delta_+^N$, computing $\beta_p^{i,j}$ using expression~\eqref{eq:betti_four} requires $O\big(\max \{\mathrm{R}_{p}(i), \mathrm{R}_{p+1}(j) \} \big)$ time.
	...
	%with $n < m$
\end{corollary} 
\noindent Observe the relation $\partial_{p\+1}^{i \+ 1, j} \subseteq \partial_{p\+1}^{1, j}$ implies the  dominant cost of computing~\eqref{eq:betti_four} lies in computing either $\mathrm{rank}(\partial_p^{1,i})$ or $\mathrm{rank}(\partial_{p+1}^{1,j})$. As a result, the cost of computing $\mu_p^R$ is localized to the pair $(K_i, K_l)$ and thus the complexity of computing the multiplicity $\mu_p^R$ depends only on the number of $(p+1)$-simplices in the interval $[i, l]$. 
\begin{corollary}
	Given a filtration $K_\bullet$ of size $N = \lvert K_\bullet \rvert$ and a rectangle $R = [i,j] \times [k,l]$ with indices $0 \leq i < j \leq k < l \leq N$, computing $\mu_p^{R}$ using expression~\eqref{eq:mu_four} requires $O(\mathrm{R}_{p+1}(l - i))$ time.
	%$ j(p+1) - i$
...
Moreover, if we only concern ourselves with persistence modules with real-valued coefficients, not only do we inherit the measure-theoretic perspective from~\ref{eq:measure}, but we may also replace the non-zero entries of $\partial_p$ with real-valued coefficients. 
...
% Change index set
All of the notation given thus far have used integer indices $(i,j) \in \Delta_+^N$ to describe persistent quantities over a filtration $(K_\bullet, f)$ of size $\lvert K \rvert = N$. Equivalently, we have thus far implicitly assumed the range of the filter function $f : K \to I$ to be the typical index set $I = [N]$.
...
\end{definition}
\noindent
We now show definition~\ref{def:time_boundary_matrix} eases the computation of both counting invariants in parameterized settings. To simplify the notation, we write $A^{x} = A^{\ast,x}$ for the setting where only columns up to $x$ of $A$ are being selected, and let $q = p + 1$. 
Additionally, for simplicity of presentation, we always assume the orientation of the simplices induced by the linear extension $(K, \preceq^\ast)$ on $K$ is inherited from the total order on $V$. 
...
In summary, by restricting ourselves to homology defined over real-valued coefficients, we may parameterize the non-zero entries of the $p$-th boundary matrix $\partial_p$ from~\ref{eq:boundary_matrix} directly using $f : K \times \mathcal{H} \to \mathbb{R}$. Moreover, since $\mathrm{rank}(A) = \mathrm{rank}(P^T A P)$ for any permutation matrix $P$, it is clear that expressions of the counting invariants from Propositions~\ref{prop:rank_reduction} and~\ref{prop:mu_reduction} 
yield equivalent values to the parameterized expressions in~\eqref{eq:pbn_parameterized} and~\eqref{eq:mu_parameterized}. 
...
Namely, if $(K, f_h)$ is an $\mathcal{H}$-parameterized filtration where $K$ is a fixed simplicial complex and $f_h : K \to \mathbb{R}$ is a filter function, one may easily construct examples where $\lVert \mu_h^R - \mu_{h + \delta}^R \rVert \sim O(\lvert K_p \rvert )$ for some arbitrarily small perturbation $\delta > 0$. 
Intuitively, since both counting invariants are $0$ outside of the portions of $\Delta_+$ they restrict  too, it's always possible to encounter such situations where small changes in the input affect the corresponding invariant in a non-Lipshitz way. In this section, we introduce a smooth relaxation to both invariants to counter this instability. 
...
The idea is, given a positive approximation parameter $\epsilon > 0$, we approximate the rank via:
...
\end{flalign}
%\end{equation}
where $\phi: \mathbb{R}_+ \times \mathbb{R}_{++} \to \mathbb{R}_+$ is an $\epsilon$-parameterized family of continuous $\mathrm{sgn}$ approximation functions. 
Following the seminal work done by Chen et al.~\cite{chen1996class}, Bi et al.~\cite{bi2013approximation} give a framework for constructing such functions, each of which when composed with the singular value function yield a continuous function that approximates the rank function to any pre-prescribed accuracy.
The form of these $\phi$ functions is obtained by integration of smoothed variations $\hat{\delta}$ of the Dirac delta function $\delta$:  
...
\end{equation}
where $\Phi_\epsilon^h(\partial) = (\Phi_\epsilon \circ \partial)(h)$ for any $h \in \mathcal{H}$, and $\tilde{\partial}_p$ is $\mathcal{H}$-parameterized $p$-th boundary matrix from~\ref{def:time_boundary_matrix} with step functions $S_\ast$ replaced by the smoothstep $\mathcal{S}_\ast^\omega$ from~\eqref{eq:smoothstep}, for some fixed $\omega > 0$.
\end{definition}
...
\section{Computing }\label{sec:methodology}
...
\State $y \gets \mathbf{0}$ \Comment{$(\in \mathbb{R}^n)$}
