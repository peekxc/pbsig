...
The $p$-th persistent Betti number $\beta_p^{i,j}$ at index $(i,j) \in \Delta_+^m$ is defined as: 
...
One such property is permutation invariance: given any $A \in \mathbb{R}^{n \times n}$, it is well known that $\mathrm{rank}(A) = \mathrm{rank}(P^T A P)$ for any permutation matrix $P$. Though  the boundary matrices $\partial_p$ in~\eqref{eq:betti_four} are given in filtration order, the permutation invariance of the rank function suggests they need not be to be evaluated---so long as the constitutive matrices have the correct non-zero pattern, the Betti number must be identical.
...
In what follows, in the spirit of~\eqref{eq:pbn_cont}, we alter our notation by re-defining the PBNs $\beta_p^{i,j}$ using pairs $(i,j) \in \Delta_{+}$ from the upper-half plane $\Delta_{+}$.
...
The formulation of the boundary matrix given in~also lends to a structured interpretation with the Laplacian. Consider two kinds of weighted graph Laplacians, one $L^V$ defined with respect to a vertex function $f_v : V \to \mathbb{R}$ and the other defined with respect to an edge function $f_E : E \to \mathbb{R}$. In the former case, we have the typical weighted Laplacian matrix-vector intereptetaioon: 
\begin{equation}
	(L^E x)_i = \mathrm{deg}(v_i) \cdot x_i \cdot d^w(v_i) - \sum\limits_{i \sim j} x_j 
\end{equation}
where $d^w(v_i) = \sum\limits_{}$...
