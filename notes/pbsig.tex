...
\begin{equation}\label{eq:param_boundary_matrix}
	...
	s_{kl} \cdot (S_{i} \circ f_h)(\sigma_k) \cdot (\tilde{S}_{j} \circ f_h)(\sigma_l) & \text{if } \sigma_k \in \partial_p(\sigma_l)\\
...
where $S_{i} : \mathbb{R} \to \{0, 1\}$ is a \emph{step} function satisfying $S_i(x) = 0$ if $x \leq i$ and $1$ otherwise, $\tilde{S}_{i} = 1 - S_i$, $f_h(\sigma) = f(\sigma, h)$, and $s_{kl} = \mathrm{sgn}([\sigma_k], \partial [\sigma_l])$ is the sign of the oriented face $[\sigma_k]$ in the boundary of its oriented coface $[\sigma_l]$.
\end{definition}
\noindent 
...
\end{equation}
where $W_p^x(h)$ denote an $\mathcal{H}$-parameterized diagonal ``weight'' matrix $\mathrm{diag}(\{\, g_h(\sigma_1), g_h(\sigma_2), \dots, g_h(\sigma_N) \, \})$ obtained by setting $g_h(\sigma) = (S_x \circ f_h)(\sigma)$ if $\mathrm{dim}(\sigma) = p$ for all $\sigma \in K$ and $g_h(\sigma) = 0$ otherwise ($\widetilde{W}_p^x$ is defined synonymous with $\tilde{g}_h(\sigma) = 1 - (S_x \circ f_h)(\sigma)$). Thus, definition~\ref{def:time_boundary_matrix} corresponds to a \emph{fixed} $p$-th boundary matrix $\partial_p$ whose rows and columns are weighted by $\widetilde{W}_p^i(h)$ and $W_{p+1}^j(h)$, respectively. As a result, there is no need to explicitly maintain an ordered basis $(\sigma_i)_{1 \leq i \leq N}$ of simplices as a function of $\mathcal{H}$ to obtain persistence-related information, as is done by the vineyards algorithm~\cite{cohen2006vines}---it is enough to simply preserve the correct sign pattern.
...
Bi et al.~\cite{bi2013approximation} give a framework for constructing smooth approximations  $\phi \approx \mathrm{sgn}_+$, including an analysis of their properties when composed with the singular value function via~\eqref{eq:rank_sgn}.
...
The idea of their approach is to approximate the $\mathrm{sgn}_+$ function by integrating smoothed variations $\hat{\delta}$ of the Dirac delta function $\delta$:  
...
In contrast to the $\mathrm{sgn}_+$ function, it was shown in~\cite{bi2013approximation} that if $p$ is continuous on $\mathbb{R}_+$, then $\phi(\cdot, \epsilon)$ is continuously differentiable in $\mathbb{R}_+$, and if $p$ is bounded above on $\mathbb{R}_+$, then $\phi(\cdot, \epsilon)$ is globally Lipshitz continuous on $\mathbb{R}_+$. 
Moreover, by replacing $\mathrm{sgn}_+ \mapsto \phi$ in~\eqref{eq:rank_sgn}, one may extend several of these properties to the rank function. 
In particular, suppose a given $X \in \mathbb{R}^{n \times m}$ satisfying $n \leq m$ admits a singular value decomposition $X = U\Sigma V^T$ and is paired with a scalar-valued function $\phi: \mathbb{R}_+ \times \mathbb{R}_{++}  \to \mathbb{R}$ that is continuously differentiable at every $\sigma \in \Sigma$. 
Then for some $\epsilon > 0$, the corresponding \emph{Löwner} operator $\Phi_\epsilon : \mathbb{R}^{n \times m} \to \mathbb{R}^{n \times m}$ defined by: 
...
In fact, if $\phi$ is twice-differentiable at each $\sigma_i(X)$ for all $i = 1, \dots, n$, then the operator~\eqref{eq:lowner} is twice continuously differentiable at $X$~\cite{}. 
We summarize this with a definition:
...
%there are a large variety of Laplacian operators, including e.g. the graph Laplacian, the combinatorial Laplacian, the edge (or vertex) weighted Laplacian, the normalized graph laplacian.
