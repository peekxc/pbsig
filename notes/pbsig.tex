...
These diagram vectorizations have proven useful for learning applications due to their stability and configurable metric structure~\cite{}, however, their scalability is limited as diagrams are prerequisite to their computation.
...
The scalability problem remains exacerbated, however, as extending the standard persistence computation to parameterized settings has proven non-trivial~\cite{}. %moves
Indeed, achieving rotation invariance in~\eqref{eq:pht1} involves a quadratic number of diagram comparisons. 
 
We seek to shift the computational paradigm: rather than constructing and vectorizing diagrams in lockstep, we propose a stable vectorization using the duality between diagrams and their rank functions to performs both simultaneously. 
...
%	\simeq H() 
\end{equation}
As above, all three operators $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are symmetric, positive semidefinite, and compact. When $p = 0$, we recover $\Delta_0(K) = \partial_1 \partial_1^T = L$ since $L_0^{\text{dn}} = 0$.
