...
One disadvantage in working with rank-type functions restricted to portions of real-plane is that they are integer-valued and not smooth. 
...
Relaxing $\mu_p^R \mapsto \hat{\mu}_p^R$ fixes this instability when $\epsilon > 0$, though the Lipshitz constant may be arbitrarily high. In what follows, we investigate the effect of $\epsilon$ has on the smoothness of $f_\alpha$ and towards stabilizing the constitutive terms in $\hat{\mu}_p^\ast$ and $\hat{\beta}_p^{\ast}$. 

Consider a filtration $(K, f)$ indexed over the real line $f : K \to \mathbb{R}$ whose filter function $f$ is perturbed by some positive amount $\delta > 0$. We would like to show stability of $\hat{\mu}_{p,\epsilon}^R$ with respect to small perturbations to entries in $\hat{\partial}_p$.
% Lipshitz stability 
% partition of unity + output-sensitive mention.
To be more precise, we require a more specific choice of $\phi$ in ~\eqref{eq:phi}. For illustrative purposes\footnote{A similar expression may be derived for other choices of $\nu(\epsilon)$ and $p(x)$.}, 
observe that setting $\nu(\epsilon) = \sqrt{\epsilon}$ and $p(x) = 2x (x^2 + 1)^{-2}$ the composition of the singular value function with~\eqref{def:cont_rank} reduces to the rank approximation studied in~\cite{},  given explicitly by: 
\begin{equation*}
...
That is, for $M = X X^T$ a given positive definite matrix, its \emph{condition number} $\kappa(M)$ is defined as: 
\begin{equation}
...
\end{equation}
Thus, the effect of adding $\epsilon I_n$ to a given matrix can be interpreted as a means of reducing $\kappa$ arbitrarily---at the expense of accuracy---to stabilize the pseudo-inverse. This, in turn, yields the following proposition: 
\begin{proposition}
...
One shortcoming of the stability result shown above is that the Lipshitz constant depends explicitly on the magnitude of $f_\alpha$, which in-turn affects the stability of the proposed relaxation. We address this issue in the following section.  
...
Moreover, since $L = \partial_1 \partial_1^T$ is the well studied \emph{graph Laplacian}, we may consider the study of spectra of \emph{combinatorial Laplacians}---generalizations of the graph Laplacian---as the study of singular values of boundary operators. 
%Towards revealing  
...
Unlike typical hash table implementations which rely on collision-resolution schemes like open addressing and chaining (and have $O(n)$ worst-case complexity), it is known how to build PMHF's over fixed input sets fo size $n$ in $O(n)$ time and $O(n \log m)$ bits~\cite{}. Note that this process happens only once for a fixed simplicial complex $K$: once $h$ has been built, it is fixed for every $\mathtt{matvec}$ operation. 
