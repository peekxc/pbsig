...
%	\mathrm{rank}(\, \partial_p^{i,j}(K_\bullet) \, ) = 
	\mathrm{rank}(\, \partial_p^{i,j}(K, f_\alpha) \, ) \Leftrightarrow \mathrm{rank}\!\left( \, D_p(\bar{S}_i \circ f_\alpha) \circ \partial_p(K_{\preceq^\ast}) \circ D_{p+1}(S_j \circ f_\alpha) \, \right)
\end{equation}
...
As a result, expressions of persistent rank invariants obtained by replacing the constitutive terms in~\eqref{eq:betti_four} and~\eqref{eq:mu_four} using their parameterized versions from definition~\ref{eq:rank_equiv_param} are permutation invariant. 
\end{remark}
...
%\end{align*}
\noindent It may be shown that $\Phi_\epsilon$ is a continuously differentiable operator in $\mathbb{R}^{n \times m}$ for any $\epsilon > 0$ and is twice continuously differentiable at $X$ if $\phi$ is twice-differentiable at each $\sigma_i(X)$ for all $i = 1, \dots, n$~\cite{ding2018spectral}.  
...
%		\item If $p$ is bounded above on $\mathbb{R}_+$, then $\phi(\cdot, \epsilon)$ is globally Lipshitz continuous on $\mathbb{R}_+$
	\end{enumerate}
\end{proposition}
\noindent Note that $\lVert\Phi_\epsilon(X)\rVert_\ast$ is not necessarily differentiable on $\mathbb{R}^{n \times m}$ due to Proposition 2.2(e) in~\cite{bi2013approximation}, but it is differentiable on the positive semi-definite cone $\mathbb{S}^n_+$, which is sufficient for our purposes here. 
%\begin{remark}
...
$$ \lVert \Phi(\partial_p^{i,j}) \rVert_\ast : \Delta_+ \to \mathbb{R}$$
...
\begin{lemma}
	For any $p \geq 0$ and simplicial pair $(K, f)$, if there exists an indexing function $h: K^p \to [n]$ with $O(k)$ access time and $O(c)$ storage, then there exists a two-phase algorithm for computing the inner product $x \mapsto \langle \; L_p, x	\; \rangle$ in $O(mk(p+1))$ time and $O(\mathrm{max}(c,m))$ storage, where $n = \lvert K^p \rvert$ and $m = \lvert K^{p+1} \rvert$. 
...
Combining Lemma~\ref{} with proposition~\ref{lemma:exact_arith_lanczos}, 
\begin{proposition}\label{prop:spectral_rank}
	For any $p \geq 0$, given a rank-$r$ combinatorial Laplacian operator $L: \mathbb{R}^n \to \mathbb{R}^n$ derived from a simplicial complex $K$ with $n = \lvert K^p \rvert$ and $m = \lvert K^{p+1} \rvert$,  
	
	whose matrix-vector operator $A \mapsto A x$ requires $O(\eta)$ time and $O(\nu)$ space, the Lanczos iteration computes $\Lambda(A) = \{ \lambda_1, \lambda_2, \dots, \lambda_r \}$ in $O(\max\{\eta, n\}\cdot r)$ time and $O(\max\{\nu, n\})$ space, when computation is done in exact arithmetic. 
\end{proposition}
