...
On of the other hand, computationally, the extending matching distance $d(\beta_f, \beta_{f'})$ requires finding the optimal matching $\phi$ between the points $p \in \Lambda_+$ with non-zero multiplicity, a quantity which effectively requires obtaining the persistence diagrams to compute. Since both (a) computing the full persistence diagram and (b) obtaining the optimal matching are both  high complexity algorithms, we would like to avoid them while still retaining some information contained in them, if possible. 
We tackled (a) by fixing specific \emph{regions} of $\Lambda_+$ a-priori. We now focus on tackling (b). 

One disadvantage in working with counting/multiplicity functions restricted to portions of real-plane is that they are not smooth. 
...
\end{equation}
Additionally, in the context of the Rank Minimization Problem (RMP), the nuclear norm forms a convex envelope of the rank function in the space of all $n \times n$ matrices on the unit-ball in the operator norm~\cite{}. 

In the spirit of~\cite{}, we consider a family of continuous rank approximation functions $\phi: \mathbb{R}_+ \times \mathbb{R}_{++} \to \mathbb{R}_+$, each of which are functions of the singular value function and some positive relaxation parameter $\epsilon > 0$. 

Let $\delta(u, \epsilon) = \nu(\epsilon)^{\text{-}1} \rho \left ( u \nu(\epsilon)^{\text{-}1} \right )$ for

The continuity of these relaxation is studied extensively in~\cite{}, wherein a plethora of results were shown. We summarize the main points: 
\begin{enumerate}
...
\end{equation}
The cylic property of the trace opertor implies...
