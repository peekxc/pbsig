...
multiset summary of $f$ formed by pairs $(a,b) \in \mathbb{R}^2$ exhibiting non-zero \emph{multiplicity} $\mu_p^{a,b}$:
...
Tackling the issue of injectivity, Turner et al. propose an injective shape statistic of directional diagrams associated to a data set $X \subset \mathbb{R}^d$,  
...
sparking both an inverse theory for persistence and a mathematical foundation for metric learning~\cite{}.  
...
The following results summarize some technical observations motivating this effort. They are not necessary to understand the general approach, though they will be used in several proofs. Collectively, these observations suggest the rank invariant computation admits several advantages in the vector space setting. 
...
\end{corollary}
\noindent Indeed, though $R$ is not unique, its non-zero pivots are, and these pivots \emph{define} the persistence diagram. Thus,~\eqref{eq:lower_left_rank} in turn enables both $\beta_p^{\ast}$ and $\mu_p^{\ast}$ to be written as a sum of ranks of submatrices of $\partial_p$ and $\partial_{p+1}$:
...
the persistent Betti number and multiplicity functions may be written as:
	...
	% \mathrm{rank}(I_p^{1,i})
	\beta_p^{a_i,a_j}(K,f) &= \lvert K_i^p \rvert - \mathrm{rank}(\partial_p^{1,i}) - \mathrm{rank}(\partial_{p\+1 }^{1,j}) + \mathrm{rank}(\partial_{p\+1}^{i \+ 1, j} ) \numberthis  \\
...
Observe both~\eqref{eq:betti_four} and~\eqref{eq:mu_four} are (1) comprised strictly of \emph{rank} computations, and (2) involve \emph{unfactored} boundary (sub)matrices.
...
The former suggests relaxations of the rank function---such as those used often in $\ell_1$ minimization and compressive sensing~\cite{zhao2012approximation} applications---may be extended in the persistence setting through these expressions. The latter suggests these quantities may be iteratively approximated in a matrix-free fashion using e.g. randomized approaches~\cite{chen2011output} or iterative subspace methods~\cite{golub2013matrix, parlett1994we}, as opposed to  direct methods that decompose $\partial$ (i.e.~\cite{edelsbrunner2022computational, zomorodian2004computing}).
...
Moreover, the structure of $\partial$ and the spectral characterization of the rank function
...
suggest tools from spectral theory, such as those developed for ``The Laplacian Paradigm''~\cite{}, may be exploited to accelerate the computation further. 
...
%\end{equation}
Computing $\beta_p^{a,b}$ via~\eqref{eq:pbn_simple} reduces to two rank computations: one nullity computation and one computation that typically requires finding a basis in intersection of the two subspaces.
% via matrix reduction or projector-based techniques 
While the former is easily solved, in general, direct methods accomplishing the latter rely on reduction or expensive projector-based techniques that require $\Omega(N^3)$ time and $\Omega(N^2)$ space~\cite{golub2013matrix}.
These methods have proven challenging to efficiently extend to the parameterized setting~\cite{piekenbrock2021move}, prompting questions of whether alternative expressions (e.g.~\eqref{eq:mu_four}) have any potential computational benefits.
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), the boundary matrix $\partial$ must maintain a total order with respect to $f_\alpha$, which results in a frequent need to permute matrices.  
...
Observe that (1) the non-zero entries from~\eqref{eq:param_boundary_matrix} vary continuously in $f_\alpha$ and (2) $\partial_p(\alpha)$ decouples into a product of diagonal matrices $D_\ast(f_\alpha)$: 
	...
	\end{equation}
	where $D_p(f_\alpha)$ and  $D_{p+1}(f_\alpha)$ are diagonal matrices whose non-zero entries are ordered by $\preceq$ based on restrictions of $f_\alpha$ to $K^p$ and $K^{p+1}$, respectively.
We refer to the fixed inner matrix $\partial_p \in \{-1, 0, 1\}^{n \times m}$ as the \emph{sign pattern matrix}.
Exploiting this decoupling, note that we may define a matrix with rank matching the rank of the matrices $\partial_p^{i,j}(K, f_\alpha)$ from Lemma~\ref{lemma:rank} via matrices of the form:
...
%	\mathrm{rank}(\, \partial_p^{i,j}(K_\bullet) \, ) = 
	...
	\end{equation}
where $\bar{S}, S : \mathbb{R} \to \{0,1\}$ are up and down step functions, respectively, of the form $\bar{S}_a(x) = \mathbbm{1}_{x > a}(x)$ and $S_b(x) = \mathbbm{1}_{x \leq b}(x)$.
...
%\end{equation}
Moreover, since these step functions are non-continuous, we may retain the element-wise continuity of~\eqref{eq:decouple} by exchanging them with clamped \emph{smoothstep} functions $\mathcal{S}: \mathbb{R} \to [0, 1]$ of the form: 
...
\begin{proposition}\label{prop:mu_betti_1}
	...
	\begin{align*}\label{eq:pbn_parameterized}
		\beta_p^{a,b}(\alpha) &\triangleq \lvert K_a^{p}(\alpha) \rvert - \mathrm{rank}\big(\,\hat{\partial}_{p}^{a}(\alpha)\,\big) - \mathrm{rank}\big(\,\hat{\partial}_{p+1}^{b}(\alpha)\,\big) + \mathrm{rank}\big(\,\hat{\partial}_{p+1}^{a \+ \delta, b}(\alpha)\,\big) \numberthis \\
%	\end{align*}
%	\vspace{-2.1em}
%	\begin{align*}
...
The results from the previous section lay the groundwork for continuous relaxations of $\mu_p^\ast$/$\beta_p^\ast$, as the only noncontinuous function in $\mu_p^\ast(\alpha)$/$\beta_p^\ast(\alpha)$ is the rank function. 
...
In this section, we discuss several of the computational advantages that arise by combining the results of section~\ref{sec:spectral_sec}.
