...
Towards ameliorating some of these issues, practitioners have equipped diagrams with additional structure by way of maps to function spaces---examples include persistence images~\cite{adams2017persistence}, persistence landscapes~\cite{bubenik2015statistical}, and template functions~\cite{perea2022approximating}. 
%Along a separate thread of research 
Tackling the issue of injectivity, Turner et al. propose the \emph{persistent homology transform} (PHT), which is a shape statistic that associates an embedded data set $X \subset \mathbb{R}^d$ with a collection of diagrams
...
sufficient for reconstruction, sparking both an inverse theory for persistence and a mathematical foundation for metric learning.  
Despite the usefulness of these extensions in learning applications,
%~\cite{pun2018persistent},
%however, their scalability is limited as diagrams are prerequisite to their computation.
individual diagrams can still be expensive to obtain, and the parameterized setting only compounds the scalability issue. Indeed, generalizing the persistence computation to dynamic settings has proven non-trivial~\cite{piekenbrock2021move}.
...
%	$\mu_p(f_\alpha; \mathcal{R})$ for all $\alpha \in \mathcal{A}$. 
	...
	\mu_p^{\mathcal{R}}(K, f_\alpha) \triangleq \big\{ \, 
...
%\end{enumerate}
%\noindent
Applications looking to vectorized persistence information over random and highly structured complexes may benefit from the concentration of mass phenomenon known to occur their spectra; examples include topology-guided image denoising~\cite{pun2018persistent}, detecting bifurcations in dynamical systems~\cite{perea2022approximating}, generating metric invariants for shape classification~\cite{chazal2009gromov}, and so on. 
The differentiability of our relaxation also prompts immediate questions of immediate applications seeking to optimize persistence information, such as... and 
...
\begin{remark}
% Change index set
The notation used thus far employed integer indices $(\,i, j\,) \in \Delta_+^N$ to describe persistent quantities over a filtration $K_\bullet = (K, f)$ of size $\lvert K \rvert = N$, which is equivalent to filtering $K$ over the index set $f : K \to [N]$.
...
\begin{proposition}\label{prop:mu_betti_1}
	Given $(K, f_\alpha)$, any rectangle $R = [i,j] \times [k,l] \subset \Delta_{+}$, and $\delta > 0$ the number satisfying $i + \delta < j - \delta$ from~\eqref{eq:measure}
	%satisfying $i < j \leq k < l$, 
	the $\mathcal{A}$-parameterized invariants $\beta_p^{i,j} : \mathcal{A} \times K \to \mathbb{N}$ and $\mu_p^{R} : \mathcal{A} \times K \to \mathbb{N}$ defined by: 
	\begin{align*}\label{eq:pbn_parameterized}
		\beta_p^{i,j}(\alpha) &\triangleq \lvert K_i^{p}(\alpha) \rvert - \mathrm{rank}\big(\,\hat{\partial}_{p}^{i}(\alpha)\,\big) - \mathrm{rank}\big(\,\hat{\partial}_{p+1}^{j}(\alpha)\,\big) + \mathrm{rank}\big(\,\hat{\partial}_{p+1}^{i \+ \delta, j}(\alpha)\,\big) \numberthis \\
%	\end{align*}
%	\vspace{-2.1em}
%	\begin{align*}
	\mu_p^{R}(\alpha) &\triangleq \mathrm{rank}\big(\, \hat{\partial}_{p+1}^{j \+ \delta, k}(\alpha)\,\big) - \mathrm{rank}\big(\, \hat{\partial}_{p+1}^{i \+ \delta, k}(\alpha)\,\big) - \mathrm{rank}\big(\, \hat{\partial}_{p+1}^{j \+ \delta, l}(\alpha)\,\big) + \mathrm{rank}\big(\, \hat{\partial}_{p+1}^{i \+ \delta, l}(\alpha)\,\big) \numberthis \label{eq:mu_parameterized}
	\end{align*}
	yield the correct quantities $\mu_p^R(K, f_\alpha) = \mathrm{card} \left( \, \restr{\mathrm{dgm}_p(f_\alpha)}{R} \, \right)$ and $\beta_p^{i,j} = \mathrm{dim}(H_p^{i,j}(K, f_\alpha))$ for all $\alpha \in \mathcal{A}$. 
...
\begin{remark}
	In~\eqref{eq:rank_equiv_param}, we write $\partial_p(K_{\preceq^\ast})$ (as opposed to $\partial_p(K_\bullet)$ or $\partial_p(K, f)$) to emphasize $\partial_p(K_{\preceq^\ast})$ is ordered  according to a fixed linear ordering $(K, \preceq^\ast)$. Note that evaluating the boundary terms from corollary~\ref{cor:rank_reduction} would require $\partial$ to be explicitly filtered in the simplexwise ordering induced by $f_\ast$---which varies in $\mathcal{A}$. 
In contrast, replacing the constitutive terms in~\eqref{eq:betti_four}/\eqref{eq:mu_four} with~\eqref{eq:pbn_parameterized}/\eqref{eq:mu_parameterized} yields equivalent persistent rank invariant expressions that are permutation invariant and thus no longer need to maintain a simplexwise ordering. 
\end{remark}
...
The results from the previous section lay the groundwork for continuous relaxations of $\mu_p^\ast$/$\beta_p^\ast$, as the only noncontinuous aspect of $\mu_p^\ast(\alpha)$/$\beta_p^\ast(\alpha)$ is the rank function. 
To address this, we exploit the spectral characterization of the rank function: given a matrix $X \in \mathbb{R}^{n \times m}$, the \emph{rank} of $X$ is given by the composition:
...
\begin{equation}\label{eq:rank_def}
	...
	\end{cases}
\end{equation}
where $\{\sigma_i \}_{i=1}^n$ are the singular values $\Sigma = \mathrm{diag}(\{\sigma_i \}_{i=1}^n)$ from $X$'s singular value decomposition (SVD) $X = U \Sigma V^T$, and $\mathrm{sgn}_+: \mathbb{R} \to \{0, 1\}$ is the one-sided sign function. 
...
Building off the seminal work from~\cite{mangasarian1994class}, Bi et al.~\cite{bi2013approximation} propose replacing the $\mathrm{sgn}_+$ function with integrated smoothed variations $\hat{\delta}$ of the Dirac delta function $\delta$:  
...
replacing the rank function in all of the constitutive terms of both $\mu_p^\ast$ and $\beta_p^{\ast}$ from proposition~\ref{prop:mu_betti_1} yields Lipshitz continuous functions whenever the filter function $f_\alpha$ is itself Lipshitz. 
\begin{remark}\label{remark:diff}
	While $\Phi_\tau$ is a continuously differentiable operator in $\mathbb{R}^{n \times m}$ for any $\tau > 0$ and it in fact may be shown  to be twice continuously differentiable at $X$ if $\phi$ is twice-differentiable at each $\sigma_i(X)$ (see~\cite{ding2018spectral}), % for all $i = 1, \dots, n$, 
	note that $\lVert\Phi_\tau(X)\rVert_\ast$ is not necessarily differentiable on $\mathbb{R}^{n \times m}$ due to Proposition 2.2(e) in~\cite{bi2013approximation}. However, $\lVert\Phi_\tau(X)\rVert_\ast$ is differentiable on the positive semi-definite cone $\mathbb{S}^n_+$, which we exploit in the following section. 
\end{remark}
...
Without loss of generality, we now describe an $\mathcal{A}$-parameterized variation of $L_p^{\mathrm{up}}$. Following~\cite{}, we write $X^+$ to denote the Moore-Penrose pseudoinverse and for brevity write $X^{+/2} = (X^{+})^{1/2}$. Using the decoupling observation from~\eqref{eq:decouple}, observe we may write the inner operator from~\cite{} as follows:
...
$$ \lim_{\tau \to 0^+ }\hat{\mu}_{p,\tau}^R = \mu_p^R(K,f), \quad \quad \lim_{\tau \to 0^+ } \hat{\beta}_{p,\tau}^{i,j} = \beta_{p}^{i,j}(K,f)$$
\end{proposition}
\begin{proof}
...
\begin{equation}\label{eq:krylov_proj_}
...
The relation from~\eqref{eq:krylov_proj_}---often called the \emph{three-term recurrence}---characterizes the power of the Lanczos method: if ($q_{j\text{-}1}, \beta_j, q_j$) are known, then ($\alpha_j$, $\beta_{j+1}, q_{j+1}$) are completely determined, prompting an iterative strategy for building $T_j$. 
...
%since $A$ is not modified during the computation,
%the iteration may be executed without explicitly storing $A$ in memory. 
Indeed, the three-term recurrence from~\eqref{eq:krylov_proj_} implies each iteration requires just three $O(n)$-sized vectors and a few $O(n)$ vector operations, motivating the following result:
