...
The former suggests spectral relaxations of the rank function may find utility in the persistence setting, such as those in compressive sensing~\cite{zhao2012approximation}. The latter suggests both quantities may admit \emph{matrix-free} iterative approximation schemes using randomized algorithms~\cite{chen2011output} or iterative subspace methods~\cite{golub2013matrix, parlett1994we}. 
...
In this section, we introduce the proposed relaxation successively by relaxing or generalizing different aspects of the expressions from Corollary~\ref{cor:rank_reduction} to the parameterized setting. To motivate why these expression are important, it is instructive to derive the more traditional expressions used to compute the persistent rank invariants. 
