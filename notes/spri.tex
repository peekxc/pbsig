...
Notably, our approach not only avoids explicitly constructing diagrams, but it entirely \emph{matrix-free}, circumventing the reduction algorithm from~\cite{edelsbrunner2022computational} entirely.  
...
%and $\Phi$ determines how this information is incorporated into the mapping. 
	\item Denote by $\Lambda(\mathcal{L}_{a,b}(\alpha))$ the spectrum of $\mathcal{L}_{a,b}$ at any $\alpha \in \mathcal{A}$ ordered non-decreasingly. For each corner point $(a,b)$ in the boundary of $\mathcal{R}$ and eigenvalue $\lambda$ of $\mathcal{L}$ 
...
Now, consider computing $\beta_p^{a,b}$ via~\eqref{eq:pbn_simple} from matrix representatives $\partial_p \in \mathbb{F}^{n \times m}$.
...
There is an alternative approach to computing~\eqref{eq:pbn_simple} that exploits the structure of persistence. In particular, the structure theorem~\cite{zomorodian2004computing} shows 1-parameter persistence modules decompose in an \emph{essentially unique} way---computationally, 
...
Given a fixed $p \geq 0$, a filtration $(K,f)$ of size $N = \lvert K \rvert$ with $f: K \to [N]$ and a rectangle $R =  [i,j] \times [k,l]$ satisfying $1 \leq i < j \leq k < l \leq N$, the persistent Betti number $\beta_p^{i,j}$ at $(i,j)$ and multiplicity $\mu_p^{R}$ of $R$ is given by:
	\begin{align*}\label{eq:betti_four}
	% \mathrm{rank}(I_p^{1,i})
	\beta_p^{i,j}(K,f) &= \lvert K_i^p \rvert - \mathrm{rank}(\partial_p^{1,i}) - \mathrm{rank}(\partial_{p\+1 }^{1,j}) + \mathrm{rank}(\partial_{p\+1}^{i \+ 1, j} ) \numberthis  \\
...
\end{corollary}
%Dey \& Wang
\noindent These rank-based expressions seem neither well known nor widely used---to the authors knowledge, the expression for $\mu_p^{R}(K,f)$ from~\eqref{eq:mu_four} was first pointed out by Chen \& Kerber~\cite{chen2011output}. A more recent and explicit derivation of~\eqref{eq:betti_four} is given by Dey \& Wang~\cite{dey2022computational}. For completeness, we give our own detailed proof of corollary~\ref{cor:rank_reduction} in the appendix. 
...
The primary technical observation motivating this effort is that these expressions suggest several advantages when specialized to the vector space setting. Indeed, as the expressions from Corollary~\ref{cor:rank_reduction} are well-defined for any totally ordered filter function, it is natural to consider their generalizations under the \emph{persistent measures} setting~\cite{chazal2016structure}.
Indeed, observe the expressions from Corollary~\ref{cor:rank_reduction} are (1) comprised strictly of \emph{rank} computations, and (2) involve \emph{unfactored} boundary (sub)matrices.
...
For example, the former observation suggest relaxations of the rank function---such as those used often in compressive sensing~\cite{zhao2012approximation}---can now readily be applied in the persistence setting. The latter observation suggests such relaxations may be computed without decomposing $\partial$ using direct methods (i.e. the reduction algorithms from~\cite{edelsbrunner2022computational, zomorodian2004computing} via e.g. iterative Krylov or subspace acceleration methods~\cite{golub2013matrix, parlett1994we}.
...
suggest tools from spectral theory, such as those developed for ``The Laplacian Paradigm''~\cite{}, may be exploited during the computation. 
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), the boundary matrix $\partial$ must maintain a simplexwise filtration order (w.r.t $f_\alpha$) to preserve the inclusion structure of $(K, f_\alpha)$.
