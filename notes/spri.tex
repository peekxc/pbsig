...
multiset summary of $f$ formed by pairing homological critical values $\{ a_i \}_{i=1}^N$ with \emph{multiplicities} $\mu_p^{i,j}$~\cite{cohen2005stability}:
%defined over the integer upper halfplane $\Delta_+^N \triangleq \{\, (i,j) : 1 \leq i \leq j \leq n \, \}$: 
\begin{equation}\label{eq:dgm}
\mathrm{dgm}_p(K, f) \triangleq \{ \, (a_i, a_j) :  \mu_p^{i,j} \neq 0  \, \}  
%\cup \Delta_+^{N}
%,\quad \quad \Delta_+^{\lvert K \rvert} \triangleq \{\, (i,j) : 1 \leq i \leq j \leq N \, \}
%i \leq j
% \quad \quad \mu_p^{i,j} \triangleq \left(\beta_p^{i,j\shortminus1} - \beta_p^{i,j} \right) - \left(\beta_p^{i\shortminus1,j\shortminus1} - \beta_p^{i\shortminus1,j} \right)
...
Though these extensions have proven useful in learning applications,
%~\cite{pun2018persistent},
%however, their scalability is limited as diagrams are prerequisite to their computation.
individual diagrams can still be quite difficult to obtain, and generalizing their computation to parameterized settings has proven non-trivial~\cite{piekenbrock2021move}.
...
Moreover, our proposed relaxation is \emph{matrix-free}, has a space complexity that is linear in $\lvert K \rvert$, 
%requiring only as much memory as is needed to enumerate simplices in the underlying complex $K$, 
can be iteratively approximated, is continuously differentiable, and is particularly efficient to compute over parameterized families of inputs. 
...
%	$\mu_p(f_\alpha; \mathcal{R})$ for all $\alpha \in \mathcal{A}$. 
	...
	\item Choose a Laplacian operator $\mathcal{L}: C^p(K, \mathbb{R}) \to C^p(K, \mathbb{R})$ and $\tau$-parameterized spectral function $\phi: \mathbb{R} \to \mathbb{R}$ to associate to $\mathcal{R}$.
...
The choice of $\mathcal{L}$ determines the kind of geometric/topological information to extract from $(K, f_\alpha)$, and the choice of $\phi$ determines how that information is encoded.
...
In contrast, the persistence term typically requires finding a basis in intersection of the two subspaces via matrix reduction or projector-based techniques. In general, direct methods accomplishing this require $\Omega(N^3)$ time and $\Omega(N^2)$ space~\cite{golub2013matrix}.
...
There is an alternative approach to computing $\beta_p^{i,j}$ that exploits the structure of persistence. In particular, the structure theorem~\cite{zomorodian2004computing} shows 1-parameter persistence modules decompose in an \emph{essentially unique} way---computationally, 
\begin{figure}
\centering
%	\includegraphics[width=0.45\textwidth]{mult_both}
	...
	\label{fig:mult}
\end{figure}
the corresponding Pairing Uniqueness Lemma~\cite{edelsbrunner2000topological} asserts that if $R = \partial V$ decomposes the boundary matrix $\partial$ to a \emph{reduced} matrix $R \in \mathbb{R}^{m \times n}$ using left-to-right column operations, then:
...
\end{corollary}
\noindent Indeed, though $R$ is not unique, its non-zero pivots are, and these pivots \emph{define} the persistence diagram. Indeed,~\eqref{eq:lower_left_rank} in turn enables both $\beta_p^{\ast}$ and $\mu_p^{\ast}$ to be written as a sum of ranks of submatrices of $\partial_p$ and $\partial_{p+1}$:
\begin{corollary}[\cite{dey2022computational, chen2011output}]\label{cor:rank_reduction}
Given a fixed $p \geq 0$, a filtration $(K_\bullet, f)$ of size $N = \lvert K_\bullet \rvert$, and a rectangle $R =  [i,j] \times [k,l] \subset \Delta_+$, the persistent Betti number $\beta_p^{i,j}$ at $(i,j)$ and multiplicity $\mu_p^{R}$ of $R$ is given by:
	\begin{align*}\label{eq:betti_four}
	% \mathrm{rank}(I_p^{1,i})
	\beta_p^{i,j}(K_\bullet, f) &= \lvert K_i^p \rvert - \mathrm{rank}(\partial_p^{1,i}) - \mathrm{rank}(\partial_{p\+1 }^{1,j}) + \mathrm{rank}(\partial_{p\+1}^{i \+ 1, j} ) \numberthis  \\
	\mu_p^{R}(K_\bullet, f) &= \mathrm{rank}(\partial_{p\+1}^{j \+ 1, k})  - \mathrm{rank}(\partial_{p\+1}^{i \+ 1, k})  - \mathrm{rank}(\partial_{p\+1}^{j \+ 1, l}) + \mathrm{rank}(\partial_{p\+1}^{i \+ 1, l}) \numberthis 
...
\end{corollary}
%Dey \& Wang
\noindent These rank-based expressions seem not well known or widely used---to the authors knowledge, the expression for $\mu_p^{R}(K_\bullet)$ was first pointed out by Chen \& Kerber~\cite{chen2011output}. A more recent and explicit derivation is given by Dey \& Wang~\cite{dey2022computational}. For completeness, we give our own detailed proof of corollary~\ref{cor:rank_reduction} in the appendix. 
...
Compared to the classical methods~\cite{edelsbrunner2022computational, zomorodian2004computing}, the primary advantage of these expressions is that (1) they involve strictly \emph{rank} computations, and (2) these computations involve \emph{unfactored} boundary (sub)matrices.
...
Moreover, the structure of $\partial$ and other properties of the rank function
...
suggest tools from spectral theory and ``The Laplacian Paradigm''~\cite{} may be readily transferred to the persistence setting. 
...
The rest of the paper is dedicated to exploring these connections and their ramifications in detail. 
...
\begin{proposition}\label{prop:mu_betti_1}
	Let $\delta > 0$ denote the number from~\eqref{eq:measure} satisfying $i + \delta < j - \delta$. Given $(K, f_\alpha)$ and any rectangle $R = [i,j] \times [k,l] \subset \Delta_{+}$ satisfying $i < j \leq k < l$, the $\mathcal{A}$-parameterized invariants $\beta_p^{i,j} : \mathcal{A} \times K \to \mathbb{N}$ and $\mu_p^{R} : \mathcal{A} \times K \to \mathbb{N}$ defined by: 
	...
	\end{align*}
	yield the correct quantities $\mu_p(R) = \mathrm{card} \left( \, \restr{\mathrm{dgm}_p(f_\alpha)}{R} \, \right)$ and $\beta_p^{i,j} = \mathrm{dim}(H_p^{i,j}(K))$ from~\eqref{eq:measure} and~\eqref{eq:pbn}, respectively, for all $\alpha \in \mathcal{A}$. Moreover, the entries of $\hat{\partial}_p^\ast(\alpha)$ vary continuously in $\mathcal{A}$.
\end{proposition}
...
\end{equation}
The connection between TR and the proposed spectral relaxation is now clear: augmenting the singular value decomposition of $\partial_p$ (or $\partial_p^T$) with a spectral function $\phi$ is akin to the regularizing the spectra of the inverse of the up- (or down-) Laplacian; 
...
In this section, we discuss several of the computational advantages that arise by using a spectral approach to approximating the rank function.
In particular, we focus on iterative subspace acceleration methods that decouple explicit matrix representations from the computation.
...
Combining lemmas~\ref{lemma:matvec_lap} and~\ref{lemma:exact_arith_lanczos} 
