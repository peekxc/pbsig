...
where $\beta_p^{i,j}$ is the rank of the linear map in homology induced by the inclusion $f^{\shortminus1}(-\infty, a_i) \hookrightarrow f^{\shortminus1}(-\infty, a_j)$.
...
Though theoretically sound, diagrams suffer from many practical issues: they are sensitive to outliers, far from injective, expensive to compute \emph{and} to compare. 
...
Tackling the issue of injectivity, Turner et al. propose the \emph{persistent homology transform} (PHT), which is an injective shape statistic of directional diagrams associated to a data set $X \subset \mathbb{R}^d$,  
...
individual diagrams can still be expensive to obtain---the parameterized setting only compounds the scalability issue. Indeed, generalizing the persistence computation to dynamic settings has proven non-trivial~\cite{piekenbrock2021move}.
...
%	\right), \quad 
	\mu_p^{a, b}(K,f) \triangleq \; \min_{\delta > 0} \left(\beta_p^{a \+ \delta, b  \shortminus \delta} - \beta_p^{a \+ \delta, b  \+ \delta} \right) - \left(\beta_p^{a\shortminus \delta, b \shortminus \delta} - \beta_p^{a \shortminus \delta, b \+ \delta} \right)%, \quad \quad \beta_p^{}
%	, \quad \hat\imath \leq \hat\jmath
\end{equation}
where $\beta_p^{a, b }$ is the rank of the lienar map in $f^{\shortminus1}(-\infty, a) \hookrightarrow f^{\shortminus1}(-\infty, b)$.
Note that $(a,b)$ are not necessarily homological critical values in the range of $f$, but rather are arbitrary. 
% \mathrm{rank}(H_p(K_{\hat\imath})\to)
A notable feature of our approach is that it not only avoids explicitly constructing diagrams, but it entirely \emph{matrix-free}, circumventing the reduction algorithm from~\cite{edelsbrunner2022computational} entirely.  
...
The space and time complexity of the computation is linear and quadratic, respectively, in $\lvert K \rvert$ ~\ref{}. 
%requiring only as much memory as is needed to enumerate simplices in the underlying complex $K$, 
Additionally, it can be iteratively approximated, is continuously differentiable, and is particularly efficient to compute over parameterized families of inputs. 
%due to its spectral formulation. 
\\
\\
\noindent 
\textbf{Contributions:} Our primary contribution is the introduction of several families of spectral approximations to the rank invariants---$\hat{\mu}_p$ and $\hat{\beta}_p$\ref{}---all of which are Lipshitz continuous\ref{}, stable under relative perturbations\ref{}, and differentiable on the positive semi-definite cone\ref{}. By a reduction to spectral methods for Laplacian operators\ref{}, we also show these approximations are computable in $\approx O(m)$ memory and $\approx O(mn)$ time, where $m, n$ are the number of $p+1, p$ simplices in $K$, respectively (see section~\ref{sec:computational_imp}). 
...
\begin{equation}\label{eq:relaxation_mapping}
	(X_\alpha, \mathcal{R}, \epsilon, \tau) \mapsto \mathbb{R}^{h}
\end{equation}
where $X_\alpha$ is an $\mathcal{A}$-parameterized input data set, $\mathcal{R} \subset \Delta_+  = \{\, (a,b) \in \mathbb{R}^2 : a \leq b \,\}$ is a region which decomposes as a disjoint union of rectangles $R_1 \, \cup \, \dots \, \cup \, R_h$---we will call such a set a \emph{sieve}---and $(\epsilon, \tau) \in \mathbb{R}_+^2$ are approximation/smoothness parameters, respectively.
...
%	\end{equation} 
	This choice is application-dependent and typically requires a priori knowledge, though in section~\ref{sec:applications} we give evidence that random sampling may be sufficient for vectorization purposes when $\mathcal{R}$ is unknown.
...
%	\item Choose rectangle $R = [i,j] \times [k,l] \subset \Delta_+$ in the upper-half plane $\Delta_+ = \{\}$
	\item Fix a homology dimension $p\geq 0$ and parameters $(\epsilon, \tau ) \in \mathbb{R}_+^2$ representing how \emph{closely} and \emph{smoothly} the relaxation $\hat{\mu}_p^\mathcal{R}$ (defined in step 5 below) should model the quantity: 
%	$\mu_p(f_\alpha; \mathcal{R})$ for all $\alpha \in \mathcal{A}$. 
	...
	\end{equation}
	\item Choose a Laplacian operator $\mathcal{L}: C^p(K, \mathbb{R}) \to C^p(K, \mathbb{R})$ on the real $p$-cochains of $K$ and a 
%	$\tau$-parameterized function $\phi: \mathbb{R} \to \mathbb{R}$ 
	$\tau$-parameterized family of continuous functions $\phi(\cdot , \tau ) : \mathbb{R} \to \mathbb{R}_+$ which converge to $\mathrm{sgn}_+$ as $\tau \to 0$.
%	$\mathcal{R}$.
...
Intuitively, choosing $\mathcal{L}$ determines the kind of geometric/topological information to extract from $(K, f_\alpha)$ and choosing $\phi$ determines how that information is encoded.
%and $\Phi$ determines how this information is incorporated into the mapping. 
	\item For each corner point $(a,b)$ in the boundary of $\mathcal{R}$ and eigenvalue $\lambda$ of $\mathcal{L}$ 
%	\item \in \Lambda(\mathcal{L}_p^{a,b})$
	$$
	\hat{\mu}^\mathcal{R}_p = \sum\limits_{(a,b)}\sum\limits_{i=1}^n s_{a,b} \cdot \phi(\lambda_{a,b}^{i}(\alpha), \tau), \quad \lambda_{a,b}^{(i)} = \Lambda(\mathcal{L}_{a,b})_i
%	\hat{\beta}_{p}^{j,k}(\tau) - \hat{\beta}_{p}^{i,k}(\tau) - \hat{\beta}_{p}^{j,l}(\tau) + \hat{\beta}_{p}^{i,l}(\tau)
	...
	\item Ideas: choose a type of Laplacian (sym, rw...) $\mathcal{L}_{a,b}^\alpha : C^p(K_b^\alpha, K_a^\alpha; \mathbb{R}) \to C^p(K_b^\alpha, K_a^\alpha; \mathbb{R})$. Denote by $\Lambda(\mathcal{L}_{a,b}^\alpha)$ denote spectrum of $\mathcal{L}_{a,b}^\alpha$ ordered non-decreasingly. For each corner point $(a,b)$ of a rectangle $R_i$, $i = 1, \dots, h$, compute: 
%		$$
...
Given a fixed $p \geq 0$, a filtration $K_\bullet$ of size $N = \lvert K_\bullet \rvert$, and a rectangle $R =  [i,j] \times [k,l] \subset \Delta_+^N$, the persistent Betti number $\beta_p^{i,j}$ at $(i,j)$ and multiplicity $\mu_p^{R}$ of $R$ is given by:
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), the boundary matrix $\partial(K_\bullet)$ must maintain a simplexwise filtration order (w.r.t $f_\alpha$) to preserve the inclusion structure of $(K, f_\alpha)$.
...
Observe that (1) the non-zero entries from~\eqref{eq:param_boundary_matrix} vary continuously in $f_\alpha$ and (2) $\partial_p(\alpha)$ decouples into a product of diagonal matrices $D_\ast(f_\alpha)$: 
	\begin{equation}\label{eq:decouple}
		\partial_p(\alpha) = D_p(f_\alpha) \cdot \partial_p(K) \cdot D_{p+1}(f_\alpha) 
	\end{equation}
	where $D_p(f_\alpha)$ and  $D_{p+1}(f_\alpha)$ are diagonal matrices whose non-zero entries are ordered by $\preceq^\ast$ based on restrictions of $f_\alpha$ to $K^p$ and $K^{p+1}$, respectively.
...
%	\mathrm{rank}(\, \partial_p^{i,j}(K_\bullet) \, ) = 
	\mathrm{rank}\big(\, \partial_p^{i,j}(K, f_\alpha) \, \big) \Leftrightarrow \mathrm{rank}\big(\,\hat{\partial}_p^{i,j}(\alpha)\,\big), \quad \hat{\partial}_p^{i,j}(\alpha) \triangleq D_p(\bar{S}_i \circ f_\alpha) \cdot \partial_p(K) \cdot D_{p+1}(S_j \circ f_\alpha) 
	\end{equation}
...
%\begin{remark}
	In~\eqref{eq:rank_equiv_param}, we write $\partial_p(K_{\preceq})$ (as opposed to $\partial_p(K_\bullet)$ or $\partial_p(K, f)$) to emphasize $\partial_p(K_{\preceq})$ is ordered  according to a fixed linear ordering $(K, \preceq)$. Note that evaluating the boundary terms from corollary~\ref{cor:rank_reduction} would require $\partial$ to be explicitly filtered in the simplexwise ordering induced by $f_\ast$---which varies in $\mathcal{A}$. 
...
\begin{definition}[Spectral Rank Approximation]
	...
	\Phi_\tau (X) = \sum\limits_{i=1}^{n} \phi(\sigma_i, \tau) u_i v_i^T, \quad \quad 
...
To better understand the implications of the relaxations discussed so far, we discuss some basic properties. 
...
\noindent Though the cancellations between the relaxed multiplicity $\hat{\mu}_{p}^\ast(\tau)$ and the relaxed cardinalities $\hat{\beta}_{p}^{\ast}(\tau)$ is always satisfied for any $\tau \geq 0$, not all of the properties of PBNs are preserved when $\tau > 0$. This is exemplified by the next proposition, which clarifies the degree to which $\hat{\beta}_p^\ast$ is ``Betti-like'' in the sense of being cumulative. 
...
GIVE AWAY THE PUNCHLINE TRANSITION
...
\subsection{Matrix-free eigenvalue computations}\label{sec:lanczos_it}
