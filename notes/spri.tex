...
\begin{figure}
	...
	\caption{From left to right: Delaunay complex $K$ realized from point set $X \subset \mathbb{R}^2$ sampled with multiple types of noise around $S^1$ (colored by $\alpha$-codensity); codensity vineyard  of $(K, f_\alpha)$ across varying bandwidths $\alpha$ and a fixed sieve point $(a,b) \in \Delta_+$; Tikhonov regularizations $\hat{\beta}_p^{a,b}(\alpha)$ at varying regularization ($\tau$) and sign width ($\omega$) values. Observe lower values of $\tau$ lead to approximations closer to the rank (black) at the cost of smoothness, while larger values can yield very smooth yet uninformative relaxations. 
...
The intuition is that there is likely some choice of bandwidth $\alpha^\ast$ which creates a large gap between the codensities of points affected by Hausdorff noise vs. those that not in the vicinity of $S^1$ at all---in this setting, the impact of the strong outliers on the persistence diagram is minimal. 
Thus, if $(a,b)$ is appropriately chosen, we expect $\beta_p^{a,b}(\alpha)$ to be $0$ for low and high bandwidths and $1$ at the optimal bandwidth $\alpha^\ast$, matching the first Betti number of $S^1$. 
...
One simple such optimization technique is the iterative sparse thresholding method, which alternates between linearization and regularization steps to solve ill-posed or poorly behaved optimization problems~\cite{}.
