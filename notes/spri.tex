...
\begin{remark}
	In~\eqref{eq:rank_equiv_param}, we write $\partial_p(K_{\preceq^\ast})$ (as opposed to $\partial_p(K_\bullet)$ or $\partial_p(K, f)$) to emphasize $\partial_p(K_{\preceq^\ast})$ is ordered  according to a fixed linear ordering $(K, \preceq^\ast)$. Note that, in contrast, evaluating the boundary terms from corollary~\ref{cor:rank_reduction} would require explicitly filtered $\partial$ in the traditional simplexwise manner by $f_\ast$ (which varies in $\mathcal{A}$). 
As the persistent rank invariants from~\eqref{eq:pbn_parameterized} and~\eqref{eq:mu_parameterized} obtained by replacing the constitutive terms in~\eqref{eq:betti_four} and~\eqref{eq:mu_four} using~\eqref{eq:rank_equiv_param} are permutation invariant, they need no longer maintain a simplexwise ordering. 
\end{remark}
...
\begin{equation}\label{eq:rank_def}
	...
	\end{cases}
\end{equation}
where $\Sigma = \mathrm{diag}(\{\sigma_i \}_{i=1}^n)$ are the singular values  of $X$ and $\mathrm{sgn}_+: \mathbb{R} \to \{0, 1\}$ is the one-sided sign function. 
