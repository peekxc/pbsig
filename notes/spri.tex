...
Two important observations stemming from~\eqref{eq:betti_four} and~\eqref{eq:mu_four} are (1) that they are comprised strictly of \emph{rank} computations, and (2) that these computations are carried out on \emph{unfactored} boundary (sub)matrices.
...
The former suggests techniques from sparse recovery or compressive sensing~\cite{zhao2012approximation} may find utility in the persistence setting, while the latter suggest these quantities may be iteratively approximated in a matrix-free fashion using e.g. randomized algorithms~\cite{chen2011output} or iterative subspace methods~\cite{golub2013matrix, parlett1994we}. 
...
%\end{equation}
Thus, computing $\beta_p^{a,b}$ via~\eqref{eq:pbn} reduces to one nullity computation and one subspace intersection computation. % via matrix reduction or projector-based techniques 
While the former is easily solved, direct methods that solve the latter typically do so in $\Omega(N^3)$ time and $\Omega(N^2)$ space by finding a basis via reduction or projector-based techniques~\cite{golub2013matrix}.
...
In typical dynamic persistence settings (e.g.~\cite{cohen2006vines}), a decomposition $R = \partial V$ of the boundary matrix $\partial$ must be permuted and modified frequently to maintain a total order with respect to $f_\alpha$.  %$P R P^T = (P \partial P^T)(P V P^T)$
