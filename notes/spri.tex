...
The former suggests variational perspectives of the rank function may yield useful relaxations of~\eqref{eq:mu_four} and~\eqref{eq:betti_four}; the latter suggests their computation may be done via iterative, \emph{matrix-free} approximation schemes. 
...
While the former is easy to re-cast as a spectral computation, computing the latter typically requires obtaining bases via matrix decomposition. Constructing these bases using conventional~\cite{bhatia2013matrix, golub2013matrix} or persistence-based~\cite{memoli2022persistent, zomorodian2004computing} algorithms effectively\footnote{Note about matrix multiplication constant} requires $\Omega(N^3)$ time and $\Omega(N^2)$ space.
As the persistence algorithm also exhibits $O(N^3)$ time complexity and completely characterizes $\beta_p^{a,b}$ over \emph{all} values $(a,b) \in I \times I$, there is little incentive to compute $\beta_p^{a,b}$ with such direct methods. Because of this, we will focus on expressions~\eqref{eq:betti_four} and~\eqref{eq:mu_four} throughout the rest of the paper.
...
At a high level, the operator $\Phi_\tau(X)$ from Definition~\eqref{def:lowner} is akin to the well-studied notion of a \emph{matrix function} often student in the context of square matrices~\cite{bhatia2013matrix}. 
However, when $\phi$ satisfies the conditions from~\eqref{eq:phi}, the corresponding operator $\Phi_\tau$ exhibits a variety of attractive properties related to monotonicity and differentiability, a few of which are recorded below.
\noindent
...
For example, consider a weighted graph $G = (V, E)$ with $n = \lvert V \rvert$ vertices with graph Laplacian $L_G : \mathbb{R}^n \to \mathbb{R}^n$. 
...
To simulate a heat diffusion process on $G$ from an initial distribution of heat $v(0) \in \mathbb{R}^n$ at time $t = 0$, it is sufficient to construct the \emph{heat kernel} $H_t \triangleq \mathrm{exp}(-t \cdot L_G)$ of $L_G = U \Lambda U^T$: 
...
$$v(t) = H_t \, v(0), \quad \text{ where } H_t = U \mathrm{exp}(-t \Lambda) U' = \sum\limits_{i=1}^n e^{-t \lambda_i} \, u_i \, u_i^T$$
The heat kernel is invariant under isometric deformations, stable under perturbations, and is known to contain rich multiscale geometric information due to its close connection to geodesics~\cite{}. Now, consider the parameterizing $\phi$ from~\eqref{} given by setting $\nu(\tau) = \tau$ and $p(x) = \mathrm{exp}(-x_+)$. Plugging these into~\eqref{}, we have: 
\begin{equation}\label{eq:tikhonov_sf}
	\phi(x, \tau) = \int\limits_{0}^z \hat{\delta}(z, \tau) dz = \frac{1}{\tau}\int\limits_{0}^z \mathrm{exp}(-z / \tau) dz = 1 - \mathrm{exp}(- x / \tau) % \nu(1/\epsilon) \cdot p \big( z \, \nu (1/\epsilon) \big ) 
\end{equation}
Thus, in this parameterization, the parameter $\tau$ can be interpreted as the reciprocal of the diffusion time, i.e. $t = 1 / \tau$. Moreover, as the graph Laplacian $L_G$ is given by the composition of boundary operators $\partial_1 \partial_1^T$, we have when $A = L_G$: 
\begin{equation}\label{eq:heat_trace}
%	\Phi_\epsilon(A) = (A^T A + \epsilon \, I_n)^{-1} A^T A, \quad \quad 
	\lVert \Phi_\tau(A) \rVert_\ast = \sum\limits_{i = 1}^n 1 - \mathrm{exp}(-t \cdot \sigma_i) = n - \mathrm{Tr}\left[H_t\right]
\end{equation}
The quantity in~\eqref{eq:heat_trace} has been studied in a variety of contexts, see e.g.~\cite{}. In this sense, we interpret the quantities obtained by applying ... as \emph{geometrically sensitive rank invariant approximations}.
...
\end{equation}
\noindent 
All three operators $\Delta_p$, $L_p^{\text{up}}$, and $L_p^{\text{dn}}$ are symmetric, positive semi-definite, and compact---moreover, the inertia of their spectra is equivalent, implying they must have identical ranks~\cite{horak2013spectra}. % TODO: verify
...
%Indeed, $\beta_p^{\ast}$ is an integer-valued invariant and thus is not amenable to first-order optimization. 
...
% satisfying $\hat{\beta}_p^{i,j} \to \beta_p^{i,j}$ as $\tau \to 0$.  

%The goal is detect whether or not $X$ lies near $S^1$, as shown by 
%We reframe the problem as follows: rather than filtering $K$ according to its ambient metric $d_X$, we ask whether there \emph{exists} significant topological information at any density scale $\alpha$. Generically, we consider the following optimization problem:   
...
%Existing ways of handling outliers has largely focused on their removal~\cite{} or on extending persistence to the $n$-parameter setting. 
%The simplest way of handling outliers is to remove them. We re-frame, this is similar in spirit to the ``distance to measure''~\cite{}, which 
%exists a filter $f$ 
%consider a simplicial complex $(K, f_\alpha)$ over a fixed Delaunay complex $K$ constructed over a point set $X \subset \mathbb{R}^d$, shown in Figure~\ref{}. 
%due to their tendency to prevent the detection of an underlying topological space via persistence.

%a common setting is the one where the data set $(X, d_X)$ comes equipped with some notion of density $f: X \to \mathbb{R}_+$, and one would like to build a persistence diagram on $d_X$ in a way that is robust to local fluctuations in density. 
...
%\begin{figure}[ht]
%	\centering
%	\includegraphics[width=0.78\textwidth]{presentations/images/codensity_dgm_ex}
...
%\begin{figure}[h]
%	\centering
%	\includegraphics[width=0.90\textwidth]{presentations/images/combined_mult}
