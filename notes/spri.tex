...
In this section, we introduce the proposed relaxation successively by relaxing and generalizing different aspects of the expressions from Corollary~\ref{cor:rank_reduction} to the parameterized setting. To motivate the advantages these expressions provide, it is instructive to derive the more traditional expressions used to compute the persistent rank invariants. 
...
Mangasarian et al.~\cite{mangasarian1994class} proposed a natural relaxation of $\mathrm{sgn}_+$ may be obtained by integrating smoothed variations $\hat{\delta}$ of the Dirac delta function $\delta$:  
