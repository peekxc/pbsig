...
Since $Q$ is a similarity transform, $\Lambda(T) = \Lambda(A)$, i.e. $T$ preserves the spectrum of $A$ and the problem of finding eigenvalues reduces to diagonalizing $T$.
...
As in~\cite{parlett1994we}, Proposition~\ref{prop:spectral_rank_complexity} assumes an exact arithmetic computation model to simplify both the presentation of the theory and the corresponding complexity statements. In practice, finite-precision arithmetic introduces both rounding and cancellation errors into the eigenvalue computation, which primarily manifests as loss of orthogonality between the Lanczos vectors. These errors not only affect the convergence rate and termination of Lanczos, but they in fact cause a host of other issues, such as the generation of spurious ``ghost'' eigenvalues~\cite{}.
Though many improvements to basic Lanczos algorithm have been proposed (i.e. selective re-orthogonalization, implicit restarts, subspace deflation, etc.), such enhancements are quite expensive in practice for purpose of rank estimation. 
...
Unlike eigenvalue estimation, it has been shown that using Lanczos in finite precision arithmetic is actually stable for matrix function approximation in the sense that Paige's A27 Lanczos variant~\cite{} up to degree $k$ is no worse than the best degree-$p$ polynomial approximation to $f$, for any $p < k$. For general matrix functions, this implies that finite-precision Lanczos essentially matches strongest known exact arithmetic bounds.
...
In certain computational topology settings, one knows about the underyling toplogy of interest but. 
