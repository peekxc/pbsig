...
\begin{equation}\label{eq:pbn_cont}
	x = (i,j) \in \mathrm{dgm}_p(f) \; \Longleftrightarrow \; \mu_p(x) > 0 \; \Longleftrightarrow \; \min_{\delta > 0} \left(\beta_p^{i \+ \delta, j \shortminus \delta} - \beta_p^{i \+ \delta, j \+ \delta} \right) - \left(\beta_p^{i \shortminus \delta, j \shortminus \delta} - \beta_p^{i \shortminus \delta, j \+ \delta} \right) > 0
\end{equation}
