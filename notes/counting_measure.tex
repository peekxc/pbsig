...
Cohen-Steiner et al.~\cite{cohen2005stability} studied PH from a more discrete perspective by defining the persistence via \emph{multiplicities}: given a tame function $f: \mathcal{X} \to \mathbb{R}$ over a topological space $\mathcal{X}$, its homological critical values $\{ a_i \}_{i=1}^n$, and an interleaved sequence $\{ b_i \}_{i=0}^n$ satisfying $b_{i-1} < a_i < b_i$ for all $1 \leq i \leq n$,~\cite{cohen2005stability} defined the $p$-th persistence diagram $\mathrm{dgm}_p(f)$ over $f$ as the multiset: 
...
\end{equation}
for all $0 \leq i < j \leq n+ 1$. The inclusion-exclusion property that~\eqref{eq:multiplicity} conveys suggests that diagrams completely characterize their PBNs, illuminating an intrinsic connection between diagrams and their Betti numbers. 
...
Chazal~\cite{chazal2016structure} further studied the duality between diagrams and PBNs from a measure-theoretic perspective, focusing on persistence modules indexed over the real line. 
...
Nonetheless, the counting invariants have certain computational advantages over the reduction algorithm from~\cite{edelsbrunner2022computational}---their computations are \emph{localized} to subcomplexes of the filtration, and they may be determined through only rank computations~\cite{zomorodian2004computing}. 
