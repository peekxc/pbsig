...
The direct consequence of~\cite{edelsbrunner2022computational} is that if one is interested in computing any of the PBNs of some space $\mathcal{X}$, then it is sufficient to compute $\mathrm{dgm}_p(\mathcal{X})$ and read them off directly via~\eqref{eq:betti_mult}. Conversely, through particular combinations of Betti numbers, one can recover portions of a spaces persistence diagram via~\eqref{eq:dgm} and~\eqref{eq:multiplicity}.
...
Cerri et al.~\cite{cerri2013betti} incorporate this interpretation in studying the stability of PBNs in multidimensional persistence via what they define as \emph{proper cornerpoints}. These are points $x = (\, \hat\imath, \hat\jmath \,) \in \Delta_+$ satisfying $\mu_p(x) > 0$, where $\mu_p(x)$:
...
Nonetheless, there are certain advantages the PBN computation has over the ``standard'' reduction algorithm from~\cite{edelsbrunner2022computational}. 
% add more 
