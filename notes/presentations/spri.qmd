...
2. is smooth + differentiable on $\mathbf{S}_+$
3. $(1{\textstyle -}\epsilon)$ approximates $\beta_p^{i,j}$ for any $\epsilon > 0$ + essentially $O(n^2)$ time
4. Vectorizes non-harmonic spectra of Laplacian operators
...
![](spri_overview.png){width=88% height=100% fig-align="center"}
...
To our knowledge: (1) noted in passing by @edelsbrunner2000topologica, though @dey2022computational first to explicitly prove (2). (3) has been exploited by @chen2011output. (2) & (3) are connected to relative homology.

:::

::: {.notes}

- (1) was mentioned in passing by Edelsbrunner et al.
...
Replacing $S \mapsto \mathcal{S}$ ensures $\partial_p^{i,j}(\alpha)$ is element-wise continuous
...
[1]: Great LASSO introduction: https://thaddeus-segura.com/lasso-ridge/
