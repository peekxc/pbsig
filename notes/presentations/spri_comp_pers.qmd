...
![](lsst.png){.fragment fragment-index=4 width="375" height="375"}
...
In this talk we introduce a <span style="color:orange"> _spectral-relaxation_ </span> of the rank invariant that:

<div style="margin-left:2.5em;">

1. Continuously interpolates the _persistent rank_ function
2. Vectorizes the spectra of Laplacian operators
3. Is smooth + differentiable$^*$
4. $(1{\textstyle -}\epsilon)$ approximates $\beta_p^{i,j}$ for any $\epsilon > 0$ + essentially $O(n^2)$ time 
5. Is computable in a "matrix-free" fashion in $O(n)$ memory 
...
![](spri_overview.png){width=68% height=80% fig-align="center"}

:::

:::{.aside}
\* The relaxation derives from Schatten-1 norms that are continuously differentiable over the positive semi-definite cone 
:::
...
<ul>
<li>Introduction &amp; Motivation   <ul>
<li>Diagram vectorization and optimization</li>
<li>The effective intractability of reduction</li>
<li>Duality between diagrams and their ranks</li>
</ul>
</li>
</ul>
<ul>
<li><p>Derivation of relaxation </p>
<ul>
<li>Parameterizing $p$-chains with $\mathbb{R}$ coefficients</li>
<li>Replacing boundary operators with Löwner operators</li>
<li>Connections to combinational Laplacians</li>
</ul>
</li>
<li><p>Experiments </p>
<ul>
<li>Shape signatures via directional transform</li>
<li>Shape signatures via intrinsic metrics  </li>
<li>Codensity example </li>
</ul>
</li>
</ul>
...
<ol>
  <li> The inner terms are _unfactored_ linear operators </li>
  <li> The rank functions may be characterized _spectrally_ </li>
  <li> We can better leverage the theory of _persistent measures_$^{\ast}$</li>
</ol>
...
__Relaxation \#1__: Parameterize $C_p(K; \mathbb{R})$ with $\mathcal{S} \circ f_\alpha : K \to \mathbb{R}_+$ 

where $\mathcal{S}: \mathbb{R} \to [0,1]$ is a _smoothstep_ function 
...
## Rank Invariances when $\mathbb{F} = \mathbb{R}$
...
Instead of solving $Ax = b$, consider the "regularized" least-squares objective: 
...
![](lasso.png){width=50% fig-align="center"}
...
Under the appropriate hyper-parameter settings^[This $\phi$ corresponds to setting $\nu(\epsilon) = \sqrt{\epsilon}$ and $p(x) = 2x (x^2 + 1)^{-2}$.], $\phi$ takes the form:
...
The corresponding Lowner operator and its Schatten $1$-norm is given by:
...
This the _Tikhonov regularization_ in standard form used in $\ell_1$-regression (LASSO)
...
![](gh_data_pose.png){width=425 height=100% fig-align="center"}
...
![](camel1_interp2.png){width=950 height=100% fig-align="center"}
...
![](dw_chi_comp.png){width=750 height=100% fig-align="center"}
...
::: {.column layout-align="right"}
![](vineyard.gif){width=60%}
:::

::: {.column layout-align="center"}
![](dgm_opt.png){height=100% fig-align="center"}
:::

::: {.column layout-align="left"}
![](complex_plain.gif){width=60%}
:::
...
![](codensity_ex1.png){width=60%, fig-align="center"}
...
![](codensity_ex2.png){width=60%, fig-align="center"}
...
![](codensity_ex3.png){width=60%, fig-align="center"}
...
![](combinatorial_explosion.png){width=60%, fig-align="center"}[^1]
...
![](codensity_ex4.png){width=60%, fig-align="center"}
...
![](codensity_ex5.png){width=60%, fig-align="center"}
