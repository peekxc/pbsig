...
__Goal:__ "Relax" these invariants using the spectral characterization of the rank function:

$$\mathrm{rank}(X) = \sum_{i=1}^n \, \mathrm{sgn}_+(\sigma_i) $$
...
$$\mathrm{rank}(X) = \sum_{i=1}^n \, \mathrm{sgn}_+(\sigma_i) \approx \sum_{i=1}^n \, \phi(\sigma_i, \tau) $$
...
$$\mathrm{tr}(\Phi_\tau(X)) = \sum\limits_{i=1}^n \phi(\sigma_i, \tau), \quad \Phi_\tau(X) \triangleq U \phi_\tau(\mathbf{\Sigma}) V^T $$
...
## Application: Manifold detection (time permitting)
