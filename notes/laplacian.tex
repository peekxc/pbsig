...
\mathcal{L}_p^{a,b}(\alpha) & \triangleq D_p(\bar{S}_a \circ f_\alpha) \cdot \partial_{p+1}(K_\preceq) \cdot D_{p+1}^2(S_b \circ f_\alpha) \cdot \partial_{p+1}^T(K_\preceq) \cdot D_p(\bar{S}_a \circ f_\alpha) 
...
In the context of inner products, 
	One may interpret sending $p$-simplices to $0$ as restricting to a sub-complex $L \subseteq K$, which suggests simplicial pairs $(L, K)$ satisfying $L \hookrightarrow K$ as in~\cite{memoli2022persistent}. However, our definition 
\end{remark}
