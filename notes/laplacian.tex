...
Indeed, a direct result of the singular value decomposition (SVD) is that the eigenvalues of $XX^T$ (or $X^T X$) are given by the squares of the singular values of $X$; thus, we may study the singular values of boundary operators through the spectra of Laplacians.  
For any simplicial complex $K$, its \emph{$p$-th combinatorial Laplacian} $\Delta_p(K)$ is given by: 
...
Let $(K, f_\alpha)$ denote parameterized family of filtrations of a simplicial complex $K$ equipped with a fixed but arbitrary linear extension $ \preceq$ of its face poset and fixed orientations $s(\sigma)$ inherited from the total order on the vertex set $(V, \preceq)$.
...
\mathcal{L}_p^{a,b}(\alpha) & \triangleq D_p^{1/2}(\bar{S}_a \circ f_\alpha) \cdot \partial_{p+1}(K_\preceq) \cdot D_{p+1}(S_b \circ f_\alpha) \cdot \partial_{p+1}^T(K_\preceq) \cdot D_p^{1/2}(\bar{S}_a \circ f_\alpha) 
...
\end{align}
where $D_p(f)$ denotes a diagonal matrix whose entries represent non-negative weights on the $p$-simplices of $K$ that depend on $f$ and $S$ ($\bar{S}$).
As before with~\eqref{eq:rank_equiv_param}, fixing step function $S_a$ and $\bar{S}_b$ at values $a, b \in \mathbb{R}$ yields operators whose ranks correspond to the ranks of certain ``lower-left'' submatrices of the matrix decomposition of $(K, f)$. In particular, if $R = \partial V$ is the decomposition of $(K, f_\alpha)$ for some fixed choice of $\alpha \in \mathcal{A}$ and $i = \sum_{\sigma \in K} (S_a \circ f_\alpha)(\sigma)$ and $j = \sum_{\sigma \in K} (S_b \circ f_\alpha)(\sigma)$, then we have: 
\begin{align*}	
	\mathrm{rank}(R_p^{i,j})& = \mathrm{rank}(\partial_p^{i, j})  = \mathrm{rank}(\hat{\partial}_p^{a,b}(\alpha)) \\
	& = \mathrm{rank}(D_{p-1}(\bar{S}_a \circ f_\alpha) \cdot \partial_p(K_\preceq) \cdot D_{p}(S_b \circ f_\alpha)) \\
	 ...
	 & = \mathrm{rank}(\mathcal{L}_{p-1}^{a,b}(\alpha)) \numberthis
\end{align*}
where the second last equality uses the identity $\mathrm{rank}(X) = \mathrm{rank}(X^T X)$. Thus, we may substitute the parameterized boundary operators from~\eqref{} with weighted Laplacian operators $\partial_{p+1}^\ast \mapsto \mathcal{L}_p^\ast$ equipped with the appropriate down- and up-step functions $S_\ast$ and $\bar{S}_\ast$, respectively.   

\begin{remark}
In the context of inner products, 
	One may interpret sending $p$-simplices to $0$ as restricting to a sub-complex $L \subseteq K$, which suggests definitions might better be defined with simplicial pairs $(L, K)$ satisfying $L \hookrightarrow K$ as in~\cite{memoli2022persistent}. However, our definition 
\end{remark}
