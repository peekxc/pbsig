...
A \emph{filtration} $K_\bullet = \{K_i\}_{i\in I}$ of a simplicial complexes indexed by a totally ordered set $I$ is a family of complexes such that $i< j \in I \implies K_i \subseteq K_j$. $K_\bullet$ is called \emph{simplexwise} if $K_j \smallsetminus K_i = \{\sigma_j\}$, and $K_\bullet$ is \emph{essential} if $i \neq j$ implies $K_i \neq K_j$ whenever $j$ is the immediate successor of $i$ in $I$:
...
Thus, the lower-left submatrices of the filtered boundary matrix $\partial$ has the same rank as $R$. 
Let $\hat{\partial} = \partial_p$ and $\bar{\partial} = \partial_{p+1}$. Utilizing the result above, observe $\beta_p^{i,j}$ may be written as: 
\begin{equation}\label{eq:betti_four}
	\beta_p^{i,j} = \lvert \hat{\partial}_1^i \rvert - \mathrm{rank}(\hat{\partial}_1^i) - \mathrm{rank}(\bar{\partial}_1^j) + \mathrm{rank}(\bar{\partial}_{i+1}^j)
\end{equation}
where $\lvert \, A \, \rvert = \mathrm{dim}(\mathrm{dom}(A))$ denotes the order of the matrix. Thus, we may write the persistent Betti number as a combination of rank computations performed directly on the dimension $p$ and $(p+1)$ boundary matrices. 
% DONT ERASE
...
\begin{equation}\label{eq:matrix_pchain}
	...
	c(\sigma)  & \text{if } \sigma_i \in \partial_p(\sigma_j) \\
	0 & \text{otherwise}
   \end{cases}
\end{equation}
where $c(\sigma) \in \mathbb{F}$ is an arbitrary constant satisfying $c(\sigma) = -c(\sigma')$ if $\sigma$ and $\sigma'$ are opposite orientations of the same simplex.
In practice, $c(\sigma)$ is typically set to $\pm 1$.  Towards relaxing the persistent Betti computation in dynamic setting, we propose an alternative choice for $c(\sigma)$ which endows continuity in the entries of $\partial_p$ in $T$.
...
\partial_p^t[i,j] = \begin{cases}
	\lvert \epsilon - \mathrm{diam}_t(\sigma) \rvert_{+} & \text{if } \sigma_i \in \partial_p(\sigma_j)\\
	0 & \text{otherwise}
\end{cases} 
...
\end{definition}
\noindent
We now show a few properties that $\partial_p^t$ exhibits which is advantageous for optimization. Clearly the entries of $\partial_p^t$ must vary continuously in $t \in T$. Moreover, for all $t, t' \in T$, we have:
% 
...
%\end{definition}
\begin{enumerate}
	\item $\mathrm{rank}(\partial_p^t) = \mathrm{dim}(\mathrm{B}_{p-1}(K_t))$ where $K_t = \mathrm{Rips}_{\epsilon}(\delta_{\mathcal{X}}(t))$  
	\item $\lVert \partial_p^t - \partial_p^{t'} \rVert_F \leq C \lvert t - t' \rvert (p+1)$ when $\delta_\mathcal{X}$ is $C$-Lipshitz over $T$
	\item $\lVert \partial_p^t \rVert_{2} \leq \sqrt{\kappa} \, (p+1) \, \epsilon$ where $\kappa = \max \sum\limits_{t \in T}\sum\limits_{\sigma \in K_t}\mathds{1}(\mathrm{diam}(\sigma) \leq \epsilon)$
...
In light of the formulation of~\eqref{eq:betti_four}, we may interpret $\beta_p^{i,j}$ from a function composition perspective: 
