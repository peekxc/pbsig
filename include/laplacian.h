...
struct UpLaplacian {
  ...
  inline void __matvec(F* x) const noexcept { 
    ...
    // Enumerate the boundaries of all the simplices
    int s = 1; 
    // #pragma omp simd
    // for (auto s_it = simplices.begin(); s_it != simplices.end(); ++s_it){
    ...
    // std::cout << "FI: " << face_indices.size() << std::endl;
    // std::cout << "me: " << *std::max_element(face_indices.begin(), face_indices.end()) << ", np: " << np << std::endl;
